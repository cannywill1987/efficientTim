...
@available(iOS 15.0, *)
class TimelineListViewController: UIViewController {
    ...
    private var model:ScreenTimeSelectAppsModel? = nil;
    // 创建你的SwiftUI视图
...
//        self.navigationController?.setNavigationBarHidden(false, animated: false)
    ...
    override func viewDidLoad() {
        ...
        if model == nil {
            model = ScreenTimeSelectAppsModel()
        }
        model!.activitySelection = familyActivitySelection ?? FamilyActivitySelection()
            
        // 创建一个UIHostingController实例
        if #available(iOS 16.0, *) {
            let hostingController = UIHostingController(rootView: UITimelineListView(viewController: self, model: model!))
//            item.viewController = self
//            DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) {
//                Utility.dismissViewController(controller: hostingController, animated: false)
//            }
