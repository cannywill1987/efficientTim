//
//  NewCustomizeMissionListView.swift
//  NewCustomizeMissionListView
//
...
struct NewCustomizeMissionListViewEntryView : View {
...
struct NewCustomizeMissionListView: Widget {
    let kind: String = "NewCustomizeMissionListView"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: Provider()) { entry in
            if #available(iOS 17.0, *) {
                NewCustomizeMissionListViewEntryView(entry: entry)
...
struct MissionListView: Widget {
    ...
    var body: some WidgetConfiguration {
        ...
        StaticConfiguration(kind: kind, provider: Provider()) { entry in
            if #available(iOSApplicationExtension 17.0, *) {
                NewCustomizeMissionListViewEntryView(entry: entry) // Widget's main view
                    .containerBackground(.fill.tertiary, for: .widget)
            } else {
                // Fallback on earlier versions
                NewCustomizeMissionListViewEntryView(entry: entry) // Widget's
            } // Background configuration for newer versions
        }
        .contentMarginsDisabled() // Disable default content margins for the widget
        .configurationDisplayName("customize_mission".localizable()) // Display name for the widget configuration
        .description("") // Description for the widget configuration
