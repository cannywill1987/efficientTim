...
//        var action: TimerAction? // 定义计时器的操作参数
        ...
        var isPaused: Bool = true // Default state is paused
...
//        self.isCountDown = isCountDown
    ...
    func updateAllActivity(counterStatusEnum: Int, totalElapsedTime: TimeInterval, isPause: Bool, startTimeForElapse: Date?) async {
        let activities = Activity<IslandAttributes>.activities
        for activity in activities {
            var state:IslandAttributes.ContentState = activity.content.state;
    ...
    func perform() async throws -> some IntentResult {
        Task {
            ...
            case .beginPause:
                ...
                await self.updateAllActivity(counterStatusEnum: 1, totalElapsedTime: totalElapsedTime, isPause: true, startTimeForElapse: nil)
                break;
            case .beginContinue:
                var  totalElapsedTime = await getAllActivityByTotalElapsedTime()

                await self.updateAllActivity(counterStatusEnum: 0, totalElapsedTime: TimeInterval(totalElapsedTime), isPause: false, startTimeForElapse: Date())
                break;
            case .beginStart:
                break;
...
//        var endDate: Date
    ...
    func getItem(action: TimerAction) -> TimerIntent2 {
        ...
        return TimerIntent2(
            ...
            //            isCountDown: context.state.isCountingDown
        );
        ...
        ActivityConfiguration(for: IslandAttributes.self) { context in
            ...
            DynamicIsland {
                ...
                DynamicIslandExpandedRegion(.leading) {
                    Label {
                        Text("\(context.state.title)-\(context.state.totalElapsedTime)-\(context.state.isCountingDown)").lineLimit(1)
                    } icon: {
