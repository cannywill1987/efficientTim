...
struct TimerManager {
    static var isLoading: Bool = false // 是否处理中
}
...
struct TimerIntent: AppIntent {
    ...
    // 初始化函数
    init() { }
    init(action: TimerAction, missionID: String, timestamp: Int?) {
        self.action = action
...
//        NetworkRequest.shared.console(pairParameters: ["1111111111":"1111111"])

//        TimerManager.isLoading = true;
//        WidgetCenter.shared.reloadAllTimelines()

            let objectId = self.missionID ?? "";
//            let timestamp = TimerIntent2.getCurrentTimeStampByMilliSeconds();
//            NetworkRequest.shared.console(pairParameters: ["1111111111":"1111111", "objectId":objectId, "timestamp": self.timestamp])
            if let responseData:[String: Any]? = await NetworkRequest.shared.startRequestWithPost(pairParameters: ["objectId":objectId, "timestamp": self.timestamp], url: Apis.updateFlomoClickInsMissionModel) {
                do {
//                    NetworkRequest.shared.console(pairParameters: ["222222222":"22222222", "objectId":objectId, "timestamp": self.timestamp])
...
//                        NetworkRequest.shared.console(pairParameters: ["3333333333":"333333333", "objectId":objectId, "timestamp": self.timestamp])
                        //                                flomoMissionData.
                        let dateString: String = TimerIntent.formatTimestampToDateString(timestamp: Int(timestamp ?? 0));
...
//                        NetworkRequest.shared.console(pairParameters: ["44444444":"44444444", "objectId":objectId, "timestamp": self.timestamp])
...
//                    var weeklyCompletionRates: [Double] = Array(repeating: 0.0, count: 7)
                    ...
                    let color: Color = Color(hex: UInt(missionModel.color ?? 0xff8800))  // 任务颜色
...
struct NewClockinWeekendWidgetEntryView : View {
    ...
    func isToday(index: Int) -> Bool{
        var todayIndex: Int = Calendar.current.component(.weekday, from: entry.date) - 1
//        if (todayIndex == 7) {
//            todayIndex = 0
//        } else {
...
//        }
    ...
    var body: some View {
        VStack {
            // 顶部显示星期几
            HStack {
//                Text("\(TimerManager.isLoading ? "处理中" : "")").foregroundColor(.white)
                ...
                HStack {
                    ...
                    ForEach(0..<7) { dayIndex in
                        if #available(iOSApplicationExtension 17.0, *) {
                            Button(intent: TimerIntent(action: TimerAction.addMinute, missionID: task.objectId ?? "",timestamp: task.timestamp + dayIndex * 24 * 60 * 60 *  1000)) {
                                Rectangle()
