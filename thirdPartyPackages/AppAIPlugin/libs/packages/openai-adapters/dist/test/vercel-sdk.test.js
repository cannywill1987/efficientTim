...
dotenv.config();
function testAiSdkProvider(config) {
    const { skipTools, skipSystemMessage, aiSdkProviderId, model, apiKey, apiBase, } = config;
...
describe("Contract Compatibility Tests", () => {
    describe("OpenAI: Original vs AI SDK", () => {
        ...
        test("streaming output format matches", async () => {
            ...
            console.log(`OpenAI - Original chunks: ${originalChunks.length}, AI SDK chunks: ${aiSdkChunks.length}`);
            expect(aiSdkChunks.length).toBeGreaterThan(0);
            const originalContentChunks = originalChunks.filter((c) => c.choices.length > 0);
            const aiSdkContentChunks = aiSdkChunks.filter((c) => c.choices.length > 0);
    ...
    describe("Anthropic: Original vs AI SDK", () => {
        ...
        test("streaming output format matches", async () => {
            ...
            expect(originalChunks.length).toBeGreaterThan(0);
            console.log(`Anthropic - Original chunks: ${originalChunks.length}, AI SDK chunks: ${aiSdkChunks.length}`);
            expect(aiSdkChunks.length).toBeGreaterThan(0);
            const originalContentChunks = originalChunks.filter((c) => c.choices.length > 0);
            const aiSdkContentChunks = aiSdkChunks.filter((c) => c.choices.length > 0);
            expect(originalContentChunks.length).toBeGreaterThan(0);
