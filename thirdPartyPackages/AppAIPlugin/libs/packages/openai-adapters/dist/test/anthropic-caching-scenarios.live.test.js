...
const TOOLS = [
    ...
    },
    {
        ...
        function: {
            name: "write_file",
            description: "Write content to a file at the given path. If the file exists, it will be overwritten. If it doesn't exist, a new file will be created. Always provide the complete intended content of the file.",
            parameters: {
                type: "object",
    ...
    },
    {
        ...
        function: {
            ...
            description: "Run a terminal command in the workspace directory. Use this for executing build commands, running tests, installing packages, or any other command-line operations.",
            parameters: {
                type: "object",
                required: ["command"],
                properties: {
    ...
    },
    {
        ...
        function: {
            name: "list_files",
            description: "List files and directories at the given path. If the path is a directory, lists all files and subdirectories within it.",
            parameters: {
                type: "object",
                required: [],
                properties: {
    ...
    },
    {
        ...
        function: {
            name: "search_code",
            description: "Search for a pattern across all files in the project. Returns matching lines with file paths and line numbers. Supports regex patterns.",
            parameters: {
                type: "object",
                required: ["pattern"],
                properties: {
...
function totalInputTokens(usage) {
    ...
    return ((details?.cache_read_tokens ?? 0) +
...
// ─── Tests ──────────────────────────────────────────────────────────
describe.skipIf(!API_KEY)("Anthropic Prompt Caching - Battle Test Scenarios", () => {
    ...
    describe("Scenario 1: Tool use round-trip", () => {
        const api = makeApi();
        let turn1Content = "";
        test("Turn 1: initial request with tool use history creates cache", { timeout: 30_000 }, async () => {
            ...
            const response = await api.chatCompletionNonStream({
                ...
                messages: [
                    ...
                    {
                        role: "user",
                        content: "I see the user service. Can you add a method to search users by name?",
                    },
                ],
                tools: TOOLS,
                max_tokens: 256,
        ...
        test("Turn 2: follow-up after tool use hits cache", { timeout: 30_000 }, async () => {
            const response = await api.chatCompletionNonStream({
                model: MODEL,
                messages: [
                    { role: "system", content: SYSTEM_MESSAGE },
                    ...
                    {
                        role: "user",
                        content: "Good. Now also add pagination support to that search method.",
                    },
                ],
                tools: TOOLS,
                max_tokens: 256,
    ...
    describe("Scenario 2: Parallel tool calls", () => {
        const api = makeApi();
        test("Turn 1: conversation with parallel tool calls creates cache", { timeout: 30_000 }, async () => {
            const response = await api.chatCompletionNonStream({
                model: MODEL,
                messages: [
                    { role: "system", content: SYSTEM_MESSAGE },
                    {
                        role: "user",
                        content: "Show me the team routes and list the test files for the API package.",
                    },
                    {
                        role: "assistant",
                        content: null,
                    ...
                    {
                        role: "user",
                        content: "Good. The team routes look basic. Can you add CRUD endpoints for team members?",
                    },
                ],
                tools: TOOLS,
                max_tokens: 256,
        ...
        test("Turn 2: follow-up after parallel tool calls hits cache", { timeout: 30_000 }, async () => {
            const response = await api.chatCompletionNonStream({
                model: MODEL,
                messages: [
                    { role: "system", content: SYSTEM_MESSAGE },
                    ...
                    {
                        role: "assistant",
                        content: "I'll add the team member CRUD endpoints to the routes file.",
                    },
                    {
                        role: "user",
    ...
    test("Scenario 3: Long 8-turn conversation maintains high cache rate", { timeout: 60_000 }, async () => {
        const api = makeApi();
        const messages = [
            { role: "system", content: SYSTEM_MESSAGE },
            {
                role: "user",
                content: "I'm getting a TypeScript error in the auth middleware. The error is: Type 'string | undefined' is not assignable to type 'string'. How do I fix it?",
            },
        ];
        const exchanges = [
    ...
    describe("Scenario 4: Large tool result payload", () => {
        ...
        const largeFileContent = Array.from({ length: 200 }, (_, i) => {
            ...
            if (line === 6)
                return `${line} |`;
            if (line === 7)
                return `${line} | @Injectable()`;
            if (line === 8)
                return `${line} | export class ProjectService {`;
            if (line === 9)
        ...
        test("Turn 1: request with large tool result creates cache", { timeout: 30_000 }, async () => {
            const response = await api.chatCompletionNonStream({
                model: MODEL,
                messages: [
                    { role: "system", content: SYSTEM_MESSAGE },
                    ...
                    {
                        role: "user",
                        content: "That's a big file. Can you summarize the main methods?",
                    },
                ],
                tools: TOOLS,
                max_tokens: 256,
        ...
        test("Turn 2: follow-up on large file hits cache", { timeout: 30_000 }, async () => {
            const response = await api.chatCompletionNonStream({
                model: MODEL,
                messages: [
                    { role: "system", content: SYSTEM_MESSAGE },
                    ...
                    {
                        role: "assistant",
                        content: "The file contains a ProjectService class with 10 methods covering standard CRUD operations.",
                    },
                    {
                        role: "user",
                        content: "Can you refactor method5 to use a transaction instead of a raw query?",
                    },
                ],
                tools: TOOLS,
                max_tokens: 256,
    ...
    test("Scenario 5: Modified system message invalidates cache", { timeout: 30_000 }, async () => {
        const api = makeApi();
    ...
    test("Scenario 6: Identical request replay gets full cache hit", { timeout: 30_000 }, async () => {
        const api = makeApi();
        const request = {
            model: MODEL,
            messages: [
                ...
                {
                    role: "user",
                    content: "List all the database migration files and explain what each one does.",
                },
            ],
            tools: TOOLS,
            max_tokens: 256,
    ...
    test("Scenario 7: Multi-step agentic workflow with chained tool calls", { timeout: 60_000 }, async () => {
        const api = makeApi();
        ...
        const messages1 = [
            ...
            {
                role: "user",
                content: "There's a bug in the team service - when you delete a team, it doesn't clean up the team members. Can you investigate and fix it?",
            },
            {
                role: "assistant",
                content: null,
            ...
            {
                role: "user",
                content: "I see the file. Can you also check the test file to see if there's a test for this?",
            },
        ];
        const resp1 = await api.chatCompletionNonStream({ model: MODEL, messages: messages1, tools: TOOLS, max_tokens: 256 }, signal);
        const usage1 = resp1.usage;
        const details1 = usage1.prompt_tokens_details;
        ...
        const messages2 = [
            ...
            {
                role: "user",
                content: "Alright, I see the problem. Please write the fix and update the test.",
            },
        ];
        const resp2 = await api.chatCompletionNonStream({ model: MODEL, messages: messages2, tools: TOOLS, max_tokens: 256 }, signal);
        const usage2 = resp2.usage;
        const rate2 = cacheHitRate(usage2);
        // Step 2 should have a cache hit from step 1's prefix
        expect(usage2.prompt_tokens_details?.cache_read_tokens).toBeGreaterThan(0);
        ...
        const messages3 = [
            ...
            {
                role: "user",
                content: "Good fix. Now run the tests to make sure everything passes.",
            },
        ];
        const resp3 = await api.chatCompletionNonStream({ model: MODEL, messages: messages3, tools: TOOLS, max_tokens: 256 }, signal);
        const usage3 = resp3.usage;
        const rate3 = cacheHitRate(usage3);
        expect(usage3.prompt_tokens_details?.cache_read_tokens).toBeGreaterThan(0);
...
//# sourceMappingURL=anthropic-caching-scenarios.live.test.js.map