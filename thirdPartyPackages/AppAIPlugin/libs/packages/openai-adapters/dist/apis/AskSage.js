...
export class AskSageApi {
    ...
    async getSessionToken() {
        if (!this.apiKey) {
            throw new Error("AskSage adapter: missing apiKey. Provide it in your configuration.");
        }
    ...
    buildRequestBody(body) {
        ...
        const message = messageArray.length === 1 && !systemPrompt
    ...
    async *chatCompletionStream(body, signal) {
        ...
        try {
            ...
            yield chatChunk({
                content: null,
                model: body.model,
                finish_reason: validToolCalls && validToolCalls.length > 0 ? "tool_calls" : "stop",
            });
        }
        catch (error) {
            if (error instanceof Error) {
    ...
    completionNonStream(_body, _signal) {
        throw new Error("AskSage does not support legacy completions API");
    }
    completionStream(_body, _signal) {
        throw new Error("AskSage does not support legacy completions API");
    }
    fimStream(_body, _signal) {
