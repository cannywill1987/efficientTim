...
import { openedFilesLruCache, prevFilepaths, } from "./autocomplete/util/openedFilesLruCache";
...
import { isProcessBackgrounded, killTerminalProcess, markProcessAsBackgrounded, } from "./util/processTerminalStates";
import { getSymbolsForManyFiles } from "./util/treeSitter";
import { TTS } from "./util/tts";
import { getDiffFn, GitDiffCache } from "./autocomplete/snippets/gitDiffCache";
import { stringifyMcpPrompt } from "./commands/slash/mcpSlashCommand";
import { createNewAssistantFile } from "./config/createNewAssistantFile";
import { isColocatedRulesFile, isContinueAgentConfigFile, isContinueConfigRelatedUri, } from "./config/loadLocalAssistants";
import { CodebaseRulesCache } from "./config/markdown/loadCodebaseRules";
import { setupLocalConfig, setupProviderConfig, setupQuickstartConfig, } from "./config/onboarding";
import { createNewGlobalRuleFile, createNewWorkspaceBlockFile, } from "./config/workspace/workspaceBlocks";
...
export class Core {
    ...
    messageAbortControllers = new Map();
    ...
    constructor(messenger, ide) {
        ...
        try {
            ...
            this.configHandler = new ConfigHandler(this.ide, this.llmLogger, initialSessionInfoPromise);
            this.docsService = DocsService.createSingleton(this.configHandler, this.ide, this.messenger);
            ...
            this.codeBaseIndexer = new CodebaseIndexer(this.configHandler, this.ide, this.messenger, this.globalContext.get("indexingPaused"));
            ...
            this.completionProvider = new CompletionProvider(this.configHandler, ide, getLlm, (e) => { }, (..._) => Promise.resolve([]));
            const codebaseRulesCache = CodebaseRulesCache.getInstance();
            void codebaseRulesCache
                .refresh(ide)
                .catch((e) => Logger.error("Failed to initialize colocated rules cache"))
                .then(() => {
                void this.configHandler.reloadConfig("Initial codebase rules post-walkdir/load reload");
            });
            this.nextEditProvider = NextEditProvider.initialize(this.configHandler, ide, getLlm, (e) => { }, (..._) => Promise.resolve([]), "fineTuned");
    ...
    registerMessageHandlers(ideSettingsPromise) {
        ...
        on("config/addModel", async (msg) => {
            ...
            const existing = allModels.find((m) => m.providerName === model.provider && m.model === model.model);
            if (existing) {
                void this.ide.showToast("warning", "Model already exists in config. Update the API key in the config file.");
        ...
        on("config/newAssistantFile", async (msg) => {
            ...
            await this.configHandler.refreshAll("Assistant file created (config/newAssistantFile message)");
        });
        on("config/addLocalWorkspaceBlock", async (msg) => {
            await createNewWorkspaceBlockFile(this.ide, msg.data.blockType, msg.data.baseFilename);
        ...
        on("config/deleteRule", async (msg) => {
            try {
                ...
                if (!isColocatedRulesFile(filepath) &&
                ...
                if (fileExists) {
                    ...
                    walkDirCache.invalidate();
                    await this.configHandler.reloadConfig("Rule file deleted (config/deleteRule message)");
                }
            }
            catch (error) {
        ...
        on("mcp/getPrompt", async (msg) => {
            ...
            const prompt = await MCPManagerSingleton.getInstance().getPrompt(serverName, promptName, args);
        ...
        on("mcp/startAuthentication", async (msg) => {
            ...
            MCPManagerSingleton.getInstance().setStatus(msg.data.serverId, "authenticating");
            const status = await performAuth(msg.data.serverId, msg.data.serverUrl, this.ide);
            if (status === "AUTHORIZED") {
                await MCPManagerSingleton.getInstance().refreshConnection(msg.data.serverId);
            }
        });
        ...
        on("llm/streamChat", (msg) => {
            ...
            return llmStreamChat(this.configHandler, abortController, msg, this.ide, this.messenger);
        });
        ...
        on("streamDiffLines", async (msg) => {
            ...
            return streamDiffLines(data, llm, abortController, undefined, data.includeRulesInSystemMessage ? config.rules : undefined);
        });
        on("getDiffLines", (msg) => {
        ...
        on("files/created", async ({ data }) => {
            ...
            const nonColocatedRuleUris = data.uris.filter((uri) => !isColocatedRulesFile(uri));
        ...
        on("files/closed", async ({ data }) => {
            ...
            try {
                ...
                Logger.error(`didChangeVisibleTextEditors: failed to update openedFilesLruCache`);
            }
            if (data.uris) {
            ...
            if (uris) {
                for (const filepath of uris) {
                    try {
                        ...
                        Logger.error(`files/opened: failed to update openedFiles cache for ${filepath}`);
                    }
                }
            }
        });
        on("files/smallEdit", async ({ data }) => {
            ...
            EditAggregator.getInstance(EDIT_AGGREGATION_OPTIONS, (beforeAfterdiff, cursorPosBeforeEdit, cursorPosAfterPrevEdit) => {
                void processSmallEdit(beforeAfterdiff, cursorPosBeforeEdit, cursorPosAfterPrevEdit, data.configHandler, data.getDefsFromLspFunction, this.ide);
            });
        ...
        on("auth/getAuthUrl", async (msg) => {
            const url = await getAuthUrlForTokenPage(ideSettingsPromise, msg.data.useOnboarding);
            return { url };
        });
        ...
        on("tools/preprocessArgs", async ({ data: { toolName, args } }) => {
            ...
            try {
                ...
                let errorReason = e instanceof ContinueError ? e.reason : ContinueErrorReason.Unknown;
    ...
    async handleCompleteOnboarding(msg) {
        ...
        switch (mode) {
            ...
            case OnboardingModes.API_KEY:
                if (provider && apiKey) {
                    editConfigYamlCallback = (config) => setupProviderConfig(config, provider, apiKey);
                }
                else {
    ...
    getContextItems = async (msg) => {
        ...
        try {
            ...
            void Telemetry.capture("useContextProvider", {
                name: provider.description.title,
            }, true);
