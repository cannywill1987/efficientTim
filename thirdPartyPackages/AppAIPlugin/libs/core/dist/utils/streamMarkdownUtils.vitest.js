...
import { processBlockNesting, stopAtLinesWithMarkdownSupport, } from "./streamMarkdownUtils";
test("processBlockNesting correctly handles starting blocks", () => {
    // Mock function to determine which lines should be removed
    const shouldRemoveLineBeforeStart = (line) => line === "[CODE]" || line === "```";
    // Test not seeing first fence yet
    const result1 = processBlockNesting("[CODE]", false, shouldRemoveLineBeforeStart);
    ...
    const result2 = processBlockNesting("normal code", false, shouldRemoveLineBeforeStart);
    expect(result2.newSeenFirstFence).toBe(true);
    expect(result2.shouldSkip).toBe(false);
    // Test already seen first fence
    const result3 = processBlockNesting("more code", true, shouldRemoveLineBeforeStart);
