    ...
    async process(ctx) {
        while (this.unprocessedQueue.length > 0 &&
            ...
            if (!location)
                break;
            try {
                const outcome = await NextEditProvider.getInstance().provideInlineCompletionItemsWithChain(ctx, location, this.abortController.signal, this.usingFullFileDiff);
                if (!outcome) {
