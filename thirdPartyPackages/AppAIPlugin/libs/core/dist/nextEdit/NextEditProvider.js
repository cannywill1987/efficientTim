/**
 * 文件类型：服务
 * 文件作用：提供 Next Edit 预测与补全能力。
 * 主要职责：在 Flutter 环境下组织补全链路与展示状态。
 */
...
export class NextEditProvider {
    ...
    autocompleteCache = AutocompleteLruCache.get();
    errorsShown = new Set();
    ...
    async deleteChain() {
        ...
        if (this.previousRequest) {
            ...
            if (ast) {
                DocumentHistoryTracker.getInstance().push(localPathOrUriToPath(this.previousRequest.filepath), fileContent, ast);
            }
        }
    }
    startChain(id) {
    ...
    async _generatePrompts(helper, opts) {
        ...
        const context = {
            ...
            historyDiff: createDiff({
                beforeContent: DocumentHistoryTracker.getInstance().getMostRecentDocumentHistory(localPathOrUriToPath(helper.filepath)) ?? "",
    ...
    async _handleCompletion(helper, prompts, token, startTime, editableRegionStartLine, editableRegionEndLine, opts) {
        ...
        if (outcome) {
            ...
            // Flutter 环境下直接记录展示状态
            this.markDisplayed(helper.input.completionId, outcome);
        }
        return outcome;
    }
    /**
