...
describe("DocumentHistoryTracker", () => {
    ...
    beforeEach(() => {
        ...
        // Create mock ASTs
        //@ts-ignore
        mockAst1 = new Parser.Tree();
        //@ts-ignore
        mockAst2 = new Parser.Tree();
    });
    afterEach(() => {
    ...
    describe("getMostRecentAst", () => {
        ...
        it("should return null if the document doesn't exist", () => {
            ...
            expect(ast).toBeNull();
            expect(consoleErrorSpy).toHaveBeenCalledWith("Document nonexistent-path not found in AST tracker");
        ...
        it("should return null if the document has no ASTs", () => {
            ...
            const ast = tracker.getMostRecentAst(testDocPath);
            expect(ast).toBeNull();
            expect(consoleErrorSpy).toHaveBeenCalledWith(`Document ${testDocPath} has no ASTs`);
            consoleErrorSpy.mockRestore();
        });
    });
    describe("getMostRecentDocumentHistory", () => {
        ...
        it("should return null if the document has no history", () => {
            ...
            const content = tracker.getMostRecentDocumentHistory(testDocPath);
            expect(content).toBeNull();
            expect(consoleErrorSpy).toHaveBeenCalledWith(`Document ${testDocPath} has no history`);
            consoleErrorSpy.mockRestore();
        });
    });
    describe("deleteDocument", () => {
        it("should delete a document from the tracker", () => {
            ...
            expect(ast).toBeNull();
            expect(content).toBeNull();
            expect(consoleErrorSpy).toHaveBeenCalledWith(`Document ${testDocPath} not found in AST tracker`);
            consoleErrorSpy.mockRestore();
        });
    });
    describe("clearMap", () => {
