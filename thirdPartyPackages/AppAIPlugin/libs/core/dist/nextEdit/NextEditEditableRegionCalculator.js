...
export var EditableRegionStrategy;
 ...
 */
export async function getNextEditableRegion(strategy, ctx) {
    switch (strategy) {
...
function slidingJump(ctx) {
    ...
    const topMargin = MODEL_WINDOW_SIZES[modelName].topMargin;
    const bottomMargin = MODEL_WINDOW_SIZES[modelName].bottomMargin;
...
async function staticRerankJump(ctx) {
    try {
        ...
        console.log("nodeQueue:", nodeQueue.map((node) => ({
        ...
        try {
            ...
            if (nodePosition) {
                ...
                const symbolChunks = filteredSymbols.map((symbol) => ({
                    content: symbol.name,
                ...
                const originalSymbol = filteredSymbols.find((symbol) => symbol.range.start.line === mostRelevantSymbol.startLine &&
...
function compareAsts(oldAst, newAst) {
    ...
    function traverse(oldNode, newNode, depth = 0) {
        if (!oldNode && !newNode)
            return;
...
function isRangeWithin(innerRange, outerRange) {
    ...
    const startWithin = innerRange.start.line > outerRange.start.line ||
    ...
    const endWithin = innerRange.end.line < outerRange.end.line ||
...
function doRangesOverlap(range1, range2) {
    ...
    const range1StartsAfterRange2Ends = range1.start.line > range2.end.line ||
        (range1.start.line === range2.end.line &&
            range1.start.character > range2.end.character);
    const range2StartsAfterRange1Ends = range2.start.line > range1.end.line ||
...
function doesUpperPartOverlap(range1, range2) {
    ...
    const range1StartsBeforeRange2Ends = range1.start.line < range2.end.line ||
        (range1.start.line === range2.end.line &&
            range1.start.character <= range2.end.character);
    // Check if range1 starts before range2 starts (meaning it's "upper" than range2)
    const range1StartsBeforeRange2Starts = range1.start.line < range2.start.line ||
...
function doesLowerPartOverlap(range1, range2) {
    ...
    const range1StartsInsideRange2 = (range1.start.line > range2.start.line ||
        (range1.start.line === range2.start.line &&
            range1.start.character >= range2.start.character)) &&
        (range1.start.line < range2.end.line ||
            (range1.start.line === range2.end.line &&
    ...
    const range1EndsAfterRange2 = range1.end.line > range2.end.line ||
...
function doesRangePartiallyOverlap(range1, range2) {
    ...
    const upperPartOverlap = (range1.start.line < range2.start.line ||
        (range1.start.line === range2.start.line &&
            range1.start.character < range2.start.character)) &&
    ...
    const lowerPartOverlap = (range1.start.line > range2.start.line ||
        (range1.start.line === range2.start.line &&
            range1.start.character >= range2.start.character)) &&
        (range1.start.line < range2.end.line ||
            (range1.start.line === range2.end.line &&
...
function printChunks(chunks) {
    console.log("chunks:", JSON.stringify(chunks.map((chunk) => ({
