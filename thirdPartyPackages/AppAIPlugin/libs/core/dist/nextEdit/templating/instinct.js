import { INSTINCT_CONTEXT_FILE_TOKEN, INSTINCT_EDITABLE_REGION_END_TOKEN, INSTINCT_EDITABLE_REGION_START_TOKEN, INSTINCT_SNIPPET_TOKEN, INSTINCT_USER_CURSOR_IS_HERE_TOKEN, } from "../constants";
import { insertCursorToken } from "./utils";
/**
 * @param contextSnippets Codestral style snippet with +++++ filename\ncontent or an empty string.
 */
export function contextSnippetsBlock(contextSnippets) {
    const headerRegex = /^(\+\+\+\+\+ )(.*)/;
...
export function currentFileContentBlock(currentFileContent, windowStart, windowEnd, editableRegionStartLine, editableRegionEndLine, cursorPosition) {
    const currentFileContentLines = currentFileContent.split("\n");
    const insertedCursorLines = insertCursorToken(currentFileContentLines, cursorPosition, INSTINCT_USER_CURSOR_IS_HERE_TOKEN);
...
export function editHistoryBlock(editDiffHistories) {
