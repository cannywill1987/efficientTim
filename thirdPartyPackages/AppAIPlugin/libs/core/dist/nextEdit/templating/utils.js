...
export function insertCursorToken(lines, cursorPos, cursorToken) {
...
export function insertEditableRegionTokensWithStaticRange(lines, cursorPos, editableRegionStart, editableRegionEnd) {
    if (cursorPos.line < 0 || cursorPos.line >= lines.length) {
        return lines;
    }
    // Ensure editable regions are within bounds.
    if (editableRegionStart === undefined) {
        editableRegionStart = Math.max(cursorPos.line - NEXT_EDIT_EDITABLE_REGION_TOP_MARGIN, 0);
    }
    if (editableRegionEnd === undefined) {
        editableRegionEnd = Math.min(cursorPos.line + NEXT_EDIT_EDITABLE_REGION_BOTTOM_MARGIN, lines.length - 1);
    }
