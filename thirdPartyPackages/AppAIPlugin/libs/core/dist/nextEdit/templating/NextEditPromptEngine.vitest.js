...
import { INSTINCT_USER_PROMPT_PREFIX, MERCURY_CURRENT_FILE_CONTENT_CLOSE, MERCURY_CURRENT_FILE_CONTENT_OPEN, MERCURY_EDIT_DIFF_HISTORY_CLOSE, MERCURY_EDIT_DIFF_HISTORY_OPEN, MERCURY_RECENTLY_VIEWED_CODE_SNIPPETS_CLOSE, MERCURY_RECENTLY_VIEWED_CODE_SNIPPETS_OPEN, } from "../constants";
import { NEXT_EDIT_MODEL_TEMPLATES, PromptTemplateRenderer, getTemplateForModel, } from "./NextEditPromptEngine";
describe("NextEditPromptEngine", () => {
    describe("NEXT_EDIT_MODEL_TEMPLATES", () => {
        ...
        it("mercury-coder template should contain expected tokens", () => {
            const template = NEXT_EDIT_MODEL_TEMPLATES[NEXT_EDIT_MODELS.MERCURY_CODER].template;
        ...
        it("instinct template should contain expected tokens", () => {
            const template = NEXT_EDIT_MODEL_TEMPLATES[NEXT_EDIT_MODELS.INSTINCT].template;
    ...
    describe("getTemplateForModel", () => {
        it("should return the correct template for a supported model", () => {
            ...
            expect(template).toBe(NEXT_EDIT_MODEL_TEMPLATES[NEXT_EDIT_MODELS.MERCURY_CODER].template);
        });
        it("should throw an error for an unsupported model", () => {
            expect(() => getTemplateForModel("unsupported")).toThrow("Model unsupported is not supported for next edit.");
        });
    });
});
