...
import { convertNextEditModelNameToEnum, isNextEditTest, isWhitespaceOnlyDeletion, } from "./utils";
...
describe("convertNextEditModelNameToEnum", () => {
    ...
    test("returns correct enum when model name is contained in a longer string", () => {
        expect(convertNextEditModelNameToEnum("inception/mercury-coder")).toBe(NEXT_EDIT_MODELS.MERCURY_CODER);
        expect(convertNextEditModelNameToEnum("blah/instinct")).toBe(NEXT_EDIT_MODELS.INSTINCT);
    });
