...
import { createBeforeAfterDiff, createDiff, DiffFormatType, extractMetadataFromUnifiedDiff, } from "./diffFormatting";
describe("diffFormatting", () => {
    ...
    describe("createBeforeAfterDiff", () => {
        it("should return BeforeAfterDiff with normalized content", () => {
            const result = createBeforeAfterDiff(beforeContent, afterContent, sampleFilePath);
        ...
        it("should not add extra newline if content already ends with one", () => {
            ...
            const result = createBeforeAfterDiff(contentWithNewline, contentWithNewline, sampleFilePath);
            expect(result.beforeContent).toBe(contentWithNewline);
            expect(result.afterContent).toBe(contentWithNewline);
        });
        it("should handle empty content", () => {
            const result = createBeforeAfterDiff("", "", sampleFilePath);
            expect(result).toEqual({
                filePath: sampleFilePath,
                beforeContent: "\n",
                afterContent: "\n",
        ...
        it("should handle mixed newline scenarios", () => {
            ...
            const result = createBeforeAfterDiff(beforeWithNewline, afterWithoutNewline, sampleFilePath);
...
+modified2
 line3`;
            const metadata = extractMetadataFromUnifiedDiff(unifiedDiff);
...
+modified1
 line2`;
        ...
        it("should handle empty diff", () => {
            const metadata = extractMetadataFromUnifiedDiff("");
...
random text`;
            const metadata = extractMetadataFromUnifiedDiff(malformedDiff);
            expect(metadata.oldFilename).toBeUndefined();
            expect(metadata.newFilename).toBeUndefined();
            expect(metadata.hunks).toEqual([]);
        });
