 ...
 */
export function getOffsetPositionAtLastNewLine(diffLines, lineContentAtCursorPos, lineOffsetAtCursorPos) {
...
export function getRenderableDiffWithGutterAnnotations(diffLines, lineContentAtCursorPos, lineOffsetAtCursorPos) {
    let lastNewLineContent = "";
    let lineOffset = -1;
    let currentResultLine = 0;
    let hasChanges = false;
 ...
 */
export function checkFim(oldEditRange, newEditRange, cursorPosition) {
...
export function calculateFinalCursorPosition(currCursorPos, editableRegionStartLine, oldEditRangeSlice, newEditRangeSlice) {
 ...
 */
export function applyCompletionToFile(fileContent, completion, startLineNumber, linesToReplace) {
 ...
 */
export function groupDiffLines(diffLines, offset = 0, maxGroupSize) {
    const groups = [];
