...
import { allDevEventNames, devDataVersionedSchemas, } from "@continuedev/config-yaml";
...
export class DataLogger {
    ...
    async logLocalData(event) {
        try {
            const filepath = getDevDataFilePath(event.name, LOCAL_DEV_DATA_VERSION);
            const localSchema = devDataVersionedSchemas[LOCAL_DEV_DATA_VERSION]["all"][event.name];
            if (!localSchema) {
                throw new Error(`Schema ${LOCAL_DEV_DATA_VERSION} doesn't exist at level "all"`);
            }
            const eventDataWithBaseValues = await this.addBaseValues(event.data, event.name, LOCAL_DEV_DATA_VERSION, localSchema);
    ...
    async logToOneDestination(dataConfig, event) {
        try {
            ...
            if (uriComponents.scheme === "https" || uriComponents.scheme === "http") {
                ...
                const response = await fetchwithRequestOptions(dataConfig.destination, {
                    method: "POST",
                    headers,
                    body: JSON.stringify({
                        name: event.name,
