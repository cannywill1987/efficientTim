...
import { validateLicenseKey } from "./mdm";
...
function createTestLicense(licenseData, apiUrl = "https://api.continue.dev", useValidSignature = true) {
...
test("validateLicenseKey returns false for invalid JSON in license key", () => {
    ...
    const malformedLicenseKey = Buffer.from("this is not valid JSON").toString("base64");
    // Test - no mocks, real validation
...
test("validateLicenseKey returns false for expired license", () => {
    ...
    const expiredLicenseKey = createTestLicense(licenseData, "https://api.continue.dev");
...
test("validateLicenseKey returns false for invalid signature", () => {
    ...
    const invalidSignatureKey = createTestLicense(licenseData, "https://api.continue.dev", false);
    // Test - no mocks, real validation
...
test("validateLicenseKey handles JSON parsing errors gracefully", () => {
    ...
    const invalidStructureKey = Buffer.from(JSON.stringify({
...
test("validateLicenseKey handles missing unsignedData gracefully", () => {
    ...
    const licenseKey = Buffer.from(JSON.stringify(licenseKeyObj)).toString("base64");
    // Test - no mocks, real validation
