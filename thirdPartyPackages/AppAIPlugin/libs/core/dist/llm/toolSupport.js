...
export const PROVIDER_TOOL_SUPPORT = {
    ...
    lmstudio: (model) => {
        ...
        const isExcluded = (name) => exclusions.some((part) => name.includes(part) || name.includes(part.replace(/-/g, "")));
    ...
    openrouter: (model) => {
        ...
        if (["vision", "math", "guard", "mistrallite", "mistral-openorca"].some((part) => baseModel.includes(part))) {
            return false;
        }
...
export function isRecommendedAgentModel(modelName) {
    // AND behavior
    const recs = [
        [/o[134]/],
...
export function modelSupportsNativeTools(modelDescription) {
