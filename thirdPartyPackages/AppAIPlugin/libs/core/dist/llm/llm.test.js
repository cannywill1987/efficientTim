...
function testLLM(llm, { skip, testFim, testToolCall, timeout, }) {
    if (skip) {
        return;
    }
    beforeAll(() => {
    ...
    describe(llm.providerName + "/" + llm.model, () => {
        test("Stream Chat works", retryOnce(async () => {
            let total = "";
            for await (const chunk of llm.streamChat([{ role: "user", content: "Hi" }], new AbortController().signal)) {
                total += chunk.content;
            }
            expect(total.length).toBeGreaterThan(0);
            return;
        }), timeout);
        test("Stream Complete works", retryOnce(async () => {
            let total = "";
            for await (const chunk of llm.streamComplete("Hi", new AbortController().signal)) {
                total += chunk;
            }
            expect(total.length).toBeGreaterThan(0);
            return;
        }), timeout);
        test("Complete works", retryOnce(async () => {
            const completion = await llm.complete("Hi", new AbortController().signal);
            expect(completion.length).toBeGreaterThan(0);
            return;
        }), timeout);
        if (testFim) {
            test("FIM works", retryOnce(async () => {
                let total = "";
                for await (const chunk of llm.streamFim("Hi", "name is ChatGPT.", new AbortController().signal)) {
                    total += chunk;
                }
                expect(total.length).toBeGreaterThan(0);
                return;
        ...
        if (testToolCall) {
            test("Tool Call works", retryOnce(async () => {
                let args = "";
                let isFirstChunk = true;
                for await (const chunk of llm.streamChat([{ role: "user", content: "Hi, my name is Nate." }], new AbortController().signal, {
                    tools: [
                        {
...
describe("LLM", () => {
    ...
    testLLM(new OpenAI({ apiKey: process.env.OPENAI_API_KEY, model: "o3-mini" }), { skip: false, timeout: 60000 });
    ...
    }), { testFim: false, skip: true, timeout: 20000 });
    testLLM(new Azure({
