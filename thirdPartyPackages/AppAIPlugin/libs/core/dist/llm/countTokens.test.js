import { cleanupAsyncEncoders, compileChatMessages, countTokens, countTokensAsync, extractToolSequence, pruneLinesFromBottom, pruneLinesFromTop, pruneRawPromptFromTop, pruneStringFromBottom, pruneStringFromTop, } from "./countTokens.js";
...
describe.skip("pruneLinesFromTop", () => {
    ...
    it("should handle a prompt with a single line that exceeds maxTokens", () => {
        const prompt = "This is a single long line that will exceed the token limit";
        const pruned = pruneLinesFromTop(prompt, 5, "gpt-4");
        expect(pruned).toEqual("");
    });
...
describe("extractToolSequence", () => {
    ...
    const createAssistantMessage = (content, toolCalls) => ({
        role: "assistant",
        content,
        toolCalls,
    });
    const createToolMessage = (content, toolCallId) => ({
        role: "tool",
        content,
        toolCallId,
    });
    ...
    test("extractToolSequence should throw error when tool message has no matching tool call", () => {
        ...
        expect(() => extractToolSequence(messages)).toThrow('Error parsing chat history: no tool call found to match tool output for id "tc_2"');
    });
    ...
    test("extractToolSequence should throw error when messages array is empty", () => {
        const messages = [];
        expect(() => extractToolSequence(messages)).toThrow("Error parsing chat history: no user/tool message found");
    });
