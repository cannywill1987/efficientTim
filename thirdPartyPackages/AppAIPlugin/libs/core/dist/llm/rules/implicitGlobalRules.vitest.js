...
describe("Implicit global rules application", () => {
    ...
    it("should apply rules with no alwaysApply and no globs to all messages, even with no file references", () => {
        ...
        const applicableRules = getApplicableRules(simpleMessage, [implicitGlobalRule, explicitGlobalRule, nestedDirRule], []);
        // Both global rules should be included regardless of context
        expect(applicableRules).toHaveLength(2);
        expect(applicableRules.map((r) => r.name)).toContain("Implicit Global Rule");
        expect(applicableRules.map((r) => r.name)).toContain("Explicit Global Rule");
    ...
    it("should treat root-level rules with no globs as global rules", () => {
        ...
        const applicableRules = getApplicableRules(undefined, [rootNoGlobsRule], []);
    ...
    it("should apply implicit global rules alongside file-specific rules", () => {
        ...
        const applicableRules = getApplicableRules(messageWithFile, [implicitGlobalRule, explicitGlobalRule, nestedDirRule], []);
    ...
    it("should apply implicit global rules for first message without code", () => {
        ...
        const applicableRules = getApplicableRules(firstMessage, [assistantRule], []);
