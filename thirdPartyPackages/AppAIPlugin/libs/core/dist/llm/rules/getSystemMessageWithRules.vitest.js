...
import { getApplicableRules, shouldApplyRule, } from "./getSystemMessageWithRules";
...
describe("Rule policies", () => {
    ...
    it("should never apply rules with 'off' policy regardless of file paths", () => {
        ...
        expect(shouldApplyRule(alwaysApplyRule, testFiles, offPolicies)).toBe(false);
    });
    it("should apply 'on' policy rules based on normal matching logic", () => {
        ...
        expect(shouldApplyRule(componentRule, nonMatchingFiles, rulePolicies)).toBe(false);
    });
    ...
    it("should prioritize 'off' policy over alwaysApply and directory restrictions", () => {
        ...
        expect(shouldApplyRule(complexRule, ["src/utils/test.ts"], offPolicies)).toBe(false);
    });
});
describe("Content pattern matching", () => {
    it("should apply rules when file content matches pattern", () => {
        ...
        const fileContents = {
            [filePath]: "export function Button() { return <button>Click me</button> }",
        };
        expect(shouldApplyRule(reactComponentRule, [filePath], {}, fileContents)).toBe(true);
        // File that doesn't match the pattern
        const nonMatchingContents = {
            [filePath]: "import React from 'react'; // Just an import, not a component",
        };
        expect(shouldApplyRule(reactComponentRule, [filePath], {}, nonMatchingContents)).toBe(false);
    });
    it("should support multiple regex regex as an array", () => {
        // Rule with multiple regex
        const multiPatternRule = {
        ...
        expect(shouldApplyRule(multiPatternRule, [filePath], {}, functionContent)).toBe(true);
        // Content with class - should match
        const classContent = {
            [filePath]: "class DateFormatter { format(date) { return date.toISOString(); } }",
        };
        expect(shouldApplyRule(multiPatternRule, [filePath], {}, classContent)).toBe(true);
        // Content with neither - should not match
        const nonMatchingContent = {
            [filePath]: "const PI = 3.14159;",
        };
        expect(shouldApplyRule(multiPatternRule, [filePath], {}, nonMatchingContent)).toBe(false);
    });
    it("should apply rules based on both globs and regex when both are specified", () => {
        // Rule with both glob and pattern restrictions
        const typescriptClassRule = {
        ...
        expect(shouldApplyRule(typescriptClassRule, [tsFilePath], {}, tsClassContent)).toBe(true);
        // TypeScript file without class - should not match
        const tsNonClassContent = {
            [tsFilePath]: "export const createUser = (name: string) => ({ name });",
        };
        expect(shouldApplyRule(typescriptClassRule, [tsFilePath], {}, tsNonClassContent)).toBe(false);
        // JavaScript file with class - should not match due to glob
        const jsClassContent = {
            [jsFilePath]: "export class Product { constructor(name) { this.name = name; } }",
        };
        expect(shouldApplyRule(typescriptClassRule, [jsFilePath], {}, jsClassContent)).toBe(false);
    });
    it("should extract file content from messages for pattern matching", () => {
        // Rule with pattern
        const arrowFunctionRule = {
        ...
        const applicableRules = getApplicableRules(messageWithArrowFn, [arrowFunctionRule], [contextItem]);
        ...
        const messageWithoutArrowFn = {
            role: "user",
            content: "Can you check this code?\n```typescript src/utils/formatter.ts\nfunction format(date) {\n  return date.toISOString();\n}\n```",
        };
        // Context item with non-matching content
        const nonMatchingContextItem = {
            id: { providerTitle: "file", itemId: "formatter2" },
            uri: { type: "file", value: "src/utils/formatter.ts" },
        ...
        const nonApplicableRules = getApplicableRules(messageWithoutArrowFn, [arrowFunctionRule], [nonMatchingContextItem]);
    ...
    it("should respect directory restrictions for pattern rules", () => {
        ...
        expect(shouldApplyRule(nestedPatternRule, [modelFilePath], {}, modelContents)).toBe(true);
        // File outside the directory with matching content - should not match
        const utilFilePath = "src/utils/types.ts";
        const utilContents = {
            [utilFilePath]: "export interface Config { apiUrl: string; timeout: number; }",
        };
        expect(shouldApplyRule(nestedPatternRule, [utilFilePath], {}, utilContents)).toBe(false);
    });
});
