...
const matchesGlobs = (filePath, globs) => {
    ...
    if (Array.isArray(globs)) {
        ...
        return (positivePatterns.some((pattern) => minimatch(filePath, pattern)) &&
 ...
 */
export const shouldApplyRule = (rule, filePaths, rulePolicies = {}, fileContents = {}) => {
 ...
 */
export const getApplicableRules = (userMessage, rules, contextItems, rulePolicies = {}) => {
    ...
    const applicableRules = rules.filter((rule) => shouldApplyRule(rule, allFilePaths, rulePolicies, fileContents));
    return applicableRules;
};
export function getRuleId(rule) {
    return rule.slug ?? rule.sourceFile ?? rule.name ?? rule.source;
}
export const getSystemMessageWithRules = ({ baseSystemMessage, userMessage, availableRules, contextItems, rulePolicies = {}, }) => {
    const appliedRules = getApplicableRules(userMessage, availableRules, contextItems, rulePolicies);
