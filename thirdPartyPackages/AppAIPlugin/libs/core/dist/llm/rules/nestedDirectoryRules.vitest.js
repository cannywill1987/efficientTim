...
describe("Nested directory rules application", () => {
    ...
    it("should apply nested directory rules to files in that directory", () => {
        ...
        const nestedFileContext = {
            id: { providerTitle: "file", itemId: "nested1" },
            uri: {
                type: "file",
        ...
        const applicableRules = getApplicableRules(undefined, // No message needed
        [nestedFolderRule, globalRule], [nestedFileContext]);
    ...
    it("should also work with file references in messages", () => {
        ...
        const messageWithNestedFile = {
            role: "user",
            content: 'Can you explain this file?\n```python manual-testing-sandbox/nested-folder/hellonested.py\nprint("Hello nested")\n```',
        };
        // Apply rules with the message containing a nested file reference
        const applicableRules = getApplicableRules(messageWithNestedFile, [nestedFolderRule, globalRule], []);
        // Both rules should apply
        expect(applicableRules).toHaveLength(2);
        expect(applicableRules.map((r) => r.name)).toContain("Global Rule");
        expect(applicableRules.map((r) => r.name)).toContain("Nested Folder Rule");
    ...
    it("should NOT apply nested directory rules to files outside that directory", () => {
        ...
        const outsideFileContext = {
            id: { providerTitle: "file", itemId: "outside1" },
        ...
        const applicableRules = getApplicableRules(undefined, [nestedFolderRule, globalRule], [outsideFileContext]);
        // Only the global rule should be included
        expect(applicableRules.map((r) => r.name)).toContain("Global Rule");
        expect(applicableRules.map((r) => r.name)).not.toContain("Nested Folder Rule");
    });
    it("should apply glob patterns relative to rules.md location", () => {
        ...
        // A file in src/
        const srcPythonFileContext = {
            id: { providerTitle: "file", itemId: "python1" },
        ...
        // A file in utils/
        const utilsPythonFileContext = {
            id: { providerTitle: "file", itemId: "python2" },
        ...
        const manualTestingPythonFileContext = {
            id: { providerTitle: "file", itemId: "python3" },
        ...
        let applicableRules = getApplicableRules(undefined, [srcPythonRule, utilsPythonRule], [srcPythonFileContext]);
        expect(applicableRules.map((r) => r.name)).toContain("Src Python Rule");
        expect(applicableRules.map((r) => r.name)).not.toContain("Utils Python Rule");
        // Apply utils rule to utils file - should match
        applicableRules = getApplicableRules(undefined, [srcPythonRule, utilsPythonRule], [utilsPythonFileContext]);
        ...
        applicableRules = getApplicableRules(undefined, [srcPythonRule, utilsPythonRule], [manualTestingPythonFileContext]);
        expect(applicableRules.map((r) => r.name)).not.toContain("Src Python Rule");
        expect(applicableRules.map((r) => r.name)).not.toContain("Utils Python Rule");
    });
});
