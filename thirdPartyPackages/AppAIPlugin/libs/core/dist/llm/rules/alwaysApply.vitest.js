...
describe("alwaysApply Behavior", () => {
    ...
    const matchingFileContext = {
        id: { providerTitle: "file", itemId: "match1" },
        uri: { type: "file", value: matchingFile },
        content: "export const Button = () => {...}",
        name: "Button.tsx",
        description: "Component file",
    };
    const nonMatchingFileContext = {
        id: { providerTitle: "file", itemId: "nonmatch1" },
    ...
    it("alwaysApply: true - Always include the rule, regardless of file context", () => {
        ...
        let applicableRules = getApplicableRules(messageWithoutFile, [alwaysApplyRule], []);
        expect(applicableRules).toHaveLength(1);
        expect(applicableRules[0].name).toBe("Always Apply Rule");
        // Test with matching file
        applicableRules = getApplicableRules(undefined, [alwaysApplyRule], [matchingFileContext]);
        expect(applicableRules).toHaveLength(1);
        expect(applicableRules[0].name).toBe("Always Apply Rule");
        // Test with non-matching file
        applicableRules = getApplicableRules(undefined, [alwaysApplyRule], [nonMatchingFileContext]);
        expect(applicableRules).toHaveLength(1);
        expect(applicableRules[0].name).toBe("Always Apply Rule");
    });
    it("alwaysApply: false - Only include if globs exist AND match file context", () => {
        ...
        // Test with no file context
        let applicableRules = getApplicableRules(messageWithoutFile, [conditionalRule, neverApplyRule], []);
        expect(applicableRules).toHaveLength(0); // No rules should apply
        // Test with matching file
        applicableRules = getApplicableRules(undefined, [conditionalRule, neverApplyRule], [matchingFileContext]);
        expect(applicableRules).toHaveLength(1);
        expect(applicableRules[0].name).toBe("Conditional Rule");
        expect(applicableRules.map((r) => r.name)).not.toContain("Never Apply Rule");
        // Test with non-matching file
        applicableRules = getApplicableRules(undefined, [conditionalRule, neverApplyRule], [nonMatchingFileContext]);
        expect(applicableRules).toHaveLength(0); // No rules should apply
    });
    it("alwaysApply: undefined - Default behavior: include if no globs exist OR globs exist and match", () => {
        ...
        // Test with no file context
        let applicableRules = getApplicableRules(messageWithoutFile, [defaultNoGlobsRule, defaultWithGlobsRule], []);
        expect(applicableRules).toHaveLength(1);
        expect(applicableRules[0].name).toBe("Default No Globs Rule");
        expect(applicableRules.map((r) => r.name)).not.toContain("Default With Globs Rule");
        // Test with matching file
        applicableRules = getApplicableRules(undefined, [defaultNoGlobsRule, defaultWithGlobsRule], [matchingFileContext]);
        expect(applicableRules).toHaveLength(2);
        expect(applicableRules.map((r) => r.name)).toContain("Default No Globs Rule");
        expect(applicableRules.map((r) => r.name)).toContain("Default With Globs Rule");
        // Test with non-matching file
        applicableRules = getApplicableRules(undefined, [defaultNoGlobsRule, defaultWithGlobsRule], [nonMatchingFileContext]);
        expect(applicableRules).toHaveLength(1);
        expect(applicableRules[0].name).toBe("Default No Globs Rule");
        expect(applicableRules.map((r) => r.name)).not.toContain("Default With Globs Rule");
    });
