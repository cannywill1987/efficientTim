...
describe("Retry Functionality", () => {
    ...
    describe("retryAsync", () => {
        ...
        it("should respect max attempts", async () => {
            ...
            error.code = "ECONNRESET";
            const mockFn = jest.fn().mockRejectedValue(error);
            await expect(retryAsync(mockFn, {
                maxAttempts: 3,
                baseDelay: 10,
        ...
        it("should retry on embedded 429 in error message (Gemini/VertexAI)", async () => {
            const error = new Error('Error from API: {"error":{"code":429,"message":"Resource exhausted"}}');
            const mockFn = jest
                .fn()
                .mockRejectedValueOnce(error)
                .mockResolvedValue("success");
        ...
        it("should retry on embedded 429 with space in error message", async () => {
            const error = new Error('Error from API: {"error":{"code": 429,"message":"Rate limit exceeded"}}');
            const mockFn = jest
                .fn()
                .mockRejectedValueOnce(error)
                .mockResolvedValue("success");
    ...
    describe("rate limiting headers", () => {
        it("should respect retry-after header in seconds", async () => {
            ...
            const error = new Error("Too Many Requests");
            error.status = 429;
            error.headers = { "retry-after": "1" }; // 1 second for fast testing
            const mockFn = jest
                .fn()
                .mockRejectedValueOnce(error)
                .mockResolvedValue("success");
        ...
        it("should respect x-ratelimit-reset header", async () => {
            ...
            error.status = 429;
            error.headers = { "x-ratelimit-reset": "0.5" }; // 500ms for fast testing
            const mockFn = jest
                .fn()
                .mockRejectedValueOnce(error)
                .mockResolvedValue("success");
        ...
        it("should respect maxDelay even with rate limit headers", async () => {
            ...
            const error = new Error("Rate limited");
            error.status = 429;
            error.headers = { "retry-after": "10" }; // 10 seconds requested
            const mockFn = jest
                .fn()
                .mockRejectedValueOnce(error)
                .mockResolvedValue("success");
        ...
        it("should handle case-insensitive header names", async () => {
            ...
            const error = new Error("Rate limited");
            error.status = 429;
            error.headers = { "Retry-After": "0.3" }; // 300ms for fast testing
            const mockFn = jest
                .fn()
                .mockRejectedValueOnce(error)
                .mockResolvedValue("success");
