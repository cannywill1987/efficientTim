function calculateAnthropicCost(model, usage) {
    ...
    if (usage.promptTokens > 0) {
        breakdownParts.push(`Input: ${usage.promptTokens.toLocaleString()} tokens × $${modelPricing.input}/MTok = $${inputCost.toFixed(6)}`);
    }
    // Output tokens breakdown
    if (usage.completionTokens > 0) {
        breakdownParts.push(`Output: ${usage.completionTokens.toLocaleString()} tokens × $${modelPricing.output}/MTok = $${outputCost.toFixed(6)}`);
    }
    ...
    if (usage.promptTokensDetails) {
        ...
        if (cacheWriteTokens && cacheWriteTokens > 0) {
            const cacheWriteCost = (cacheWriteTokens / 1_000_000) * modelPricing.cacheWrite;
            cacheCost += cacheWriteCost;
            breakdownParts.push(`Cache Write: ${cacheWriteTokens.toLocaleString()} tokens × $${modelPricing.cacheWrite}/MTok = $${cacheWriteCost.toFixed(6)}`);
        }
        if (cachedTokens && cachedTokens > 0) {
            const cacheReadCost = (cachedTokens / 1_000_000) * modelPricing.cacheRead;
            cacheCost += cacheReadCost;
            breakdownParts.push(`Cache Read: ${cachedTokens.toLocaleString()} tokens × $${modelPricing.cacheRead}/MTok = $${cacheReadCost.toFixed(6)}`);
        }
    }
...
export function calculateRequestCost(provider, model, usage) {
