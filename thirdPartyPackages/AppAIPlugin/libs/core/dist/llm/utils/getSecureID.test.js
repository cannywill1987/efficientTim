...
describe("getSecureID", () => {
    ...
    test("should generate a UUID on first call", () => {
        ...
        expect(result).toMatch(/^<!-- SID: [0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12} -->$/);
        // Should have set the static uuid property
        expect(getSecureID.uuid).toBeDefined();
        expect(typeof getSecureID.uuid).toBe("string");
    });
