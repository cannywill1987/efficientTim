...
function getFunctionCalls(items) {
    return items.filter((i) => isItemType(i, "function_call"));
}
function getFunctionCallOutputs(items) {
    return items.filter((i) => isItemType(i, "function_call_output"));
}
function getReasoningItems(items) {
    return items.filter((i) => isItemType(i, "reasoning"));
}
function getMessagesByRole(items, role) {
    ...
    describe("toResponsesInput", () => {
        ...
        describe("reasoning item sanitization", () => {
            ...
            it("should strip msg_ id from message after removed reasoning", () => {
                ...
                const assistantMsgs = getMessagesByRole(result, "assistant");
                expect(assistantMsgs.length).toBe(1);
                expect(assistantMsgs[0].id).toBeUndefined();
            });
