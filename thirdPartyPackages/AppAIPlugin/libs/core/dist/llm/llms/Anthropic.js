...
import { addCacheControlToLastTwoUserMessages, getAnthropicErrorMessage, getAnthropicHeaders, getAnthropicMediaTypeFromDataUrl, } from "@continuedev/openai-adapters";
...
class Anthropic extends BaseLLM {
    ...
    async *_streamChat(messages, signal, options) {
        if (!this.apiKey || this.apiKey === "") {
            throw new Error("Request not sent. You have an Anthropic model configured in your config.yaml, but the API key is not set.");
        }
        const systemMessage = stripImages(messages.filter((m) => m.role === "system")[0]?.content ?? "");
        ...
        const headers = getAnthropicHeaders(this.apiKey, shouldCacheSystemMessage || shouldCachePrompt, this.apiBase);
        const body = {
