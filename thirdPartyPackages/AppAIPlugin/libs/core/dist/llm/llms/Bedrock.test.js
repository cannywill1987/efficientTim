...
describe("Bedrock", () => {
    ...
    describe("message conversion", () => {
        ...
        it("should handle redacted thinking messages", () => {
            ...
            const redactedContent = converted[1].content?.[0].reasoningContent?.redactedContent;
            expect(redactedContent).toBeInstanceOf(Uint8Array);
            expect(Buffer.from(redactedContent).toString()).toBe("redacted-content-bytes");
        });
        it("should convert multimodal content correctly", () => {
            ...
            expect(Buffer.from(imageBytes).toString("base64")).toBe("/9j/4AAQSkZJRg==");
        });
        it("should add cache points when cacheBehavior.cacheConversation is enabled", () => {
            ...
            const availableTools = new Set();
            const converted = bedrockWithCaching["_convertMessages"](messages, availableTools);
            ...
            expect(converted[0].content?.some((block) => block.cachePoint)).toBe(true);
            expect(converted[1].role).toBe("assistant");
            expect(converted[2].role).toBe("user");
            expect(converted[2].content?.some((block) => block.cachePoint)).toBe(true);
        ...
        it("should add cache points when completionOptions.promptCaching is enabled", () => {
            ...
            const availableTools = new Set();
            const converted = bedrockWithPromptCaching["_convertMessages"](messages, availableTools);
            // The last two user messages should have cache points
            expect(converted.length).toBe(3);
            expect(converted[0].role).toBe("user");
            expect(converted[0].content?.some((block) => block.cachePoint)).toBe(true);
        ...
        it("should not add cache points when both caching options are disabled", () => {
            ...
            const availableTools = new Set();
            const converted = bedrockNoCaching["_convertMessages"](messages, availableTools);
            // No cache points should be added
            expect(converted.length).toBe(3);
            expect(converted[0].role).toBe("user");
            expect(converted[0].content?.some((block) => block.cachePoint)).toBe(false);
            expect(converted[1].role).toBe("assistant");
            expect(converted[2].role).toBe("user");
            expect(converted[2].content?.some((block) => block.cachePoint)).toBe(false);
