...
export class CodeRenderer {
    ...
    themeExists(themeNameKebab) {
        ...
        return themeArray.includes(themeNameKebab);
    }
    async highlightCode(code, language = "javascript", currLineOffsetFromTop, newDiffLines) {
    ...
    async convertToSVG(code, language = "javascript", options, currLineOffsetFromTop, newDiffLines, newDiffChars) {
        const strokeWidth = 1;
        const highlightedCodeHtml = await this.highlightCode(code, language, currLineOffsetFromTop, newDiffLines);
        const { guts, lineBackgrounds } = this.convertShikiHtmlToSvgGut(highlightedCodeHtml, options, newDiffChars);
    ...
    convertShikiHtmlToSvgGut(shikiHtml, options, diffChars) {
        ...
        const lineBackgrounds = lines
            ...
            const backgrounds = [];
            if (classes.includes("highlighted")) {
                backgrounds.push(`<rect x="0" y="${y}" width="100%" height="${options.lineHeight}" fill="${this.editorLineHighlight}" shape-rendering="crispEdges" />`);
            }
            segments.forEach(({ start, end }) => {
                ...
                backgrounds.push(`<rect x="${x}" y="${y}" width="${segmentWidth}" height="${options.lineHeight}" fill="${additionFill}" shape-rendering="crispEdges" />`);
            });
    ...
    async getDataUri(code, language = "javascript", options, currLineOffsetFromTop, newDiffLines, newDiffChars) {
        switch (options.imageType) {
            ...
            case "svg":
                const svgBuffer = await this.convertToSVG(code, language, options, currLineOffsetFromTop, newDiffLines, newDiffChars);
