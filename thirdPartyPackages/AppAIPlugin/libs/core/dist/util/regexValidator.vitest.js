...
import { escapeLiteralForRegex, looksLikeLiteralSearch, prepareQueryForRipgrep, validateAndSanitizeRegex, } from "./regexValidator";
describe("validateAndSanitizeRegex", () => {
    ...
    it("should warn about unescaped brackets without fixing", () => {
        ...
        expect(result.isValid).toBe(true);
        expect(result.sanitizedQuery).toBe("hello[world]");
        expect(result.warning).toContain("Warning: Unescaped brackets or parentheses");
    });
    ...
    it("should warn about lookbehind assertions", () => {
        ...
        expect(result.isValid).toBe(true);
        expect(result.warning).toContain("Lookahead/lookbehind assertions require ripgrep to be compiled with PCRE2");
    });
});
describe("escapeLiteralForRegex", () => {
    ...
    it("should handle multiple metacharacters", () => {
        expect(escapeLiteralForRegex("$regex.test(input)")).toBe("\\$regex\\.test\\(input\\)");
    });
...
describe("patterns that should NOT be over-sanitized", () => {
    ...
    describe("complex patterns that mix literal and regex", () => {
        it("should not break URLs with query parameters", () => {
            const result = prepareQueryForRipgrep("https://api\\.example\\.com\\?param=\\w+");
