...
import { pathToUriPathSegment, findUriInDirs, getUriPathBasename, getUriFileExtension, getFileExtensionFromBasename, getLastNUriRelativePathParts, joinPathsToUri, getShortestUniqueRelativeUriPaths, getLastNPathParts, } from "./uri";
describe("uri utils", () => {
    describe("pathToUriPathSegment", () => {
        it("should convert Windows paths to URI segments", () => {
            ...
            expect(pathToUriPathSegment("\\this\\is\\afile.ts")).toBe("this/is/afile.ts");
        });
        ...
        it("should encode URI special characters", () => {
            expect(pathToUriPathSegment("path/with spaces/file")).toBe("path/with%20spaces/file");
            expect(pathToUriPathSegment("special#chars?in&path")).toBe("special%23chars%3Fin%26path");
        });
    });
    describe("findUriInDirs", () => {
        ...
        it("should find URI in directory", () => {
            const result = findUriInDirs("file:///workspace/project1/src/file.ts", dirUris);
        ...
        it("should throw error for invalid URIs", () => {
            expect(() => findUriInDirs("invalid-uri", dirUris)).toThrow("Invalid uri: invalid-uri");
        });
    });
    ...
    describe("joinPathsToUri", () => {
        it("should join paths to base URI", () => {
            expect(joinPathsToUri("file:///base", "path", "to", "file.txt")).toBe("file:///base/path/to/file.txt");
        });
        it("should handle paths with special characters", () => {
            expect(joinPathsToUri("file:///base", "path with spaces", "file.txt")).toBe("file:///base/path%20with%20spaces/file.txt");
        });
    });
    ...
    describe("getLastNPathParts", () => {
        it("should get last N parts of path", () => {
            expect(getLastNPathParts("path/to/some/file.txt", 2)).toBe("some/file.txt");
            expect(getLastNPathParts("path/to/some/file.txt", 0)).toBe("");
        });
        it("should handle Windows paths", () => {
            expect(getLastNPathParts("path\\to\\some\\file.txt", 2)).toBe("some/file.txt");
        });
    });
    describe("getLastNUriRelativePathParts", () => {
        it("should get last N parts of URI relative path", () => {
            ...
            expect(getLastNUriRelativePathParts(dirUris, uri, 2)).toBe("components/Button.tsx");
        });
