...
import { getSessionFilePath, getSessionsFolderPath, getSessionsListPath, } from "./paths.js";
...
export class HistoryManager {
    list(options) {
        ...
        if (options.workspaceDirectory) {
            ...
            sessions = sessions.filter((session) => typeof session.workspaceDirectory === "string" &&
    ...
    save(session) {
        ...
        fs.writeFileSync(getSessionFilePath(session.sessionId), JSON.stringify(orderedSession, undefined, 2));
        // Read and update the sessions list
        ...
        try {
            ...
            let found = false;
            const messageCount = session.history.filter((item) => item.message.role === "assistant").length;
