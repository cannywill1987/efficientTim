import { SentryLogger, captureException, captureLog, createSpan, initializeSentry, } from "./SentryLogger";
...
describe("SentryLogger Integration Tests", () => {
    ...
    describe("Telemetry Toggle Behavior", () => {
        it("should not initialize when telemetry is disabled", async () => {
            await SentryLogger.setup(false, "test-id", mockIdeInfo, "test@continue.dev");
        ...
        it("should not initialize in test environment even when telemetry is enabled", async () => {
            ...
            await SentryLogger.setup(true, "test-id", mockIdeInfo, "test@continue.dev");
        ...
        it("should not reinitialize if client already exists", async () => {
            ...
            await SentryLogger.setup(true, "test-id-2", mockIdeInfo, "test@continue.dev");
...
describe("Sentry Utility Functions", () => {
    ...
    describe("createSpan", () => {
        ...
        it("should handle async callbacks", async () => {
            ...
            const result = await createSpan("test.operation", "test span", asyncCallback);
    ...
    describe("captureException", () => {
        ...
        it("should handle malformed context gracefully", async () => {
            ...
            await SentryLogger.setup(true, "test-id", mockIdeInfo, "test@continue.dev");
            const error = new Error("test error");
            expect(() => captureException(error, null)).not.toThrow();
    ...
    describe("captureLog", () => {
        ...
        it("should handle malformed context gracefully", async () => {
            ...
            process.env.NODE_ENV = "development";
            await SentryLogger.setup(true, "test-id", mockIdeInfo, "test@continue.dev");
            expect(() => captureLog("test message", "info", null)).not.toThrow();
...
describe("Sentry Data Anonymization Tests", () => {
    describe("anonymizeSentryEvent", () => {
        it("should anonymize user file paths in stack traces", () => {
            ...
            expect(result?.exception?.values?.[0]?.stacktrace?.frames?.[0]?.filename).toBe("<file>");
            ...
            expect(result?.exception?.values?.[0]?.stacktrace?.frames?.[0]?.pre_context).toBeUndefined();
            expect(result?.exception?.values?.[0]?.stacktrace?.frames?.[0]?.post_context).toBeUndefined();
            expect(result?.exception?.values?.[0]?.stacktrace?.frames?.[0]?.context_line).toBe("<code>");
        });
        ...
        it("should preserve node_modules package names for debugging", () => {
                    ...
                    values: [
                        {
                            stacktrace: {
                                frames: [
                                    {
                                        filename: "/Users/dev/project/node_modules/@sentry/node/index.js",
                                        abs_path: "/Users/dev/project/node_modules/@sentry/node/index.js",
                                        function: "init",
                                        lineno: 123,
                                    },
                                ],
            ...
            const result = anonymizeSentryEvent(event);
            expect(result).not.toBeNull();
            expect(result?.exception?.values?.[0]?.stacktrace?.frames?.[0]?.filename).toBe("node_modules/@sentry/<file>");
            expect(result?.exception?.values?.[0]?.stacktrace?.frames?.[0]?.abs_path).toBe("");
        });
        ...
        it("should preserve breadcrumbs unchanged", () => {
            const event = {
                breadcrumbs: [
                    {
                        message: "File opened: /Users/john/Documents/secret-project/config.js",
                        category: "file",
                        data: {
            ...
            expect(result?.breadcrumbs?.[0]?.message).toBe("File opened: /Users/john/Documents/secret-project/config.js");
            expect(result?.breadcrumbs?.[0]?.data?.filename).toBe("/Users/john/Documents/secret-project/config.js");
            expect(result?.breadcrumbs?.[0]?.data?.action).toBe("open");
            expect(result?.breadcrumbs?.[1]?.message).toBe("Error in module initialization");
        ...
        it("should anonymize thread stack traces", () => {
            ...
            const result = anonymizeSentryEvent(event);
            expect(result).not.toBeNull();
            expect(result?.threads?.values?.[0]?.stacktrace?.frames?.[0]?.filename).toBe("<file>");
            expect(result?.threads?.values?.[0]?.stacktrace?.frames?.[0]?.abs_path).toBe("");
            expect(result?.threads?.values?.[0]?.stacktrace?.frames?.[0]?.vars).toBeUndefined();
            expect(result?.threads?.values?.[0]?.stacktrace?.frames?.[0]?.context_line).toBe("<code>");
        });
        ...
        it("should anonymize all absolute paths regardless of format", () => {
                    ...
                    values: [
                        {
                            stacktrace: {
                                frames: [
                                    {
                                        filename: "C:\\Users\\dev\\continue\\extensions\\vscode\\src\\extension.js",
                                        abs_path: "C:\\Users\\dev\\continue\\extensions\\vscode\\src\\extension.js",
                                    },
                                    {
            ...
            expect(result?.exception?.values?.[0]?.stacktrace?.frames?.[0]?.filename).toBe("<file>");
            expect(result?.exception?.values?.[0]?.stacktrace?.frames?.[1]?.filename).toBe("<file>");
            expect(result?.exception?.values?.[0]?.stacktrace?.frames?.[2]?.filename).toBe("relative/path/file.js"); // Relative path preserved
            // All abs_path fields cleared
            expect(result?.exception?.values?.[0]?.stacktrace?.frames?.[0]?.abs_path).toBe("");
            expect(result?.exception?.values?.[0]?.stacktrace?.frames?.[1]?.abs_path).toBe("");
            expect(result?.exception?.values?.[0]?.stacktrace?.frames?.[2]?.abs_path).toBe("");
        });
    });
});
