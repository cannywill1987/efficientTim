...
/** Converts any OS path to cleaned up URI path segment format with no leading/trailing slashes
  ...
  **/
export function pathToUriPathSegment(path) {
...
export function getCleanUriPath(uri) {
    const path = URI.parse(uri).path ?? "";
    let clean = path.replace(/^\//, ""); // remove start slash
    clean = clean.replace(/\/$/, ""); // remove end slash
    return clean;
}
export function findUriInDirs(uri, dirUriCandidates) {
...
*/
export function getUriPathBasename(uri) {
    const path = getCleanUriPath(uri);
    const basename = path.split("/").pop() || "";
    return decodeURIComponent(basename);
}
export function getFileExtensionFromBasename(basename) {
...
export function joinEncodedUriPathSegmentToUri(uri, pathSegment) {
    let baseUri = uri;
    if (baseUri.at(-1) !== "/") {
        baseUri += "/";
    }
...
export function getShortestUniqueRelativeUriPaths(uris, dirUriCandidates) {
...
export function getLastNPathParts(filepath, n) {
    if (n <= 0) {
        return "";
    }
    return filepath.split(/[\\/]/).slice(-n).join("/");
}
export function getUriDescription(uri, dirUriCandidates) {
