...
import { copyOf, dedent, dedentAndGetCommonWhitespace, deduplicateArray, getMarkdownLanguageTagForFile, removeCodeBlocksAndTrim, removeQuotesAndEscapes, } from "./";
describe("deduplicateArray", () => {
    ...
    it("should work with custom equality function for complex objects", () => {
        ...
        const result = deduplicateArray(input, (a, b) => a.x === b.x && a.y === b.y);
        expect(result).toEqual([
            { x: 1, y: 2 },
            { x: 2, y: 1 },
            { x: 3, y: 4 },
...
describe("removeCodeBlocksAndTrim", () => {
    ...
    it("should remove think blocks from text", () => {
        const text = "<think>Okay, the user wants me to help with...</think>Here is my actual response.";
        const output = removeCodeBlocksAndTrim(text);
        expect(output).toBe("Here is my actual response.");
    });
    it("should remove both code blocks and think blocks", () => {
        const text = "<think>Let me plan this...</think>Here's some code:\n```javascript\nconsole.log('hello');\n```\nThat should work!";
        const output = removeCodeBlocksAndTrim(text);
