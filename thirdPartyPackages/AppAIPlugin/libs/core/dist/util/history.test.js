...
describe("Full session lifecycle", () => {
    ...
    test("Deleting session", () => {
        ...
        const sessions = historyManager.list({});
        const sessionWasDeleted = sessions.every((session) => session?.sessionId !== testSession.sessionId);
...
describe("Workspace directory filtering", () => {
    ...
    test("Filter sessions by workspace directory", () => {
        ...
        expect(sessions.every((s) => s.workspaceDirectory === "/home/user/project-a")).toBe(true);
    });
...
describe("Many sessions created", () => {
    ...
    test("List 10 sessions, offest by 10", () => {
        ...
        const sessionIds = Array.from({ length: limit }, (_, i) => (99 - offset - i).toString());
        const isSessionIdInList = (sessionId) => sessions.some((session) => session.sessionId === sessionId);
    ...
    test("List sessions offset by 75", () => {
        ...
        const sessionIds = Array.from({ length: sessions.length - offset }, (_, i) => (i + offset).toString());
        const isSessionIdInList = (sessionId) => sessions.some((session) => session.sessionId === sessionId);
        expect(sessionIds.every(isSessionIdInList)).toBe(true);
    });
