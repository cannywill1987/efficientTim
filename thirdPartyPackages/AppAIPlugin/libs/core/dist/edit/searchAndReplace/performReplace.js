 ...
 */
function getLineIndentAtPosition(fileContent, position) {
    const lineStart = fileContent.lastIndexOf("\n", position - 1) + 1;
    const lineEnd = fileContent.indexOf("\n", lineStart);
    const line = fileContent.substring(lineStart, lineEnd === -1 ? fileContent.length : lineEnd);
    const match = line.match(/^(\s*)/);
    return match ? match[1] : "";
}
/**
...
function adjustReplacementIndentation(fileContent, match, oldString, newString) {
    if (match.strategyName === "exactMatch" ||
...
export function executeFindAndReplace(fileContent, oldString, newString, replaceAll, editIndex = 0) {
    const matches = findSearchMatches(fileContent, oldString);
    if (matches.length === 0) {
        throw new ContinueError(ContinueErrorReason.FindAndReplaceOldStringNotFound, `Edit at index ${editIndex}: string not found in file: "${oldString}"`);
    }
    if (replaceAll) {
        ...
        for (let i = matches.length - 1; i >= 0; i--) {
            ...
            const adjustedNew = adjustReplacementIndentation(result, match, oldString, newString);
            result =
        ...
        if (matches.length > 1) {
            throw new ContinueError(ContinueErrorReason.FindAndReplaceMultipleOccurrences, `Edit at index ${editIndex}: String "${oldString}" appears ${matches.length} times in the file. Either provide a more specific string with surrounding context to make it unique, or use replace_all=true to replace all occurrences.`);
        }
        // Apply single replacement
        const match = matches[0];
        const adjustedNew = adjustReplacementIndentation(fileContent, match, oldString, newString);
        return (fileContent.substring(0, match.startIndex) +
            adjustedNew +
            fileContent.substring(match.endIndex));
    }
}
export function executeMultiFindAndReplace(fileContent, edits) {
    let result = fileContent;
