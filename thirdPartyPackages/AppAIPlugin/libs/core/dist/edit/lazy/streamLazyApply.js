import { filterLeadingAndTrailingNewLineInsertion, filterLeadingNewline, removeTrailingWhitespace, } from "../../autocomplete/filtering/streamTransforms/lineStream.js";
...
export async function* streamLazyApply(oldCode, filename, newCode, llm, abortController) {
    ...
    async function* replacementFunction(oldCode, linesBefore, linesAfter) {
        for await (const line of getReplacementWithLlm(oldCode, linesBefore, linesAfter, llm, abortController)) {
            yield line;
        }
    }
...
async function* streamFillUnchangedCode(lines, oldCode, replacementFunction) {
    const newLines = [];
    let buffer = [];
