...
async function collectDiffs(oldFile, newFile, filename) {
    const ourDiffs = [];
...
function displayDiff(diff) {
    return diff
        .map(({ type, line }) => type === "same" ? line : `${UNIFIED_DIFF_SYMBOLS[type]} ${line}`)
        .join("\n");
}
function normalizeDisplayedDiff(d) {
    return d
        .split("\n")
        .map((line) => (line.trim() === "" ? "" : line))
        .join("\n");
}
async function expectDiff(file) {
    const testFilePath = path.join(__dirname, "edit", "lazy", "test-examples", file + ".diff");
    ...
    if (!expectedDiff || expectedDiff.trim() === "") {
        ...
        fs.writeFileSync(testFilePath, `${oldFile}\n\n---\n\n${newFile}\n\n---\n\n${displayDiff(continueMyersDiff(oldFile, newFile))}`);
        throw new Error("Expected diff is empty");
    }
    expect(normalizeDisplayedDiff(displayedDiff)).toEqual(normalizeDisplayedDiff(expectedDiff));
}
describe("deterministicApplyLazyEdit(", () => {
    ...
    `;
        const { ourDiffs: streamDiffs, myersDiffs } = await collectDiffs(file, file, "test.js");
        expect(streamDiffs).toEqual(file.split("\n").map((line) => ({
            line,
            type: "same",
        })));
