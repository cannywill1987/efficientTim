...
export function isLazyText(text) {
...
export async function deterministicApplyLazyEdit({ oldFile, newLazyFile, filename, 
/**
...
onlyFullFileRewrite = false, }) {
    ...
    let reconstructedNewFile = undefined;
    ...
    if (!findInAst(newTree.rootNode, isLazyBlock)) {
        ...
        const firstSimilarNode = findInAst(oldTree.rootNode, (node) => nodesAreSimilar(node, newTree.rootNode.children[0]));
        if (firstSimilarNode?.parent?.equals(oldTree.rootNode)) {
            ...
            const matchingNode = findInAst(oldTree.rootNode, (node) => programNodeIsSimilar(newTree.rootNode, node), 
...
function nodesAreSimilar(a, b) {
    ...
    if (a.namedChildren[0]?.text === b.namedChildren[0]?.text &&
...
function findLazyBlockReplacements(oldNode, newNode, replacements) {
    ...
    let isLazy = false;
    let currentLazyBlockNode = undefined;
