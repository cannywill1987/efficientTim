...
\`\`\``;
        ...
        const diffLines = [];
        for await (const diffLine of streamLazyApply(oldCode, filename, newCode, llm, abortController)) {
...
function calculateProduct(a, b) {
  return a * b;
        ...
        const llm = new MockJavaScriptLLM();
        const abortController = new AbortController();
        const diffLines = [];
        for await (const diffLine of streamLazyApply(oldCode, filename, newCode, llm, abortController)) {
            diffLines.push(diffLine);
...
function anotherFunction() {
  return 'updated';
        ...
        const llm = new MockWithUnchangedCodeLLM();
        const abortController = new AbortController();
        const diffLines = [];
        for await (const diffLine of streamLazyApply(oldCode, filename, newCode, llm, abortController)) {
            diffLines.push(diffLine);
...
function getUser(id: number): User {
  return { id, name: 'Test User' };
        ...
        const llm = new MockTypeScriptLLM();
        const abortController = new AbortController();
        const diffLines = [];
        for await (const diffLine of streamLazyApply(oldCode, filename, newCode, llm, abortController)) {
            diffLines.push(diffLine);
...
\`\`\``;
        const filename = "README.md";
        const llm = new MockRegularMarkdownLLM();
        const abortController = new AbortController();
        const diffLines = [];
        for await (const diffLine of streamLazyApply(oldCode, filename, newCode, llm, abortController)) {
            diffLines.push(diffLine);
