...
async function genToArr(generator) {
    const result = [];
...
async function genToStrs(generator) {
...
describe.skip("codeChunker", () => {
    ...
    test("should capture small class and function from large python file", async () => {
        ...
        const file = Array(100).fill(extraLine).join("\n") +
            "\n\n" +
            myClass +
            "\n\n" +
            myFunction +
    ...
    test("should split large python class into methods and class with truncated methods", async () => {
        ...
        const chunks = await genToStrs(codeChunker("test.py", file, 200));
        expect(chunks.length).toBeGreaterThan(1);
        expect(chunks[0].startsWith("class MyClass:\n    def method1():\n        ...")).toBe(true);
...
async function collectContents(filepath, contents, maxChunkSize) {
    const out = [];
...
describe("codeChunker function definition", () => {
    ...
    test("class methods never exceed maxChunkSize", async () => {
        const body = Array.from({ length: 200 }, (_, i) => `console.log(${i});`).join("\n");
        const contents = `
class C {
  a() {
    ${body}
...
`;
    ...
    test("top-level functions never exceed maxChunkSize", async () => {
        const body = Array.from({ length: 150 }, (_, i) => `const x${i} = ${i};`).join("\n");
        const contents = `
function f() {
  ${body}
}
