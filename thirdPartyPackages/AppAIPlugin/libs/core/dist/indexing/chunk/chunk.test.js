...
describe("chunkDocument", () => {
    ...
    test("should filter out chunks that exceed maxChunkSize in tokens", async () => {
        ...
        const contents = "This is a much longer line with many words that will likely exceed the maxChunkSize token limit and should be filtered out by the chunkDocument function. This is a much longer line with many words that will likely exceed the maxChunkSize token limit and should be filtered out by the chunkDocument function.";
        const chunks = [];
        for await (const chunk of chunkDocument({
            filepath,
            contents,
...
describe("chunkDocumentWithoutId", () => {
    ...
    test("should return no chunks for empty content", async () => {
        const contents = "";
        const chunks = [];
        for await (const chunk of chunkDocumentWithoutId("file.txt", contents, 100)) {
            chunks.push(chunk);
        }
    ...
    test("should chunk for code files", async () => {
        ...
        const chunks = [];
        for await (const chunk of chunkDocumentWithoutId("file.js", contents, 100)) {
            chunks.push(chunk);
        }
        expect(chunks.length).toBeGreaterThan(0);
    ...
    test("should chunk large content", async () => {
        ...
        const chunks = [];
        for await (const chunk of chunkDocumentWithoutId("file.txt", contents, maxChunkSize)) {
            chunks.push(chunk);
        }
