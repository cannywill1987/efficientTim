...
import { addToTestDir, setUpTestDir, tearDownTestDir, TEST_DIR, TEST_DIR_PATH, } from "../test/testDir.js";
...
class TestCodebaseIndexer extends CodebaseIndexer {
    ...
    testHasIndexingProvider() {
        return this.hasIndexingContextProvider();
    }
    async testHandleConfigUpdate(configResult) {
        return this.handleConfigUpdate({ config: configResult.config });
    }
}
...
describe("CodebaseIndexer", () => {
    ...
    beforeAll(async () => {
        ...
        codebaseIndexer = new TestCodebaseIndexer(testConfigHandler, testIde, mockMessenger, false);
    ...
    async function refreshIndex() {
        ...
        const updates = [];
        for await (const update of codebaseIndexer.refreshDirs([TEST_DIR], abortSignal)) {
            updates.push(update);
        }
        return updates;
    }
    async function getAllIndexedFiles() {
        const files = await testIndex.getIndexedFilesForTags(await testIde.getTags(testIndex.artifactId));
        return files;
    }
    async function getIndexPlan() {
        ...
        const [results, lastUpdated, markComplete] = await getComputeDeleteAddRemove(tag, { ...stats }, (filepath) => testIde.readFile(filepath), undefined);
        return results;
    }
    async function expectPlan(compute, addTag, removeTag, del) {
    ...
    describe("Sequential Integration Tests", () => {
        ...
        test("should have created index folder with all necessary files", async () => {
            const exists = await testIde.fileExists(localPathToUri(getIndexSqlitePath()));
        ...
        test("should create git repo for testing", async () => {
            execSync(`cd ${TEST_DIR_PATH} && git init && git checkout -b main && git add -A && git commit -m "First commit"`);
        });
        test.skip("should only re-index the changed files when changing branches", async () => {
            ...
            execSync(`cd ${TEST_DIR_PATH} && git add -A && git commit -m "Change .ts file"`);
        });
    ...
    describe("Core.ts Methods (Isolated Unit Tests)", () => {
        ...
        test("should call messenger with progress updates when refreshing codebase index", async () => {
            jest
                .spyOn(codebaseIndexer, "refreshDirs")
        ...
        test("should call messenger with progress updates when refreshing specific files", async () => {
            jest
                .spyOn(codebaseIndexer, "refreshFiles")
                .mockImplementation(async function* () {
                yield { status: "done", progress: 1, desc: "Completed" };
            });
        ...
        test("should abort previous indexing when starting a new one", async () => {
            ...
            codebaseIndexer.indexingCancellationController = controller;
    ...
    describe("Configuration Handling (Isolated Unit Tests)", () => {
        ...
        let testIndexer;
        let mockRefreshCodebaseIndex;
        let mockGetWorkspaceDirs;
        beforeEach(() => {
            testIndexer = new TestCodebaseIndexer(testConfigHandler, testIde, mockMessenger, false);
        ...
        describe("handleConfigUpdate", () => {
            ...
            test("should set config property before checking conditions", async () => {
                ...
                expect(testIndexer.config).toBe(testConfig);
        ...
        describe("getIndexesToBuild", () => {
            let indexer;
            let mockConfig;
            let mockEmbeddingsModel;
            let mockIdeSettings;
            beforeEach(async () => {
                indexer = new CodebaseIndexer(testConfigHandler, testIde, mockMessenger, false);
                mockEmbeddingsModel = {
            ...
            test("should create specific indexTypes for the requested context providers", async () => {
                ...
                const indexes = (await indexer.getIndexesToBuild());
    ...
    describe("wasAnyOneIndexAdded", () => {
        ...
        test("should return true when indexes to build differ from built indexes", async () => {
            ...
            testIndexer.builtIndexes = [mockIndex2];
        ...
        test("should return true when there are more indexes to build than built indexes", async () => {
            ...
            testIndexer.builtIndexes = [mockIndex1];
            const result = await testIndexer.wasAnyOneIndexAdded();
            expect(result).toBe(true);
        });
        test("should return false when there are fewer indexes to build than built indexes", async () => {
            ...
            const mockIndex2 = new FullTextSearchCodebaseIndex();
            mockGetIndexesToBuild.mockResolvedValue([mockIndex1]);
            testIndexer.builtIndexes = [mockIndex1, mockIndex2];
            const result = await testIndexer.wasAnyOneIndexAdded();
            expect(result).toBe(false);
        });
    });
