...
import { editConfigFile, getDocsSqlitePath, getLanceDbPath, } from "../../util/paths";
import { Telemetry } from "../../util/posthog";
import { htmlPageToArticleWithChunks, markdownPageToArticleWithChunks, } from "./article";
...
export function embedModelsAreEqual(llm1, llm2) {
...
export default class DocsService {
    ...
    isInitialized;
    isSyncing = false;
    docsIndexingQueue = new Set();
    lanceTableNamesSet = new Set();
    config;
    sqliteDb;
    ideInfoPromise;
    githubToken;
    ...
    async indexAndAdd(siteIndexingConfig, forceReindex = false) {
        ...
        try {
            ...
            const docsCrawler = new DocsCrawler(this.ide, this.config, maxDepth, undefined, useLocalCrawling, this.githubToken);
    ...
    hasDocsContextProvider() {
        return !!this.config.contextProviders?.some((provider) => provider.description.title === DocsContextProvider.description.title);
    }
