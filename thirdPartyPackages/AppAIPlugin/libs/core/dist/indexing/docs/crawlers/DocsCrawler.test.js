...
import DocsCrawler, { ChromiumInstaller, } from "./DocsCrawler";
...
describe.skip("DocsCrawler", () => {
    ...
    describe.skip("GitHub Crawler", () => {
        const repoUrl = "https://github.com/Patrick-Erichsen/test-github-repo-for-crawling";
        let crawlResults;
        let crawlerUsed;
        beforeAll(async () => {
        ...
        test("finds expected markdown files", async () => {
            expect(crawlResults.some((page) => page.path.endsWith("README.md"))).toBe(true);
            expect(crawlResults.some((page) => page.path.endsWith("docs-depth-1.md"))).toBe(true);
        }, TIMEOUT_MS);
        test("html includes the correct content", async () => {
            const doc = crawlResults.find((page) => page.path.endsWith("docs-depth-1.md"));
        ...
        test("ignores non-markdown files, e.g. `test.js` at the root", async () => {
            expect(crawlResults.some((page) => page.path.endsWith("test.js"))).toBe(false);
        }, TIMEOUT_MS);
    });
    ...
    describe.skip("Chromium Crawler", () => {
        ...
        commonDocsSites.forEach((url) => {
            test(`Chromium crawler common site: ${url}`, async () => {
    ...
    describe("Default Crawler", () => {
        ...
        test("works on static site", async () => {
