...
export async function runLanceMigrations(table) {
    await new Promise((resolve) => {
        void migrate("rename_baseurl_column_for_lance_docs", async () => {
            try {
        ...
        void migrate("sqlite_modify_docs_columns_and_copy_to_config", async () => {
            try {
                ...
                if (needsToUpdateConfig) {
                    const sqliteDocs = await db.all(`SELECT title, startUrl FROM ${DocsService.sqlitebTableName}`);
