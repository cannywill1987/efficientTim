...
import { mockPathAndCacheKey, mockTag, updateIndexAndAwaitGenerator, } from "./test/indexing";
...
describe.skip("ChunkCodebaseIndex", () => {
    ...
    beforeAll(async () => {
        ...
        index = (await LanceDbIndex.create(mockConfig.selectedModelByRole.embed, testIde.readFile.bind(testIde)));
    ...
    it("should update the index and maintain expected database state", async () => {
        ...
        await updateIndexAndAwaitGenerator(index, "compute", mockMarkComplete, mainTag);
        ...
        expect(mockMarkComplete).toHaveBeenCalledWith([mockPathAndCacheKey], IndexResultType.Compute);
        // AddTag test - only inserts into Lance DB
        await updateIndexAndAwaitGenerator(index, "addTag", mockMarkComplete, devTag);
        let devTagTable = await lanceDb.openTable(devTagTableName);
        expect(await devTagTable.countRows()).toBe(1);
        expect(mockMarkComplete).toHaveBeenCalledWith([mockPathAndCacheKey], IndexResultType.AddTag);
        // RemoveTag - only removes from  Lance DB
        await updateIndexAndAwaitGenerator(index, "removeTag", mockMarkComplete, devTag);
        devTagTable = await lanceDb.openTable(devTagTableName);
        expect(await devTagTable.countRows()).toBe(0);
        expect(mockMarkComplete).toHaveBeenCalledWith([mockPathAndCacheKey], IndexResultType.RemoveTag);
        ...
        expect(mockMarkComplete).toHaveBeenCalledWith([mockPathAndCacheKey], IndexResultType.Delete);
    });
});
