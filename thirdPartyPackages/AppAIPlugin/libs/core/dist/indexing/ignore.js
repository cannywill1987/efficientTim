...
export const defaultSecurityIgnoreFile = ignore().add(DEFAULT_SECURITY_IGNORE_FILETYPES);
export const defaultSecurityIgnoreDir = ignore().add(DEFAULT_SECURITY_IGNORE_DIRS);
...
export const DEFAULT_SECURITY_IGNORE = DEFAULT_SECURITY_IGNORE_FILETYPES.join("\n") +
    "\n" +
...
export function isSecurityConcern(filePathOrUri) {
    if (!filePathOrUri) {
        return false;
    }
...
export function throwIfFileIsSecurityConcern(filepath) {
    if (isSecurityConcern(filepath)) {
        throw new ContinueError(ContinueErrorReason.FileIsSecurityConcern, `Reading or Editing ${filepath} is not allowed because it is a security concern. Do not attempt to read or edit this file in any way.`);
    }
}
export function gitIgArrayFromFile(file) {
    return file
        .split(/\r?\n/) // Split on new line
