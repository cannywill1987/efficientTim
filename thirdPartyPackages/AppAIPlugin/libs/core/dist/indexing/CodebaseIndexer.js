...
export class CodebaseIndexer {
    ...
    initPromise;
    config;
    indexingCancellationController;
    codebaseIndexingState;
    pauseToken;
    builtIndexes = [];
    getUserFriendlyIndexName(artifactId) {
        ...
        if (artifactId === ChunkCodebaseIndex.artifactId)
            return "Chunking";
    ...
    async getIndexesToBuild() {
        ...
        const continueServerClient = new ContinueServerClient(ideSettings.remoteConfigServerUrl, ideSettings.userToken);
        ...
        const indexTypeToIndexerMapping = {
            chunk: async () => new ChunkCodebaseIndex(this.ide.readFile.bind(this.ide), continueServerClient, embeddingsModel.maxEmbeddingChunkSize),
            codeSnippets: async () => new CodeSnippetsCodebaseIndex(this.ide),
            fullTextSearch: async () => new FullTextSearchCodebaseIndex(),
            embeddings: async () => {
                const lanceDbIndex = await LanceDbIndex.create(embeddingsModel, this.ide.readFile.bind(this.ide));
                return lanceDbIndex;
            },
        };
        const indexes = [];
    ...
    async refreshFile(file, workspaceDirs) {
        ...
        for (const index of indexesToBuild) {
            ...
            const [fullResults, fullLastUpdated, markComplete] = await getComputeDeleteAddRemove(tag, { ...stats }, (filepath) => this.ide.readFile(filepath), repoName);
    ...
    *batchRefreshIndexResults(results) {
    ...
    async *indexFiles(directory, files, branch, repoName) {
        ...
        for (const codebaseIndex of indexesToBuild) {
            ...
            try {
                const [results, lastUpdated, markComplete] = await getComputeDeleteAddRemove(tag, { ...stats }, (filepath) => this.ide.readFile(filepath), repoName);
    ...
    async refreshCodebaseIndex(paths) {
        ...
        const indexLockTimestampUpdateInterval = setInterval(() => void IndexLock.updateTimestamp(), 5_000);
        try {
    ...
    isIndexingConfigSame(config1, config2) {
        return embedModelsAreEqual(config1?.selectedModelByRole.embed, config2.selectedModelByRole.embed);
    }
