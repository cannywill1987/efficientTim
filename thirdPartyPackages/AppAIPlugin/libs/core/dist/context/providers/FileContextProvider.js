...
import { getShortestUniqueRelativeUriPaths, getUriDescription, getUriPathBasename, } from "../../util/uri";
...
class FileContextProvider extends BaseContextProvider {
    ...
    async getContextItems(query, extras) {
        ...
        const { relativePathOrBasename, last2Parts, baseName } = getUriDescription(fileUri, await extras.ide.getWorkspaceDirs());
        return [
            {
                name: baseName,
    ...
    async loadSubmenuItems(args) {
        ...
        const results = await walkDirs(args.ide, {
