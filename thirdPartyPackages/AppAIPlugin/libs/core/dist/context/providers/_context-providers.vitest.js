...
const CONTEXT_PROVIDERS_TO_TEST = [
    { name: "diff", params: {} },
...
async function getContextProviderExtras(fullInput) {
    setUpTestDir();
    const ide = new FileSystemIde(TEST_DIR);
    const llmLogger = new LLMLogger();
    const configHandler = new ConfigHandler(ide, llmLogger, Promise.resolve(undefined));
...
describe("Should successfully run all context providers", () => {
    ...
    CONTEXT_PROVIDERS_TO_TEST.forEach((provider) => {
        test(`should successfully run ${provider.name} context provider`, async () => {
            const cls = contextProviderClassFromName(provider.name);
            const instance = new cls(provider.params);
