...
export default class BaseRetrievalPipeline {
    options;
    ftsIndex = new FullTextSearchCodebaseIndex();
    lanceDbIndex = null;
    lanceDbInitPromise = null;
    constructor(options) {
    ...
    async retrieveAndChunkRecentlyEditedFiles(n) {
        const recentlyEditedFilesSlice = Array.from(openedFilesLruCache.keys()).slice(0, n);
  ...
  "tools": [
    {
      ...
      "args": { "<required_parameter_name>": "<required_parameter_value>" }
        ...
        for (const toolCall of toolCalls) {
            ...
            const contextItems = await callBuiltInTool(toolCall.name, args, toolExtras);
