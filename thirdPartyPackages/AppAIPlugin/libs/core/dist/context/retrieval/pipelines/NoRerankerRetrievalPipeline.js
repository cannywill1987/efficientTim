...
export default class NoRerankerRetrievalPipeline extends BaseRetrievalPipeline {
    async run(args) {
        ...
        try {
            ...
            await Telemetry.captureError("no_reranker_recently_edited_retrieval", error);
            console.error("Error retrieving recently edited files:", error);
        }
        let repoMapChunks = [];
        try {
            repoMapChunks = await requestFilesFromRepoMap(this.options.llm, this.options.config, this.options.ide, input, filterDirectory);
        }
        catch (error) {
        ...
        if (filterDirectory) {
            ...
            retrievalResults = retrievalResults.filter((chunk) => !!findUriInDirs(chunk.filepath, [filterDirectory]).foundInDir);
        }
        const deduplicatedRetrievalResults = deduplicateChunks(retrievalResults);
