...
describe("MCPConnection", () => {
    ...
    describe("resolveCwd", () => {
        ...
        it("should return absolute cwd unchanged", async () => {
            ...
            await expect(conn.resolveCwd("/tmp/project")).resolves.toBe("/tmp/project");
        });
        it("should resolve relative cwd using IDE workspace", async () => {
            ...
            await expect(conn.resolveCwd("src")).resolves.toBe("/workspace/src");
        ...
        it("should fall back to homedir for remote URIs that cannot be used as local cwd", async () => {
            const ide = {};
            vi.spyOn(ideUtils, "resolveRelativePathInDir").mockResolvedValue("vscode-remote://ssh-remote+192.168.137.2/home/user/project");
            const conn = new MCPConnection(baseOptions, { ide });
            const { homedir } = require("os");
            await expect(conn.resolveCwd("src")).resolves.toBe(homedir());
        });
    });
    ...
    describe("resolveCommandForPlatform", () => {
        ...
        it("should NOT wrap with cmd.exe when Windows host connects to WSL remote", async () => {
            ...
            try {
                const result = await conn.resolveCommandForPlatform("npx", [
                    "-y",
                    "test",
                ]);
        ...
        it("should NOT wrap non-batch commands even on Windows", async () => {
            ...
            Object.defineProperty(process, "platform", { value: "win32" });
            try {
                const result = await conn.resolveCommandForPlatform("python", [
                    "script.py",
                ]);
