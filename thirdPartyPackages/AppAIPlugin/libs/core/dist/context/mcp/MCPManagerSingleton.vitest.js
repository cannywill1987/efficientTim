...
describe("MCPManagerSingleton", () => {
    ...
    beforeEach(() => {
        // Reset singleton
        MCPManagerSingleton.instance = undefined;
        ...
        manager.createConnection = function (id, options) {
    ...
    describe("setConnections", () => {
        it("should add new connections", () => {
            ...
            expect(manager.getConnection(testOptions.id)).toBeInstanceOf(MCPConnection);
        });
    ...
    describe("refreshConnection", () => {
        it("should refresh specific connection", async () => {
            ...
            expect(connection.connectClient).toHaveBeenCalledWith(true, expect.any(AbortSignal));
        });
        it("should throw error for non-existent connection", async () => {
            await expect(manager.refreshConnection("non-existent")).rejects.toThrow("MCP Connection non-existent not found");
        });
    });
    describe("refreshConnections", () => {
        it("should refresh all connections", async () => {
            ...
            expect(connection1.connectClient).toHaveBeenCalledWith(true, expect.any(AbortSignal));
            expect(connection2.connectClient).toHaveBeenCalledWith(true, expect.any(AbortSignal));
        });
