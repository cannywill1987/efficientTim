...
import { decodeSecretLocation, getTemplateVariables, } from "@continuedev/config-yaml";
...
Org-level secrets can only be used for MCP by Background Agents (https://docs.continue.dev/hub/agents/overview) when \"Include in Env\" is enabled.`);
     ...
     */
    async resolveCommandForPlatform(originalCommand, originalArgs) {
        // Check if we're on Windows host connected to WSL remote
        const ideInfo = await this.extras?.ide?.getIdeInfo();
        const isWindowsHostWithWslRemote = process.platform === "win32" && ideInfo?.remoteName === "wsl";
    ...
    async constructStdioTransport(options) {
        ...
        const { command, args } = await this.resolveCommandForPlatform(options.command, options.args || []);
