import { claudeCodeLikeConfigFileSchema, claudeDesktopLikeConfigFileSchema, convertJsonMcpConfigToYamlMcpConfig, mcpServersJsonSchema, } from "@continuedev/config-yaml";
import * as JSONC from "comment-json";
import ignore from "ignore";
import { convertYamlMcpConfigToInternalMcpOptions } from "../../../config/yaml/yamlToContinueConfig";
import { DEFAULT_IGNORE_DIRS, DEFAULT_IGNORE_FILETYPES, } from "../../../indexing/ignore";
 ...
 */
export async function loadJsonMcpConfigs(ide, includeGlobal, globalRequestOptions = undefined) {
    const errors = [];
    // Get dirs
    const workspaceDirs = await ide.getWorkspaceDirs();
    const mcpDirs = workspaceDirs.map((dir) => joinPathsToUri(dir, ".continue", "mcpServers"));
    if (includeGlobal) {
    ...
    const overrideDefaultIgnores = ignore()
        .add(DEFAULT_IGNORE_FILETYPES.filter((val) => !["config.json", "settings.json"].includes(val)))
    ...
    // De-duplicate
    const deduplicatedJsonConfigs = deduplicateArray(validJsonConfigs, (a, b) => a.name === b.name);
    // Two levels of conversion for now.
    const yamlConfigs = deduplicatedJsonConfigs.map((c) => {
        const { warnings, yamlConfig } = convertJsonMcpConfigToYamlMcpConfig(c.name, c.mcpJson);
        return {
            warnings,
            yamlConfig: {
                ...yamlConfig,
    ...
    const mcpServers = yamlConfigs.map((c) => {
        ...
        return convertYamlMcpConfigToInternalMcpOptions(c.yamlConfig, globalRequestOptions);
    });
