...
const createServerForOAuth = () => http.createServer((req, res) => {
    try {
        if (!req.url) {
...
class MCPConnectionOauthProvider {
    ...
    async clientInformation() {
        ...
        return await OAuthClientInformationSchema.parseAsync(existingClientInformation);
    }
    async tokens() {
...
export async function getOauthToken(mcpServerUrl, ide) {
 ...
 */
export async function performAuth(serverId, url, ide) {
...
export function removeMCPAuth(url, ide) {
    const authProvider = new MCPConnectionOauthProvider(url, ide);
    authProvider.clear();
}
