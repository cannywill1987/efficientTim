...
describe("MCPOauth", () => {
    let globalContextFilePath;
    let mockIde;
    let mockMcpServerId;
    let mockMcpServerUrl;
    beforeEach(() => {
    ...
    describe("performAuth", () => {
        test("should call auth with correct parameters and return auth result", async () => {
            ...
            const result = await performAuth(mockMcpServerId, mockMcpServerUrl, mockIde);
            expect(mockAuth).toHaveBeenCalledWith(expect.any(Object), // MCPConnectionOauthProvider instance
            {
    ...
    describe("redirect URL handling", () => {
        test("should use getExternalUri when available for VS Code", async () => {
            const vscodeIde = {
                ...mockIde,
                getExternalUri: vi.fn((uri) => Promise.resolve("https://vscode.dev/redirect")),
            };
            const { auth } = await import("@modelcontextprotocol/sdk/client/auth.js");
            const mockAuth = vi.mocked(auth);
            mockAuth.mockResolvedValue("AUTHORIZED");
            await performAuth(mockMcpServerId, mockMcpServerUrl, vscodeIde);
            expect(vscodeIde.getExternalUri).toHaveBeenCalledWith("http://localhost:3000");
        });
        test("should fallback to localhost when getExternalUri is not available", async () => {
            ...
            const mockAuth = vi.mocked(auth);
            mockAuth.mockResolvedValue("AUTHORIZED");
            await performAuth(mockMcpServerId, mockMcpServerUrl, ideWithoutExternalUri);
    ...
    describe("error handling", () => {
        test("should clean up context on auth failure", async () => {
            ...
            await expect(performAuth(mockMcpServerId, mockMcpServerUrl, mockIde)).rejects.toThrow("Auth failed");
