...
describe("lineStream", () => {
    let mockFullStop;
    async function getLineGenerator(lines) {
    ...
    async function getFilteredLines(results) {
        const output = [];
    ...
    describe("lineIsRepeated", () => {
        it("should return true for similar lines", () => {
            expect(lineStream.lineIsRepeated("const x = 5;", "const x = 6;")).toBe(true);
        });
        it("should return false for different lines", () => {
            expect(lineStream.lineIsRepeated("const x = 5;", "let y = 10;")).toBe(false);
        });
    ...
    describe("stopAtSimilarLine", () => {
        it("should stop at the exact same line", async () => {
            ...
            const result = lineStream.stopAtSimilarLine(linesGenerator, lineToTest, mockFullStop);
            const filteredLines = await getFilteredLines(result);
        ...
        it("should stop at a similar line", async () => {
            ...
            const result = lineStream.stopAtSimilarLine(linesGenerator, "a" + lineToTest, mockFullStop);
            const filteredLines = await getFilteredLines(result);
            expect(filteredLines).toEqual(["console.log();", "const y = () => {};"]);
            expect(mockFullStop).toHaveBeenCalledTimes(1);
        });
        it("should continue on bracket ending lines", async () => {
            ...
            const result = lineStream.stopAtSimilarLine(linesGenerator, "}", mockFullStop);
            const filteredLines = await getFilteredLines(result);
            expect(filteredLines).toEqual([
                " if (x > 0) {",
                "   console.log(x);",
    ...
    describe("fixCodeLlamaFirstLineIndentation", () => {
        it("should fix indentation of the first line", async () => {
            ...
            const result = lineStream.fixCodeLlamaFirstLineIndentation(linesGenerator);
            const filteredLines = await getFilteredLines(result);
            expect(filteredLines).toEqual(["const x = 5;", "let y = 10;"]);
        });
    });
    describe("filterLeadingAndTrailingNewLineInsertion", () => {
        it("should ignore 'useless' new first lines", async () => {
            ...
            const result = lineStream.filterLeadingAndTrailingNewLineInsertion(linesGenerator);
            const filteredLines = await getFilteredLines(result);
    ...
    describe("stopAtRepeatingLines", () => {
        it("should handle non-repeating lines correctly", async () => {
            ...
            const result = lineStream.stopAtRepeatingLines(linesGenerator, mockFullStop);
            const filteredLines = await getFilteredLines(result);
            expect(filteredLines).toEqual([
                "const x = 5;",
                "let y = 10;",
    ...
    describe("hasNestedMarkdownBlocks", () => {
        it("should detect nested markdown blocks from first line", () => {
            expect(lineStream.hasNestedMarkdownBlocks("```markdown", undefined)).toBe(true);
            expect(lineStream.hasNestedMarkdownBlocks("```md", undefined)).toBe(true);
            expect(lineStream.hasNestedMarkdownBlocks("```gfm", undefined)).toBe(true);
        });
        it("should detect nested markdown blocks from filepath", () => {
            expect(lineStream.hasNestedMarkdownBlocks("```typescript", "README.md")).toBe(true);
            expect(lineStream.hasNestedMarkdownBlocks("```javascript", "docs.markdown")).toBe(true);
        });
        it("should return false for non-markdown scenarios", () => {
            expect(lineStream.hasNestedMarkdownBlocks("```typescript", "script.js")).toBe(false);
            expect(lineStream.hasNestedMarkdownBlocks("```python", undefined)).toBe(false);
            expect(lineStream.hasNestedMarkdownBlocks("const x = 5;", undefined)).toBe(false);
        });
        it("should handle complex first line scenarios", () => {
            expect(lineStream.hasNestedMarkdownBlocks("```markdown README.md", undefined)).toBe(true);
            expect(lineStream.hasNestedMarkdownBlocks("```md with extra text", undefined)).toBe(true);
        });
    });
    ...
    describe("shouldChangeLineAndStop", () => {
        ...
        it("should return partial line before [/CODE] when at start/after whitespace", () => {
            const result = lineStream.shouldChangeLineAndStop("some code [/CODE] more text");
        ...
        it("should NOT trigger on [/CODE] within quotes or non-whitespace preceded", () => {
            ...
            const result = lineStream.shouldChangeLineAndStop('console.log("test [/CODE] end");');
        ...
        it("should NOT trigger on [/CODE] in middle of identifier", () => {
            ...
            const result = lineStream.shouldChangeLineAndStop('const var[/CODE]Name = "test";');
            expect(result).toBeUndefined(); // Should not trigger stopping
        });
        ...
        it("should NOT handle ``` in middle of line", () => {
            const result = lineStream.shouldChangeLineAndStop('console.log("test ``` end");');
    ...
    describe("filterCodeBlockLines with optimized MarkdownBlockState", () => {
        ...
        it("should fall back to legacy behavior when no markdown nesting detected", async () => {
            ...
            const result = lineStream.filterCodeBlockLines(linesGenerator, "script.ts");
            const filteredLines = await getFilteredLines(result);
            expect(filteredLines).toEqual(["const x = 5;"]);
        });
    ...
    describe("validatePatternInLine", () => {
        it("should return false when pattern is not found", () => {
            const result = lineStream.validatePatternInLine("const x = 5;", "[/CODE]");
        ...
        it("should return false when pattern is preceded by non-whitespace", () => {
            const result = lineStream.validatePatternInLine('const var[/CODE]Name = "test";', "[/CODE]");
            expect(result.isValid).toBe(false);
            expect(result.patternIndex).toBe(9);
        });
        it("should return false when pattern is inside double quotes", () => {
            const result = lineStream.validatePatternInLine('console.log("test [/CODE] end");', "[/CODE]");
            expect(result.isValid).toBe(false);
            expect(result.patternIndex).toBe(18);
        });
        it("should return false when pattern is inside single quotes", () => {
            const result = lineStream.validatePatternInLine("const message = 'test [/CODE] end';", "[/CODE]");
            expect(result.isValid).toBe(false);
            expect(result.patternIndex).toBe(22);
        });
        it("should return true when pattern is properly separated by whitespace", () => {
            const result = lineStream.validatePatternInLine("some code [/CODE] more text", "[/CODE]");
        ...
        it("should handle complex quote scenarios correctly", () => {
            ...
            const result1 = lineStream.validatePatternInLine('const x = "unclosed quote [/CODE]', "[/CODE]");
            expect(result1.isValid).toBe(false);
            // Matched quotes should make it valid
            const result2 = lineStream.validatePatternInLine('const x = "closed"; [/CODE]', "[/CODE]");
            expect(result2.isValid).toBe(true);
        });
        it("should work with different patterns", () => {
            const result1 = lineStream.validatePatternInLine("# End of file. Done", "# End of file.");
