...
function isEnglishFirstLine(line) {
    ...
    if (line.endsWith(":") &&
        !CODE_KEYWORDS_ENDING_IN_SEMICOLON.some((keyword) => line.startsWith(keyword))) {
        return true;
    }
 ...
 */
export function validatePatternInLine(line, pattern) {
...
export function shouldChangeLineAndStop(line) {
    if (line.trimStart() === "```") {
        return line;
    }
 ...
 */
export function hasNestedMarkdownBlocks(firstLine, filepath) {
...
export function processBlockNesting(line, seenFirstFence) {
    return processBlockNestingUtil(line, seenFirstFence, shouldRemoveLineBeforeStart);
}
...
export async function* noTopLevelKeywordsMidline(lines, topLevelKeywords, fullStop) {
 ...
 */
export async function* avoidPathLine(stream, comment) {
 ...
 */
export async function* avoidEmptyComments(stream, comment) {
 ...
 */
export async function* streamWithNewLines(stream) {
    let firstLine = true;
 ...
 */
export function lineIsRepeated(a, b) {
 ...
 */
export async function* stopAtSimilarLine(stream, line, fullStop) {
 ...
 */
export async function* stopAtLines(stream, fullStop, linesToStopAt = LINES_TO_STOP_AT) {
    for await (const line of stream) {
...
export async function* stopAtLinesExact(stream, fullStop, linesToStopAt) {
    for await (const line of stream) {
 ...
 */
export async function* skipPrefixes(lines) {
 ...
 */
export async function* skipLines(stream) {
    for await (const line of stream) {
 ...
 */
export async function* removeTrailingWhitespace(stream) {
    for await (const line of stream) {
        yield line.trimEnd();
    }
}
 ...
 */
export async function* filterEnglishLinesAtStart(lines) {
    let i = 0;
 ...
 */
export async function* filterEnglishLinesAtEnd(lines) {
...
export async function* filterLeadingNewline(lines) {
    let firstLine = true;
    for await (const line of lines) {
 ...
 */
export async function* fixCodeLlamaFirstLineIndentation(lines) {
    let isFirstLine = true;
    for await (const line of lines) {
 ...
 */
export async function* filterLeadingAndTrailingNewLineInsertion(diffLines) {
    let isFirst = true;
    let buffer = [];
 ...
 * lines is yieled.
 */
export async function* stopAtRepeatingLines(lines, fullStop) {
    let previousLine;
    let repeatCount = 0;
    const MAX_REPEATS = 3;
    for await (const line of lines) {
 ...
 */
export async function* logLines(lines, prefix = "STREAMED LINES") {
    let linesToLog = [];
    for await (const line of lines) {
        yield line;
...
export async function* showWhateverWeHaveAtXMs(lines, ms) {
...
export async function* noDoubleNewLine(lines) {
    let isFirstLine = true;
    for await (const line of lines) {
