...
export function findEnclosingTypeDeclaration(sourceCode, cursorLine, cursorColumn, ast) {
...
export async function extractTopLevelDecls(currentFile, givenParser) {
    ...
    const query = await getQueryForFile(currentFile, `static-context-queries/relevant-headers-queries/${language}-get-toplevel-headers.scm`);
    if (!query) {
        throw new Error(`failed to get query for file ${currentFile} and language ${language}`);
    }
    return query.matches(ast.rootNode);
}
export async function extractTopLevelDeclsWithFormatting(currentFile, givenParser) {
    const ast = await getAst(currentFile, await fs.readFile(currentFile, "utf8"));
    if (!ast) {
        throw new Error(`failed to get ast for file ${currentFile}`);
    }
...
export function extractFunctionTypeFromDecl(match) {
...
export function unwrapToBaseType(node) {
    if ([
        "function_type",
        "tuple_type",
        "type_identifier",
