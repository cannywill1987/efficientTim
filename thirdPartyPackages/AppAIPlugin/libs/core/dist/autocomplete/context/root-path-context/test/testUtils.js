...
function splitTextAtPosition(fileContent, position) {
...
export async function testRootPathContext(folderName, relativeFilepath, position, expectedDefinitionPositions) {
    ...
    const folderPath = path.join(process.cwd(), "autocomplete", "context", "root-path-context", "test", folderName);
    ...
    expectedDefinitionPositions.forEach((position, index) => {
        expect(getSnippetsMock).toHaveBeenNthCalledWith(index + 1, expect.any(String), // filepath argument
