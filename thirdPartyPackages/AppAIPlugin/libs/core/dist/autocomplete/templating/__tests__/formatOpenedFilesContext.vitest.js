...
describe("formatOpenedFilesContext main function tests", () => {
    ...
    const createCodeSnippet = (filepath, content) => ({
        type: AutocompleteSnippetType.Code,
        filepath,
        content,
    });
    const createDiffSnippet = (content) => ({
    ...
    test("should handle limited token count", () => {
        ...
        const result = formatOpenedFilesContext(snippets, 1, // Extremely small token count
        mockHelper, [], TOKEN_BUFFER);
...
describe("rankByScore function", () => {
    ...
    test("should rank snippets by score", () => {
        ...
        expect(result.map((s) => s.filepath)).not.toEqual(snippets.map((s) => s.filepath));
    });
