...
export const getSnippets = (helper, payload) => {
    const snippets = {
    ...
    for (const { key } of snippetOrder) {
        ...
        if (key === "recentlyOpenedFiles" && helper.options.useRecentlyOpened) {
            // Custom trimming
            const processedSnippets = formatOpenedFilesContext(payload.recentlyOpenedFileSnippets, remainingTokenCount, helper, finalSnippets, TOKEN_BUFFER);
            ...
            const snippetsToProcess = snippets[key].filter((snippet) => snippet.type !== AutocompleteSnippetType.Code ||
