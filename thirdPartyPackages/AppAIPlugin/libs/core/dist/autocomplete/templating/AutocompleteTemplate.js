...
import { AutocompleteSnippetType, } from "../snippets/types.js";
...
const qwenMultifileFimTemplate = {
    compilePrefixSuffix: (prefix, suffix, filepath, reponame, snippets, workspaceUris) => {
        ...
        const relativePaths = getShortestUniqueRelativeUriPaths([
            ...snippets.map((snippet) => "filepath" in snippet ? snippet.filepath : "Untitled.txt"),
            filepath,
        ], workspaceUris);
        const fileContents = snippets
            ...
            .join("\n");
        const fullPrefix = `<|repo_name|>${reponame}\n${fileContents}\n<|file_sep|>${getFileName(relativePaths[relativePaths.length - 1])}<|system_separator_istruction_repository_level|>${prefix}`;
        return [fullPrefix, suffix];
    },
    template: (prefix, suffix) => {
        if (prefix.includes("<|system_separator_istruction_repository_level|>")) {
            const [beforeSeparator, ...afterSeparator] = prefix.split("<|system_separator_istruction_repository_level|>");
...
const codegeexFimTemplate = {
    template: (prefix, suffix, filepath, reponame, language, allSnippets, workspaceUris) => {
        const snippets = allSnippets.filter((snippet) => snippet.type === AutocompleteSnippetType.Code);
        const relativePaths = getShortestUniqueRelativeUriPaths([...snippets.map((snippet) => snippet.filepath), filepath], workspaceUris);
...
export function getTemplateForModel(model) {
