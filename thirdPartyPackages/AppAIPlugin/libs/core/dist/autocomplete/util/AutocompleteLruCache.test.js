...
describe("AutocompleteLruCache", () => {
    ...
    beforeEach(() => {
        ...
        mockDb = createMockDb();
        cache = new AutocompleteLruCache(mockDb);
    ...
    afterEach(async () => {
        ...
        if (cache.flushTimer) {
            clearInterval(cache.flushTimer);
        }
    ...
    describe("Basic Cache Operations", () => {
        ...
        describe("get() method", () => {
            ...
            it("should update timestamp when entry is accessed", async () => {
                await cache.put("key", "value");
                const originalTimestamp = cache.cache.get("key").timestamp;
                currentTime += 10000;
                await cache.get("key");
                const updatedTimestamp = cache.cache.get("key").timestamp;
            ...
            it("should mark entry as dirty after updating timestamp", async () => {
                ...
                cache.dirty.clear();
                await cache.get("key");
                expect(cache.dirty.has("key")).toBe(true);
            });
    ...
    describe("LRU Eviction", () => {
        beforeEach(() => {
            AutocompleteLruCache.capacity = 3;
        });
        ...
        it("should handle capacity of 1", async () => {
            AutocompleteLruCache.capacity = 1;
            await cache.put("a", "1");
            await cache.put("b", "2");
    ...
    describe("Database Persistence", () => {
        describe("flush() method", () => {
            ...
            it("should perform upsert for existing cache entries", async () => {
                ...
                await cache.flush();
                expect(mockDb.run).toHaveBeenCalledWith(expect.stringContaining("INSERT INTO cache"), "foo", "bar", currentTime, "bar", currentTime);
            });
            it("should delete entries removed from cache", async () => {
                await cache.put("temp", "value");
                cache.cache.delete("temp");
                cache.dirty.add("temp");
                await cache.flush();
                expect(mockDb.run).toHaveBeenCalledWith("DELETE FROM cache WHERE key = ?", "temp");
            });
            ...
            it("should log error when flush fails", async () => {
                ...
                await cache.flush();
                expect(consoleError).toHaveBeenCalledWith("Error flushing cache:", dbError);
            ...
            it("should release mutex even if error occurs", async () => {
                ...
                const releaseSpy = jest.fn();
                jest
                    .spyOn(cache.mutex, "acquire")
                    .mockResolvedValue(releaseSpy);
                await cache.put("key", "value");
                const originalRun = mockDb.run;
                mockDb.run = jest.fn().mockImplementation((sql) => {
            ...
            it("should handle multiple dirty entries in one flush", async () => {
                ...
                await cache.flush();
                const insertCalls = mockDb.run.mock.calls.filter((call) => call[0].includes("INSERT INTO cache"));
        ...
        describe("loadFromDb() method", () => {
            ...
            it("should query with correct SQL", async () => {
                await cache.loadFromDb();
                expect(mockDb.all).toHaveBeenCalledWith("SELECT key, value, timestamp FROM cache");
            });
        });
    });
    // ═══════════════════════════════════════════════════════════════
    ...
    describe("Automatic Flush Timer", () => {
        it("should start timer on startFlushTimer()", () => {
            cache.startFlushTimer();
            expect(cache.flushTimer).toBeDefined();
        });
        it("should call flush at configured intervals", async () => {
            AutocompleteLruCache.flushInterval = 1000;
        ...
        it("should handle flush errors gracefully", async () => {
            ...
            expect(consoleError).toHaveBeenCalledWith("Error flushing cache:", flushError);
            consoleError.mockRestore();
        });
    });
    // ═══════════════════════════════════════════════════════════════
    ...
    describe("Resource Cleanup", () => {
        it("should clear timer on close()", async () => {
            cache.startFlushTimer();
            const timerId = cache.flushTimer;
        ...
        it("should flush pending changes on close()", async () => {
            await cache.put("key", "value");
            await cache.close();
            expect(mockDb.run).toHaveBeenCalledWith(expect.stringContaining("INSERT INTO cache"), "key", "value", currentTime, "value", currentTime);
        });
