...
describe("ASSISTANTS getAllDotContinueDefinitionFiles with fileExtType option", () => {
    ...
    it("should return only YAML files when fileExtType is 'yaml'", async () => {
        ...
        const result = await getAllDotContinueDefinitionFiles(testIde, options, "assistants");
        expect(result).toHaveLength(4);
        expect(result.map((f) => f.path.split("/").pop())).toEqual(expect.arrayContaining([
            "assistant1.yaml",
            "assistant2.yml",
            "config.yaml",
            "config.yml",
        ]));
        expect(result.map((f) => f.path.split("/").pop())).not.toContain("assistant3.md");
    });
    it("should return only Markdown files when fileExtType is 'markdown'", async () => {
        ...
        expect(result.map((f) => f.path.split("/").pop())).not.toContain("assistant1.yaml");
        expect(result.map((f) => f.path.split("/").pop())).not.toContain("assistant2.yml");
        expect(result.map((f) => f.path.split("/").pop())).not.toContain("config.yml");
        expect(result.map((f) => f.path.split("/").pop())).not.toContain("config.yaml");
    });
    it("should return all supported files when fileExtType is not specified", async () => {
        ...
        expect(result.map((f) => f.path.split("/").pop())).not.toContain("assistant4.txt");
    });
    it("should respect includeWorkspace option with fileExtType", async () => {
        // Test with includeWorkspace: false
        const workspaceOffOptions = {
            includeGlobal: false,
            includeWorkspace: false,
            fileExtType: "yaml",
        };
        const noWorkspaceResult = await getAllDotContinueDefinitionFiles(testIde, workspaceOffOptions, "assistants");
        expect(noWorkspaceResult).toHaveLength(0);
        // Test with includeWorkspace: true
        const workspaceOnOptions = {
            includeGlobal: false,
            includeWorkspace: true,
            fileExtType: "yaml",
        };
        const workspaceResult = await getAllDotContinueDefinitionFiles(testIde, workspaceOnOptions, "assistants");
        expect(workspaceResult).toHaveLength(4);
        expect(workspaceResult.map((f) => f.path.split("/").pop())).toEqual(expect.arrayContaining([
            "assistant1.yaml",
            "assistant2.yml",
            "config.yaml",
            "config.yml",
    ...
    it("should filter by file extension case sensitively", async () => {
        ...
        const yamlResult = await getAllDotContinueDefinitionFiles(testIde, yamlOptions, "assistants");
        // Should only get lowercase extensions (current implementation)
        expect(yamlResult).toHaveLength(4);
        expect(yamlResult.map((f) => f.path.split("/").pop())).toEqual(expect.arrayContaining([
            "assistant1.yaml",
            "assistant2.yml",
            "config.yaml",
            "config.yml",
        ]));
        expect(yamlResult.map((f) => f.path.split("/").pop())).not.toContain("assistant5.YAML");
        const markdownOptions = {
            includeGlobal: false,
            includeWorkspace: true,
            fileExtType: "markdown",
        };
        const markdownResult = await getAllDotContinueDefinitionFiles(testIde, markdownOptions, "assistants");
        expect(markdownResult).toHaveLength(1);
        expect(markdownResult.map((f) => f.path.split("/").pop())).toEqual([
            "assistant3.md",
        ]);
        expect(markdownResult.map((f) => f.path.split("/").pop())).not.toContain("assistant7.MD");
    });
});
describe("AGENTS getAllDotContinueDefinitionFiles with fileExtType option", () => {
    ...
    it("should return only YAML files when fileExtType is 'yaml'", async () => {
        ...
        const result = await getAllDotContinueDefinitionFiles(testIde, options, "agents");
        expect(result).toHaveLength(2);
        expect(result.map((f) => f.path.split("/").pop())).toEqual(expect.arrayContaining(["agent1.yaml", "agent2.yml"]));
        expect(result.map((f) => f.path.split("/").pop())).not.toContain("agent3.md");
    });
    it("should return only Markdown files when fileExtType is 'markdown'", async () => {
        const options = {
            includeGlobal: false,
        ...
        expect(result.map((f) => f.path.split("/").pop())).not.toContain("agent1.yaml");
        expect(result.map((f) => f.path.split("/").pop())).not.toContain("agent2.yml");
    });
    it("should return all supported files when fileExtType is not specified", async () => {
        const options = {
            includeGlobal: false,
        ...
        expect(result.map((f) => f.path.split("/").pop())).toEqual(expect.arrayContaining(["agent1.yaml", "agent2.yml", "agent3.md"]));
        // Should not include .txt files
        expect(result.map((f) => f.path.split("/").pop())).not.toContain("agent4.txt");
    });
    it("should respect includeWorkspace option with fileExtType", async () => {
        // Test with includeWorkspace: false
        const workspaceOffOptions = {
        ...
        const noWorkspaceResult = await getAllDotContinueDefinitionFiles(testIde, workspaceOffOptions, "agents");
        expect(noWorkspaceResult).toHaveLength(0);
        // Test with includeWorkspace: true
        const workspaceOnOptions = {
            includeGlobal: false,
        ...
        const workspaceResult = await getAllDotContinueDefinitionFiles(testIde, workspaceOnOptions, "agents");
        expect(workspaceResult).toHaveLength(2);
        expect(workspaceResult.map((f) => f.path.split("/").pop())).toEqual(expect.arrayContaining(["agent1.yaml", "agent2.yml"]));
    });
    it("should return empty array when no files match the specified extension type", async () => {
        // Create a test directory with only non-matching files
        tearDownTestDir();
    ...
    it("should filter by file extension case sensitively", async () => {
        ...
        const yamlResult = await getAllDotContinueDefinitionFiles(testIde, yamlOptions, "agents");
        // Should only get lowercase extensions (current implementation)
        expect(yamlResult).toHaveLength(2);
        expect(yamlResult.map((f) => f.path.split("/").pop())).toEqual(expect.arrayContaining(["agent1.yaml", "agent2.yml"]));
        expect(yamlResult.map((f) => f.path.split("/").pop())).not.toContain("agent5.YAML");
        const markdownOptions = {
            includeGlobal: false,
            includeWorkspace: true,
            fileExtType: "markdown",
        };
        const markdownResult = await getAllDotContinueDefinitionFiles(testIde, markdownOptions, "agents");
        expect(markdownResult).toHaveLength(1);
        expect(markdownResult.map((f) => f.path.split("/").pop())).toEqual([
            "agent3.md",
        ]);
        expect(markdownResult.map((f) => f.path.split("/").pop())).not.toContain("agent7.MD");
    });
});
