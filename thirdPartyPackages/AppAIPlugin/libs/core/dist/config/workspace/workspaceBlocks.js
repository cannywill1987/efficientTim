import { createPromptMarkdown, createRuleMarkdown, sanitizeRuleName, } from "@continuedev/config-yaml";
...
export function getFileContent(blockType) {
    if (blockType === "rules") {
        ...
        return createPromptMarkdown("New prompt", "Please write a thorough suite of unit tests for this code, making sure to cover all relevant edge cases", {
            description: "New prompt",
            invokable: true,
        });
    }
...
export async function findAvailableFilename(baseDirUri, blockType, fileExists, extension, isGlobal, baseFilenameOverride) {
    ...
    do {
        ...
        fileUri = joinPathsToUri(baseDirUri, `${baseFilename}${suffix}.${fileExtension}`);
        counter++;
    } while (await fileExists(fileUri));
    return fileUri;
}
export async function createNewWorkspaceBlockFile(ide, blockType, baseFilename) {
...
export async function createNewGlobalRuleFile(ide, baseFilename) {
    try {
