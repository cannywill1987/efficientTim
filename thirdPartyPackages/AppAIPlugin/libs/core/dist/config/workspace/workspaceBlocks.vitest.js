import { RULE_FILE_EXTENSION } from "@continuedev/config-yaml";
...
describe("findAvailableFilename", () => {
    test("returns base filename when it doesn't exist", async () => {
        const mockFileExists = async (uri) => false;
        const result = await findAvailableFilename("/workspace/.continue/models", "models", mockFileExists);
        expect(result).toBe("/workspace/.continue/models/new-model.yaml");
    });
    test("returns filename with counter when base exists", async () => {
        const mockFileExists = async (uri) => {
    ...
    test("increments counter until available filename is found", async () => {
        ...
        const result = await findAvailableFilename("/workspace/.continue/context", "context", mockFileExists);
    ...
    test("handles different block types correctly with proper extensions", async () => {
        ...
        for (const { blockType, expected } of testCases) {
            const result = await findAvailableFilename("/test", blockType, mockFileExists);
            expect(result).toBe(expected);
        }
    });
    test("respects custom extension parameter", async () => {
        const mockFileExists = async (uri) => false;
        const result = await findAvailableFilename("/test", "models", mockFileExists, "json");
    ...
    test("handles rules markdown files with counter", async () => {
        ...
        const result = await findAvailableFilename("/workspace/.continue/rules", "rules", mockFileExists);
        expect(result).toBe(`/workspace/.continue/rules/new-rule-2.${RULE_FILE_EXTENSION}`);
    });
    test("handles large counter values", async () => {
        const existingFiles = new Set(Array.from({ length: 100 }, (_, i) => i === 0
            ? "/workspace/.continue/prompts/new-prompt.md"
            : `/workspace/.continue/prompts/new-prompt-${i}.md`));
        const mockFileExists = async (uri) => {
            return existingFiles.has(uri);
        };
        const result = await findAvailableFilename("/workspace/.continue/prompts", "prompts", mockFileExists);
