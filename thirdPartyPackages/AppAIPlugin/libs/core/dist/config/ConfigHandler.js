...
import { AuthType, } from "../control-plane/AuthTypes.js";
import { getControlPlaneEnv } from "../control-plane/env.js";
import { PolicySingleton } from "../control-plane/PolicySingleton.js";
import { Logger } from "../util/Logger.js";
import { Telemetry } from "../util/posthog.js";
import { getAllDotContinueDefinitionFiles, } from "./loadLocalAssistants.js";
import LocalProfileLoader from "./profile/LocalProfileLoader.js";
import PlatformProfileLoader from "./profile/PlatformProfileLoader.js";
import { ProfileLifecycleManager, } from "./ProfileLifecycleManager.js";
export class ConfigHandler {
    ...
    constructor(ide, llmLogger, initialSessionInfoPromise) {
        ...
        this.globalLocalProfileManager = new ProfileLifecycleManager(new LocalProfileLoader(ide, this.controlPlaneClient, this.llmLogger), this.ide);
    ...
    async cascadeInit(reason, isLogin) {
        ...
        this.globalLocalProfileManager = new ProfileLifecycleManager(new LocalProfileLoader(this.ide, this.controlPlaneClient, this.llmLogger), this.ide);
        try {
    ...
    async getLocalProfiles(options) {
        ...
        if (options.includeWorkspace) {
            const yamlOptions = { ...options, fileExtType: "yaml" };
            ...
            const profiles = allFiles.map((assistant) => {
                return new LocalProfileLoader(this.ide, this.controlPlaneClient, this.llmLogger, assistant);
            });
            const localAssistantProfiles = profiles.map((profile) => new ProfileLifecycleManager(profile, this.ide));
    ...
    async updateControlPlaneSessionInfo(sessionInfo) {
        ...
        if (newSession) {
            if (currentSession) {
                if (newSession.AUTH_TYPE !== AuthType.OnPrem &&
        ...
        if (reload) {
            this.controlPlaneClient = new ControlPlaneClient(Promise.resolve(sessionInfo), this.ide);
            this.abortCascade();
