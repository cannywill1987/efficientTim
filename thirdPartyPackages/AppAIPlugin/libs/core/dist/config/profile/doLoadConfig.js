/**
 * 文件类型：配置加载
 * 文件作用：根据 YAML/JSON 配置生成最终运行时配置。
 * 主要职责：合并配置、注入工具与上下文提供者、初始化遥测。
 */
import fs from "fs";
...
export default async function doLoadConfig(options) {
    ...
    const configYamlPath = localPathOrUriToPath(overrideConfigYamlByPath || getConfigYamlPath(ideInfo.ideType));
    let newConfig;
    let errors;
    let configLoadInterrupted = false;
    let configName;
    const hasPreReadContent = packageIdentifier.uriType === "file" &&
    ...
    if (overrideConfigYaml ||
        ...
        const result = await loadContinueConfigFromJson(ide, ideSettings, ideInfo, uniqueId, llmLogger, workOsAccessToken, overrideConfigJson);
        newConfig = result.config;
        errors = result.errors;
        configLoadInterrupted = result.configLoadInterrupted;
    }
    ...
    newConfig.tools.push(...(await getConfigDependentToolDefinitions({
    ...
    await SentryLogger.setup(newConfig.allowAnonymousTelemetry ?? false, await ide.getUniqueId(), ideInfo, userEmail);
