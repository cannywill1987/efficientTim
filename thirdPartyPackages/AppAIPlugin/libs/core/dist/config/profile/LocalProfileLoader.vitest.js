...
vi.mock("./doLoadConfig.js", () => ({
    default: (...args) => mockDoLoadConfig(...args),
}));
describe("LocalProfileLoader", () => {
    const controlPlaneClient = new ControlPlaneClient(Promise.resolve(undefined), testIde);
    ...
    it("should pass pre-read content in packageIdentifier for override files", async () => {
        ...
        const loader = new LocalProfileLoader(testIde, controlPlaneClient, llmLogger, overrideFile);
        await loader.doLoadConfig();
        expect(mockDoLoadConfig).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it("should not include content in packageIdentifier when no override file", async () => {
        const loader = new LocalProfileLoader(testIde, controlPlaneClient, llmLogger);
        await loader.doLoadConfig();
        expect(mockDoLoadConfig).toHaveBeenCalledWith(expect.objectContaining({
            packageIdentifier: expect.objectContaining({
                uriType: "file",
