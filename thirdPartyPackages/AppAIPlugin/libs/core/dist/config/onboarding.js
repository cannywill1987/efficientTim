 ...
 */
export function setupBestConfig(config) {
    return {
        ...config,
        models: config.models,
    };
}
export function setupLocalConfig(config) {
    return {
        ...config,
        models: [
            {
...
export function setupQuickstartConfig(config) {
    return config;
}
export function setupProviderConfig(config, provider, apiKey) {
    let newModels;
    switch (provider) {
        case "openai":
    ...
    const modelsToAdd = newModels.filter((n) => !existingModels.some((m) => "uses" in m && m.uses === n.uses));
