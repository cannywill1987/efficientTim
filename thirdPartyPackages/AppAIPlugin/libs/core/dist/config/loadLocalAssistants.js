...
export function isContinueConfigRelatedUri(uri) {
    return (uri.endsWith(".continuerc.json") ||
        ...
        (uri.includes(".continue") &&
            (uri.endsWith(".yaml") ||
                uri.endsWith(".yml") ||
                uri.endsWith(".json"))) ||
        [...BLOCK_TYPES, "agents", "assistants", "configs"].some((blockType) => uri.includes(`.continue/${blockType}`)));
}
export function isContinueAgentConfigFile(uri) {
...
export function isColocatedRulesFile(uri) {
...
async function getDefinitionFilesInDir(ide, dir, fileExtType) {
    try {
        ...
        const overrideDefaultIgnores = ignore()
            .add(DEFAULT_IGNORE_FILETYPES.filter((t) => t !== "config.yaml" && t !== "config.yml"))
...
export function getDotContinueSubDirs(ide, options, workspaceDirs, subDirName) {
    let fullDirs = [];
    // Workspace .continue/<subDirName>
    if (options.includeWorkspace) {
        fullDirs = workspaceDirs.map((dir) => joinPathsToUri(dir, ".continue", subDirName));
    }
    // ~/.continue/<subDirName>
 ...
 */
export async function getAllDotContinueDefinitionFiles(ide, options, subDirName) {
