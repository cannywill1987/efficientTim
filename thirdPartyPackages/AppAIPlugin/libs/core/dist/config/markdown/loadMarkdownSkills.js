 ...
 */
async function getClaudeSkillsDir(ide) {
    const fullDirs = (await ide.getWorkspaceDirs()).map((dir) => joinPathsToUri(dir, ".claude", SKILLS_DIR));
    fullDirs.push(localPathToUri(getGlobalFolderWithName(SKILLS_DIR)));
    return (await Promise.all(fullDirs.map(async (dir) => {
...
export async function loadMarkdownSkills(ide) {
    const errors = [];
    const skills = [];
    try {
        const yamlAndMarkdownFileUris = [
            ...(await getAllDotContinueDefinitionFiles(ide, {
                includeGlobal: true,
                includeWorkspace: true,
                fileExtType: "markdown",
            }, SKILLS_DIR)).map((file) => file.path),
            ...(await getClaudeSkillsDir(ide)),
        ];
        const skillFiles = yamlAndMarkdownFileUris.filter((path) => path.endsWith("SKILL.md"));
        ...
        for (const fileUri of skillFiles) {
            try {
                ...
                const filesInSkillsDirectory = (await walkDir(fileUri.substring(0, fileUri.lastIndexOf("/")), ide, {
