...
describe("loadCodebaseRules", () => {
    ...
    const mockRuleContent = {
        "file:///workspace/src/rules.md": "# General Rules\nFollow coding standards",
        "file:///workspace/src/redux/rules.md": '---\nglobs: "**/*.{ts,tsx}"\n---\n# Redux Rules\nUse Redux Toolkit',
        "file:///workspace/src/components/rules.md": '---\nglobs: ["**/*.tsx", "**/*.jsx"]\n---\n# Component Rules\nUse functional components',
        "file:///workspace/.continue/rules.md": "# Global Rules\nFollow project guidelines",
    };
    ...
    beforeEach(() => {
        ...
        mockIde.getWorkspaceDirs.mockResolvedValue(["file:///workspace"]);
        // Mock readFile to return content based on path
        mockIde.readFile.mockImplementation((path) => {
            return Promise.resolve(mockRuleContent[path] || "");
        });
        // Mock markdownToRule to return converted rules
        markdownToRule.mockImplementation((content, options) => {
    ...
    it("should load rules from all rules.md files in the workspace", async () => {
        ...
        expect(mockIde.readFile).toHaveBeenCalledWith("file:///workspace/src/rules.md");
        expect(mockIde.readFile).toHaveBeenCalledWith("file:///workspace/src/redux/rules.md");
        expect(mockIde.readFile).toHaveBeenCalledWith("file:///workspace/src/components/rules.md");
        expect(mockIde.readFile).toHaveBeenCalledWith("file:///workspace/.continue/rules.md");
        // Should convert all rules
        expect(markdownToRule).toHaveBeenCalledTimes(4);
        // Should return all rules
        expect(rules).toHaveLength(4);
        expect(rules).toContainEqual(mockConvertedRules["file:///workspace/src/rules.md"]);
        expect(rules).toContainEqual(mockConvertedRules["file:///workspace/src/redux/rules.md"]);
        expect(rules).toContainEqual(mockConvertedRules["file:///workspace/src/components/rules.md"]);
        expect(rules).toContainEqual(mockConvertedRules["file:///workspace/.continue/rules.md"]);
    ...
    it("should handle errors when reading a rule file", async () => {
        ...
        expect(errors[0].message).toContain("Failed to parse colocated rule file file:///workspace/src/redux/rules.md: Failed to read file");
    });
    it("should handle errors when walkDirs fails", async () => {
        // Setup mock to throw
        walkDirs.mockRejectedValue(new Error("Failed to walk directories"));
        const { rules, errors } = await loadCodebaseRules(mockIde);
