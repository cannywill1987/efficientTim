...
describe("Rule Colocation Application", () => {
    ...
    let userMessageWithComponentFile;
    let userMessageWithReduxFile;
    let userMessageWithRootFile;
    let userMessageWithApiUtilFile;
    let userMessageWithComponentJsxFile;
    let componentTsxContextItem;
    let componentJsxContextItem;
    let reduxContextItem;
    let rootContextItem;
    let apiUtilContextItem;
    let otherUtilContextItem;
    beforeEach(() => {
        ...
        userMessageWithApiUtilFile = {
            role: "user",
            content: "Can you help with this API utility file?\n```ts src/utils/api/requests.ts\nexport const fetchData = () => {...}\n```",
        };
        // Setup context items
        componentTsxContextItem = {
            id: { providerTitle: "file", itemId: "context1" },
        ...
        componentJsxContextItem = {
            id: { providerTitle: "file", itemId: "context1b" },
        ...
        reduxContextItem = {
            id: { providerTitle: "file", itemId: "context2" },
        ...
        rootContextItem = {
            id: { providerTitle: "file", itemId: "context3" },
        ...
        apiUtilContextItem = {
            id: { providerTitle: "file", itemId: "context4" },
        ...
        otherUtilContextItem = {
            id: { providerTitle: "file", itemId: "context5" },
    ...
    describe("Basic directory-specific rule application", () => {
        it("should apply root rules to all files", () => {
            ...
            let applicableRules = getApplicableRules(userMessageWithComponentFile, rules, [componentTsxContextItem]);
    ...
    describe("Directory-specific rule application with implied globs", () => {
        it("should only apply component rules to files in the component directory when no globs specified", () => {
            ...
            let applicableRules = getApplicableRules(userMessageWithComponentFile, [impliedComponentRule], [componentTsxContextItem]);
            expect(applicableRules.map((r) => r.name)).toContain("Implied Components Rule");
            // Test with redux file - should NOT apply the component rule
            // This is failing currently - we need to fix the implementation
            applicableRules = getApplicableRules(userMessageWithReduxFile, [impliedComponentRule], [reduxContextItem]);
            // THIS WILL FAIL - Current implementation doesn't restrict by directory
            expect(applicableRules.map((r) => r.name)).not.toContain("Implied Components Rule");
            // Test with root-level file - should NOT apply the component rule
            // This is failing currently - we need to fix the implementation
            applicableRules = getApplicableRules(userMessageWithRootFile, [impliedComponentRule], [rootContextItem]);
            // THIS WILL FAIL - Current implementation doesn't restrict by directory
            expect(applicableRules.map((r) => r.name)).not.toContain("Implied Components Rule");
        });
    });
    describe("Combined directory and glob pattern matching", () => {
        it("should respect directory + glob pattern when both are present", () => {
            ...
            let applicableRules = getApplicableRules(userMessageWithComponentFile, [typescriptComponentRule], [componentTsxContextItem]);
            expect(applicableRules.map((r) => r.name)).toContain("TypeScript Component Rule");
            ...
            applicableRules = getApplicableRules(userMessageWithComponentJsxFile, [typescriptComponentRule], [componentJsxContextItem]);
            expect(applicableRules.map((r) => r.name)).not.toContain("TypeScript Component Rule");
            ...
            applicableRules = getApplicableRules(userMessageWithReduxFile, [typescriptComponentRule], [reduxContextItem]);
    ...
    describe("Nested directory rules with globs", () => {
        it("should apply API utils rule only to files in that directory matching the glob", () => {
            ...
            let applicableRules = getApplicableRules(userMessageWithApiUtilFile, [apiUtilsRule], [apiUtilContextItem]);
            ...
            // This test will fail because current implementation doesn't consider directory boundaries
            applicableRules = getApplicableRules(userMessageWithRootFile, [apiUtilsRule], [rootContextItem]);
            // THIS WILL FAIL - Current impl only checks file extension, not directory
            expect(applicableRules.map((r) => r.name)).not.toContain("API Utils Rule");
        });
    });
    describe("Rule application inference strategy", () => {
        it("should infer directory-specific glob patterns from rule file location", () => {
            ...
            function createRuleWithAutomaticGlobInference(sourceFilePath) {
            ...
            const modelsRule = createRuleWithAutomaticGlobInference("src/models/rules.md");
            const servicesRule = createRuleWithAutomaticGlobInference("src/services/rules.md");
            // Create context items for different files
            const modelFileContext = {
                id: { providerTitle: "file", itemId: "models1" },
                uri: { type: "file", value: "src/models/user.ts" },
                content: "export interface User {...}",
                name: "user.ts",
                description: "User model",
            };
            const serviceFileContext = {
                id: { providerTitle: "file", itemId: "services1" },
            ...
            const applicableModelsRules = getApplicableRules(undefined, // No user message needed
            [modelsRule, servicesRule], [modelFileContext]);
            // These assertions will fail with current implementation
            // but represent the desired behavior
            expect(applicableModelsRules.map((r) => r.name)).toContain("Inferred Rule for src/models");
            expect(applicableModelsRules.map((r) => r.name)).not.toContain("Inferred Rule for src/services");
            // Test with service file - should apply only the services rule
            const applicableServicesRules = getApplicableRules(undefined, // No user message needed
            [modelsRule, servicesRule], [serviceFileContext]);
            // These assertions will fail with current implementation
            // but represent the desired behavior
            expect(applicableServicesRules.map((r) => r.name)).not.toContain("Inferred Rule for src/models");
            expect(applicableServicesRules.map((r) => r.name)).toContain("Inferred Rule for src/services");
        });
    });
});
