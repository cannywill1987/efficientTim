...
export class CodebaseRulesCache {
    ...
    async update(ide, uri) {
        ...
        const { relativePathOrBasename, foundInDir } = findUriInDirs(uri, workspaceDirs);
        if (!foundInDir) {
 ...
 */
export async function loadCodebaseRules(ide) {
    const errors = [];
    const rules = [];
    try {
        ...
        for (const filePath of rulesMdFiles) {
            try {
                ...
                const { relativePathOrBasename, foundInDir, uri } = findUriInDirs(filePath, await ide.getWorkspaceDirs());
                if (foundInDir) {
