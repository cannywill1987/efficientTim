...
vi.mock("../context/providers/ProblemsContextProvider", () => {
    ...
    MockProblemsContextProvider.description = { title: "problems" };
...
vi.mock("../context/providers/TerminalContextProvider", () => {
    ...
    MockTerminalContextProvider.description = { title: "terminal" };
    return { default: MockTerminalContextProvider };
});
const mockedContextProviderClassFromName = contextProviderClassFromName;
beforeEach(() => {
    vi.clearAllMocks();
});
describe("loadConfigContextProviders", () => {
    describe("with empty or undefined config", () => {
        it("should return only default providers when config is undefined", () => {
            ...
            expect(providerTitles).toEqual(expect.arrayContaining([
                "file",
                "current-file",
                "diff",
                "terminal",
        ...
        it("should exclude problems and terminal in jetbrains", () => {
            ...
            const providerTitles = result.providers.map((p) => p.description.title);
            expect(providerTitles).toEqual(expect.arrayContaining(["file", "current-file", "diff", "rules"]));
        });
    });
...
describe("with valid config", () => {
    it("should load providers from config with correct parameters", () => {
        ...
        expect(result.errors).toEqual([]);
        expect(mockedContextProviderClassFromName).toHaveBeenCalledWith("custom-provider");
        ...
        expect(result.providers).toHaveLength(7);
        const customProvider = result.providers.find((p) => p.description.title === "custom-provider");
        expect(customProvider).toBeDefined();
        expect(customProvider?.options).toEqual({
            name: "my-custom-provider",
            apiKey: "test-key",
            baseUrl: "https://api.example.com",
        });
...
describe("error handling", () => {
    it("should add error for unknown provider", () => {
        ...
        expect(result.providers).toHaveLength(6);
        expect(mockedContextProviderClassFromName).toHaveBeenCalledWith("unknown-provider");
    });
...
describe("default provider merging", () => {
    it("should not duplicate providers when config matches defaults", () => {
        ...
        expect(result.providers).toHaveLength(6);
        const fileProviders = result.providers.filter((p) => p.description.title === "file");
...
describe("edge cases", () => {
    it("should handle provider constructor throwing error", () => {
        ...
        expect(() => loadConfigContextProviders(config, false, "vscode")).toThrow("Provider construction failed");
    });
    ...
    it("should handle provider with same title as default but different implementation", () => {
        ...
        const fileProviders = result.providers.filter((p) => p.description.title === "file");
        expect(fileProviders).toHaveLength(1);
        const fileProvider = fileProviders[0];
