/**
 * 文件类型：配置加载
 * 文件作用：将 Continue 配置转换为运行时可用的配置对象。
 * 主要职责：处理配置解析、模型实例化与工具/上下文提供者构建。
 */
...
import { DEFAULT_CONFIG_TS_CONTENTS, getConfigJsonPath, getConfigJsonPathForRemote, getConfigJsPath, getConfigJsPathForRemote, getConfigTsPath, getContinueDotEnv, getEsbuildBinaryPath, } from "../util/paths";
...
import { getModelByRole, isSupportedLanceDbCpuTargetForLinux, serializePromptTemplates, } from "./util";
 ...
 * 返回：解析后的序列化配置对象。
 */
export function resolveSerializedConfig(filepath) {
...
/**
 * 功能：合并全局与工作区配置，输出序列化配置。
 * 入参：workspaceConfigs 与 ideSettings 等环境参数。
 * 返回：包含合并结果与校验错误的配置结果。
 */
function loadSerializedConfig(workspaceConfigs, ideSettings, ideType, overrideConfigJson, ide) {
    ...
    if (ideSettings.remoteConfigServerUrl) {
        try {
            const remoteConfigJson = resolveSerializedConfig(getConfigJsonPathForRemote(ideSettings.remoteConfigServerUrl));
...
export function isContextProviderWithParams(contextProvider) {
...
async function intermediateToFinalConfig({ config, ide, ideSettings, ideInfo, uniqueId, llmLogger, workOsAccessToken, loadPromptFiles = true, }) {
    ...
    await Promise.all(config.models.map(async (desc) => {
        if ("title" in desc) {
            const llm = await llmFromDescription(desc, ide.readFile.bind(ide), getUriFromPath, uniqueId, ideSettings, llmLogger, config.completionOptions);
            if (!llm) {
                return;
            }
            if (llm.model === "AUTODETECT") {
                try {
                    ...
                    const detectedModels = await Promise.all(modelNames.map(async (modelName) => {
                        ...
                        }, ide.readFile.bind(ide), getUriFromPath, uniqueId, ideSettings, llmLogger, copyOf(config.completionOptions));
                    }));
            ...
            if (llm.model === "AUTODETECT") {
                try {
                    const modelNames = await llm.listModels();
                    const models = modelNames.map((modelName) => new CustomLLMClass({
                        ...desc,
                        options: {
                            ...desc.options,
                            model: modelName,
    ...
    const { providers: contextProviders, errors: contextErrors } = loadConfigContextProviders(config.contextProviders
    ...
    if (orgPolicy?.policy?.allowMcpServers === false) {
        ...
        const mcpOptions = (config.experimental?.modelContextProtocolServers ?? []).map((server, index) => ({
            ...
            name: `MCP Server`,
            requestOptions: mergeConfigYamlRequestOptions(server.transport.type !== "stdio"
...
async function handleEsbuildInstallation(ide, _ideType) {
    ...
    try {
        ...
        try {
            const userEsbuild = path.join(os.homedir(), ".continue", "node_modules", "esbuild");
...
export { finalToBrowserConfig, loadContinueConfigFromJson, };
