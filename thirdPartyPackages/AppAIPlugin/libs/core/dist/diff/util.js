 ...
 */
export function matchLine(newLine, oldLines, permissiveAboutIndentation = false) {
 ...
 */
export async function* streamLines(streamCompletion, log = false) {
    let allLines = [];
    let buffer = "";
    try {
...
export async function* generateLines(lines) {
    for (const line of lines) {
        yield line;
