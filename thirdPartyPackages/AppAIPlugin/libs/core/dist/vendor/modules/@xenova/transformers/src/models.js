...
import { ForcedBOSTokenLogitsProcessor, ForcedEOSTokenLogitsProcessor, ForceTokensLogitsProcessor, GenerationConfig, LogitsProcessorList, MinLengthLogitsProcessor, MinNewTokensLengthLogitsProcessor, NoBadWordsLogitsProcessor, NoRepeatNGramLogitsProcessor, RepetitionPenaltyLogitsProcessor, Sampler, SuppressTokensAtBeginLogitsProcessor, WhisperTimeStampLogitsProcessor, } from "./utils/generation.js";
...
async function constructSession(pretrained_model_name_or_path, fileName, options) {
    ...
    try {
        ...
        console.warn(err);
        console.warn("Something went wrong during model construction (most likely a missing operation). " +
...
function seq2seqStartBeams(self, inputTokenIds, generation_config, numOutputTokens) {
    ...
    let decoder_input_ids = generation_config.decoder_input_ids ??
...
export class PreTrainedModel extends Callable {
    ...
    constructor(config, session) {
        ...
        else if (modelType === MODEL_TYPES.Seq2Seq ||
    ...
    logits_processor = null) {
        ...
        //     ));
        // }
        if (generation_config.repetition_penalty !== null &&
            generation_config.repetition_penalty !== 1.0) {
            processors.push(new RepetitionPenaltyLogitsProcessor(generation_config.repetition_penalty));
        }
        if (generation_config.no_repeat_ngram_size !== null &&
        ...
        if (generation_config.begin_suppress_tokens !== null) {
            let begin_index = input_ids_seq_length > 1 ||
            ...
            if (generation_config.forced_decoder_ids !== null) {
                ...
                begin_index +=
                    generation_config.forced_decoder_ids[generation_config.forced_decoder_ids.length - 1][0];
            }
    ...
    async generate(inputs, generation_config = null, logits_processor = null, { inputs_attention_mask = null } = {}) {
        if (!this.can_generate) {
            ...
            const possibleInfo = MODEL_WITH_LM_HEAD_MAPPING_NAMES.get(modelType) ??
        ...
        const useMaxLength = Number.isInteger(generation_config.max_length) &&
        ...
        while (beams.some((x) => !x.done) && numOutputTokens < maxOutputTokens) {
            ...
            newest_beams = this.groupBeams(newest_beams).map((group) => group
                .sort((a, b) => b.score - a.score) // sort by score
                .slice(0, generation_config.num_beams));
            // Flatten beams
        ...
        const getFlattened = (key) => groupedBeams
            .map((batch) => {
    ...
    addAttentionsToBeam(beam, output) {
        if (this.config.is_encoder_decoder) {
            if (!output.cross_attentions || output.cross_attentions.length === 0) {
                throw Error("`output_attentions` is true, but the model did not produce cross-attentions. " +
        ...
        if (!output.decoder_attentions || output.decoder_attentions.length === 0) {
            throw Error("`output_attentions` is true, but the model did not produce decoder-attentions. " +
                "This is most likely because the model was not exported with `output_attentions=True`.");
        }
...
export class WhisperForConditionalGeneration extends WhisperPreTrainedModel {
    ...
    async generate(inputs, generation_config = null, logits_processor = null) {
        ...
        if (generation_config.return_token_timestamps) {
            ...
            if (!generation_config.alignment_heads) {
                throw new Error("Model generation config has no `alignment_heads`, token-level timestamps not available. " +
    ...
    _extract_token_timestamps(generate_outputs, alignment_heads, num_frames = null, time_precision = 0.02) {
        ...
        const batchedMatrices = generate_outputs.cross_attentions.map((batch) => {
            ...
            let cross_attentions = Array.from({ length: this.config.decoder_layers }, (_, i) => cat(batch.map((x) => x[i]), 2));
...
export class VisionEncoderDecoderModel extends PreTrainedModel {
    ...
    constructor(config, session, decoder_merged_session, generation_config) {
        ...
        const encoderModel = MODEL_MAPPING_NAMES_ENCODER_ONLY.get(encoderModelType) ??
...
export class SamModel extends SamPreTrainedModel {
     ...
     */
    async forward(model_inputs) {
        if (!model_inputs.image_embeddings ||
...
export class PretrainedMixin {
    ...
    static async from_pretrained(pretrained_model_name_or_path, { quantized = true, progress_callback = null, config = null, cache_dir = null, local_files_only = false, revision = "main", model_file_name = null, } = {}) {
        ...
        if (!this.MODEL_CLASS_MAPPINGS) {
            throw new Error("`MODEL_CLASS_MAPPINGS` not implemented for this type of `AutoClass`: " +
                this.name);
        }
