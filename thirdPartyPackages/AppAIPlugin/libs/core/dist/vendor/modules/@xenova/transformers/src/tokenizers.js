...
class LegacyTokenizerModel extends TokenizerModel {
    ...
    constructor(config, moreConfig) {
        super(config);
        /**@type {Map<string, number>} */
        this.tokens_to_ids = objectToMap(moreConfig.target_lang
...
class PreTokenizer extends Callable {
    ...
    pre_tokenize(text, options) {
        return (Array.isArray(text)
            ...
            : this.pre_tokenize_text(text, options)).flat();
    }
    /**
...
export class PreTrainedTokenizer extends Callable {
    ...
    { text_pair = null, add_special_tokens = true, padding = false, truncation = null, max_length = null, return_tensor = true, // Different to HF
     } = {}) {
        ...
        if (return_tensor) {
            if (!(padding && truncation)) {
                ...
                if (encodedTokens.some((x) => {
                    ...
                    throw Error("Unable to create tensor, you should probably activate truncation and/or padding " +
    ...
    _encode_text(text) {
        ...
        const tokens = sections
            ...
            if (addedToken !== undefined) {
                ...
                const sectionTokens = this.pre_tokenizer !== null
    ...
    get default_chat_template() {
        if (!this._warned_about_chat_template) {
            console.warn("No chat template is defined for this tokenizer - using a default chat template " +
     ...
     */
    apply_chat_template(conversation, { chat_template = null, add_generation_prompt = false, tokenize = true, padding = false, truncation = false, max_length = null, return_tensor = true, } = {}) {
...
export class WhisperTokenizer extends PreTrainedTokenizer {
    ...
    findLongestCommonSequence(sequences, token_timestamp_sequences = null) {
        ...
        let totalSequence = [];
        const use_token_timestamp_sequences = Array.isArray(token_timestamp_sequences) &&
...
export class AutoTokenizer {
    ...
    static async from_pretrained(pretrained_model_name_or_path, { quantized = true, progress_callback = null, config = null, cache_dir = null, local_files_only = false, revision = "main", legacy = null, } = {}) {
        ...
        const tokenizerName = tokenizerConfig.tokenizer_class?.replace(/Fast$/, "") ??
            "PreTrainedTokenizer";
