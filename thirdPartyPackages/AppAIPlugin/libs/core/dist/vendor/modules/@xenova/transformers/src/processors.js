...
function validate_audio_inputs(audio, feature_extractor) {
    if (!(audio instanceof Float32Array || audio instanceof Float64Array)) {
        throw new Error(`${feature_extractor} expects input to be a Float32Array or a Float64Array, but got ${audio?.constructor?.name ?? typeof audio} instead.` +
...
export class ImageFeatureExtractor extends FeatureExtractor {
                ...
                for (let i = 0; i < paddedImageHeight; ++i) {
                    ...
                    for (let j = 0; j < paddedImageWidth; ++j) {
                        if (i < imageHeight && j < imageWidth)
                            continue; // Do not overwrite original image
     ...
     */
    async preprocess(image, { do_normalize = null, do_pad = null, do_convert_rgb = null, do_convert_grayscale = null, } = {}) {
        ...
        if (do_normalize ?? this.do_normalize) {
            ...
            if (image_mean.length !== image.channels ||
...
export class WhisperFeatureExtractor extends FeatureExtractor {
    constructor(config) {
        ...
        this.config.mel_filters ??= mel_filter_bank(Math.floor(1 + this.config.n_fft / 2), // num_frequency_bins
    ...
    _extract_fbank_features(waveform) {
        const { data, dims } = spectrogram(waveform, this.window, // window
    ...
    async _call(audio) {
        ...
        if (audio.length > this.config.n_samples) {
            console.warn("Attempting to extract features for audio longer than 30 seconds. " +
...
export class Wav2Vec2FeatureExtractor extends FeatureExtractor {
    ...
    _zero_mean_unit_var_norm(input_values) {
        ...
        const variance = input_values.reduce((a, b) => a + (b - mean) ** 2, 0) /
            input_values.length;
...
export class ASTFeatureExtractor extends FeatureExtractor {
    constructor(config) {
        ...
        const mel_filters = mel_filter_bank(256, // num_frequency_bins
    ...
    _extract_fbank_features(waveform, max_length) {
        ...
        return spectrogram(waveform, this.window, // window
        400, // frame_length
        160, // hop_length
        {
            fft_length: 512,
...
export class ClapFeatureExtractor extends FeatureExtractor {
    constructor(config) {
        super(config);
        this.mel_filters = mel_filter_bank(this.config.nb_frequency_bins, // num_frequency_bins
        this.config.feature_size, // num_mel_filters
        ...
        null, // norm
        "htk");
        this.mel_filters_slaney = mel_filter_bank(this.config.nb_frequency_bins, // num_frequency_bins
        this.config.feature_size, // num_mel_filters
        this.config.frequency_min, // min_frequency
        this.config.frequency_max, // max_frequency
        this.config.sampling_rate, // sampling_rate
...
export class AutoProcessor {
    ...
    static async from_pretrained(pretrained_model_name_or_path, { progress_callback = null, config = null, cache_dir = null, local_files_only = false, revision = "main", } = {}) {
        ...
        let processor_class = this.PROCESSOR_CLASS_MAPPING[preprocessorConfig.processor_class] ??
            Processor;
