...
function handleError(status, remoteURL, fatal) {
    ...
    const message = ERROR_MAPPING[status] ??
...
export async function getModelFile(path_or_repo_id, filename, fatal = true, options = {}) {
    ...
    if (!cache && env.useCustomCache) {
        ...
        if (!env.customCache.match || !env.customCache.put) {
            throw new Error("`env.customCache` must be an object which implements the `match` and `put` functions of the Web Cache API. " +
    ...
    let remoteURL = pathJoin(env.remoteHost, env.remotePathTemplate
        .replaceAll("{model}", path_or_repo_id)
        .replaceAll("{revision}", encodeURIComponent(revision)), filename);
    // Choose cache key for filesystem cache
    // When using the main revision (default), we use the request URL as the cache key.
    // If a specific revision is requested, we account for this in the cache key.
    let fsCacheKey = revision === "main"
        ? requestURL
    ...
    toCacheResponse &&
        ...
        await cache
            .put(cacheKey, new Response(buffer, {
