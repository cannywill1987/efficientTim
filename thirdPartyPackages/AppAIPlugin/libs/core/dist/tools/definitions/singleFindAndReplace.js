...
export const singleFindAndReplaceTool = {
    type: "function",
...
- The edit will likely fail if you have not recently used the \`${BuiltInToolNames.ReadFile}\` tool to view up-to-date file contents.`,
  ...
  For example, you could respond with:`,
    ...
    preprocessArgs: async (args, extras) => {
        const { oldString, newString, replaceAll } = validateSingleEdit(args.old_string, args.new_string, args.replace_all);
        const fileUri = await validateSearchAndReplaceFilepath(args.filepath, extras.ide);
        const editingFileContents = await extras.ide.readFile(fileUri);
        const newFileContents = executeFindAndReplace(editingFileContents, oldString, newString, replaceAll ?? false, 0);
        return {
            ...args,
            fileUri,
            editingFileContents,
