...
export function encodeMCPToolUri(mcpId, toolName) {
    return `mcp://${encodeURIComponent(mcpId)}/${encodeURIComponent(toolName)}`;
}
export function decodeMCPToolUri(uri) {
...
async function callToolFromUri(uri, args, extras) {
    ...
    switch (parsedUri?.protocol) {
        ...
        case "mcp:":
            ...
            const coercedArgs = coerceArgsToSchema(args, extras.tool?.function?.parameters);
            const response = await client.client.callTool({
                name: toolName,
                arguments: coercedArgs,
            }, CallToolResultSchema, { timeout: client.options.timeout });
            if (response.isError === true) {
                throw new Error(JSON.stringify(response.content));
            }
            let mcpUiState = undefined;
            const uiResourceUri = extras.tool?.mcpMeta?.ui?.resourceUri ||
            ...
            if (uiResourceUri) {
                try {
                    ...
                    if (!mcpUiState) {
                        console.error("Invalid MCP UI resource content", JSON.stringify(resource));
                    }
                }
                catch (e) {
...
export async function callBuiltInTool(functionName, args, extras) {
    switch (functionName) {
