...
test("readFileRange should validate positive line number arguments", () => {
    ...
    expect(() => getNumberArg({}, "startLine")).toThrow("Argument `startLine` is required (type number)");
    expect(() => getNumberArg({}, "endLine")).toThrow("Argument `endLine` is required (type number)");
    // Test invalid arguments
    expect(() => getNumberArg({ startLine: "not-a-number" }, "startLine")).toThrow("Argument `startLine` must be a valid number");
    expect(() => getNumberArg({ endLine: true }, "endLine")).toThrow("Argument `endLine` must be a valid number");
});
