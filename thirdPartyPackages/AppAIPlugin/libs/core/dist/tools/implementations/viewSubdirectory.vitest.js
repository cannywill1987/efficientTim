...
describe("viewSubdirectoryImpl", () => {
    it("should throw DirectoryNotFound when resolveInputPath returns null", async () => {
        ...
        await expect(viewSubdirectoryImpl({ directory_path: "/non/existent/path" }, mockExtras)).rejects.toThrow(ContinueError);
    });
    it("should throw DirectoryNotFound when path exists in resolveInputPath but not on filesystem", async () => {
        ...
        try {
            await viewSubdirectoryImpl({ directory_path: "/some/absolute/path" }, mockExtras);
            expect.fail("Should have thrown DirectoryNotFound error");
        }
        catch (error) {
            expect(error).toBeInstanceOf(ContinueError);
            expect(error.reason).toBe(ContinueErrorReason.DirectoryNotFound);
            expect(error.message).toContain("does not exist or is not accessible");
        }
    });
});
