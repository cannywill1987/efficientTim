...
describe("runTerminalCommand timeout functionality", () => {
    ...
    beforeEach(() => {
        ...
        mockChildProc.kill = vi.fn((signal) => {
    ...
    it("should NOT set up timeout when waitForCompletion is false", async () => {
        ...
        const timeoutCalls = setTimeoutSpy.mock.calls.filter((call) => call[1] === 120_000);
    ...
    it("should kill process when timeout fires (streaming)", async () => {
        ...
        expect(result[0].content).toContain("[Timeout: process killed after 2 minutes]");
    });
