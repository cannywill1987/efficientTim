...
test("readFileRangeImpl validates line number constraints", async () => {
    ...
    await expect(readFileRangeImpl({
        filepath: "test.txt",
        startLine: 0,
        endLine: 5,
    }, mockExtras)).rejects.toThrow("startLine must be 1 or greater");
    // Test negative startLine (no longer supported)
    await expect(readFileRangeImpl({
        filepath: "test.txt",
        startLine: -1,
        endLine: 5,
    }, mockExtras)).rejects.toThrow("Negative line numbers are not supported");
    // Test endLine < 1 (invalid)
    await expect(readFileRangeImpl({
        filepath: "test.txt",
        startLine: 1,
        endLine: 0,
    }, mockExtras)).rejects.toThrow("endLine must be 1 or greater");
    ...
    await expect(readFileRangeImpl({
        ...
        startLine: 10,
        endLine: 5,
    }, mockExtras)).rejects.toThrow("endLine (5) must be greater than or equal to startLine (10)");
});
