...
export const grepSearchImpl = async (args, extras) => {
    ...
    try {
        ...
        throw new ContinueError(ContinueErrorReason.SearchExecutionFailed, errorMessage);
    }
    const { formatted, numResults, truncated } = formatGrepSearchResults(results, DEFAULT_GREP_SEARCH_CHAR_LIMIT);
    if (numResults === 0) {
        return [
            {
                name: "Search results",
