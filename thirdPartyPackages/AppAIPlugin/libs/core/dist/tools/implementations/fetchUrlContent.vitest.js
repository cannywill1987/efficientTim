...
test("fetchUrlContent should not truncate content under character limit", async () => {
    ...
    getUrlContextItems.mockResolvedValue(mockContextItems);
    const result = await fetchUrlContentImpl({ url: "https://example.com" }, mockExtras);
...
test("fetchUrlContent should propagate errors when URL fetch fails", async () => {
    ...
    getUrlContextItems.mockRejectedValue(new Error(errorMessage));
    await expect(fetchUrlContentImpl({ url: "https://example.com/404" }, mockExtras)).rejects.toThrow(errorMessage);
});
