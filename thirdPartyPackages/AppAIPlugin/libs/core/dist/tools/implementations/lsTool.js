...
export function resolveLsToolDirPath(dirPath) {
    if (!dirPath || dirPath === ".") {
        return ".";
    }
...
export const lsToolImpl = async (args, extras) => {
    const dirPath = resolveLsToolDirPath(args?.dirPath);
    const resolvedPath = await resolveInputPath(extras.ide, dirPath);
    if (!resolvedPath) {
        throw new ContinueError(ContinueErrorReason.DirectoryNotFound, `Directory ${args.dirPath} not found or is not accessible. You can use absolute paths, relative paths, or paths starting with ~`);
    }
