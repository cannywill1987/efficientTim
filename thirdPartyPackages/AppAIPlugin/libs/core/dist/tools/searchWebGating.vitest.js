...
test("searchWeb tool is only available when user is signed in", async () => {
    ...
    const searchWebToolSignedIn = signedInTools.find((tool) => tool.function.name === BuiltInToolNames.SearchWeb);
    ...
    const searchWebToolNotSignedIn = notSignedInTools.find((tool) => tool.function.name === BuiltInToolNames.SearchWeb);
