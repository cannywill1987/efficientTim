...
import { getInitialToolCallParseState, } from "./types";
/*
...
*/
export async function* interceptSystemToolCalls(messageGenerator, abortController, systemToolFramework) {
    let buffer = "";
    let parseState;
    while (true) {
        ...
        if (result.done) {
            ...
            for await (const message of result.value) {
                ...
                for (const chunk of chunks) {
                    buffer += chunk;
                    if (!parseState) {
                        const { isInPartialStart, isInToolCall, modifiedBuffer } = detectToolCallStart(buffer, systemToolFramework);
                        if (isInPartialStart) {
                            continue;
                        }
                        if (isInToolCall) {
                    ...
                    if (parseState && !parseState.done) {
                        const delta = systemToolFramework.handleToolCallBuffer(buffer, parseState);
                        if (delta) {
                            yield [
                                {
                                    ...message,
