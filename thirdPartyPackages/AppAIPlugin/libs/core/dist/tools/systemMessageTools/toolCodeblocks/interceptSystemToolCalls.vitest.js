...
describe("interceptSystemToolCalls", () => {
    ...
    const createAsyncGenerator = async function* (messages) {
    ...
    it("passes through non-assistant messages unchanged", async () => {
        ...
        const generator = interceptSystemToolCalls(createAsyncGenerator(messages), abortController, framework);
    ...
    it("processes standard tool call format", async () => {
        ...
        // Tool name detection
        result = await generator.next();
        expect(result.value[0].toolCalls?.[0].function?.name).toBe("test_tool");
        // Begin argument
        result = await generator.next();
        expect(result.value[0].toolCalls?.[0].function
            ?.arguments).toContain('{"arg1":');
        // Argument value
        result = await generator.next();
        expect(result.value[0].toolCalls?.[0].function
            ?.arguments).toBe('"value1"');
        // End of tool call
        result = await generator.next();
        expect(result.value[0].toolCalls?.[0].function
            ?.arguments).toBe("}");
