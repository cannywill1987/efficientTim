...
export const generateToolsSystemMessage = (tools, framework) => {
    if (tools.length === 0) {
        return "";
    }
    ...
    if (withPredefinedMessage.length > 0) {
        ...
        for (const tool of withPredefinedMessage) {
            const definition = framework.createSystemMessageExampleCall(tool.function.name, tool.systemMessageDescription.prefix, tool.systemMessageDescription.exampleArgs);
            instructions.push(`\n${definition}`);
        }
    }
    if (withDynamicMessage.length > 0) {
        ...
        for (const tool of withDynamicMessage) {
            try {
                ...
                console.error("Failed to convert tool to system message tool:\n" +
                    JSON.stringify(tool));
            }
        }
...
export function addSystemMessageToolsToSystemMessage(framework, baseSystemMessage, systemMessageTools) {
    let systemMessage = baseSystemMessage;
    if (systemMessageTools.length > 0) {
        const toolsSystemMessage = generateToolsSystemMessage(systemMessageTools, framework);
        systemMessage += `\n\n${toolsSystemMessage}`;
    }
