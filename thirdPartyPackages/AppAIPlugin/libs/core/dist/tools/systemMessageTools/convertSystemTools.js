import { normalizeToMessageParts, renderContextItems, } from "../../util/messageContent";
import { CANCELLED_TOOL_CALL_MESSAGE, NO_TOOL_CALL_OUTPUT_MESSAGE, } from "../constants";
function toolCallStateToSystemToolOutput(state) {
...
export function convertToolCallStatesToSystemCallsAndOutput(originalAssistantMessage, toolCallStates, framework) {
    const parts = normalizeToMessageParts(originalAssistantMessage);
    if (originalAssistantMessage.toolCalls) {
        parts.push(...toolCallStates.map((state) => ({
            type: "text",
