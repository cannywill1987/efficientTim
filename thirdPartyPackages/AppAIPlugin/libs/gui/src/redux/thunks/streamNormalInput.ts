...
>(
  ...
  async (
    ...
    // Keep chat stream resilient during initial WebView config hydration.
    ...
    if (resolvedChatModel.toolOverrides?.length) {
      const { tools: overriddenTools, errors } = applyToolOverrides(
        activeTools,
        resolvedChatModel.toolOverrides,
      );
    ...
    const useNativeTools = state.config.config.experimental
      ...
      ? false
      : modelSupportsNativeTools(resolvedChatModel);
    ...
    try {
        ...
        try {
          extra.ideMessenger.post("devdata/log", {
            ...
            data: {
              ...
              modelProvider: resolvedChatModel.underlyingProviderName,
              modelName: resolvedChatModel.title,
              modelTitle: resolvedChatModel.title,
      ...
      posthog.capture("stream_premature_close_error", {
        ...
        provider: resolvedChatModel.underlyingProviderName,
