...
export function Chat() {
  ...
  const sendInput = useCallback(
    (
      ...
      const availableChatModels = stateSnapshot.config.config.modelsByRole.chat;
      ...
      let model = isCurrentlyInEdit
        ? (selectedModelByRole.edit ?? selectedModelByRole.chat)
        : selectedModelByRole.chat;

      // In embedded Flutter/WebView mode the initial config message can arrive
      // after the first render, leaving the UI on "Select model" even though a
      // chat model is available. Auto-select the first chat model instead of
      // silently dropping Send/Enter.
      if (!model && availableChatModels.length > 0) {
        const fallbackChatModel = availableChatModels[0];
        reduxStore.dispatch(
          updateConfig({
        ...
        model = isCurrentlyInEdit
          ? (selectedModelByRole.edit ?? fallbackChatModel)
          : fallbackChatModel;
      }

      if (!model) {
        console.error("No chat model selected and no fallback chat model found");
        return;
      }
  ...
  return (
    <>
      ...
      {widget}
      <div className="border-command-border mx-2 flex items-center justify-end gap-1 border-0 border-b border-solid pb-1.5 pt-1">
        <HeaderButtonWithToolTip
          text="New chat"
          onClick={() => {
            void handleNewSessionClick();
          }}
        >
          <PlusIcon className="text-description-muted h-3.5 w-3.5" />
        </HeaderButtonWithToolTip>
        <HeaderButtonWithToolTip
          text="History"
          onClick={() => navigate("/history")}
        >
          <QueueListIcon className="text-description-muted h-3.5 w-3.5" />
        </HeaderButtonWithToolTip>
        <HeaderButtonWithToolTip
          text="Settings"
          onClick={() => navigate(CONFIG_ROUTES.SETTINGS)}
        >
          <Cog6ToothIcon className="text-description-muted h-3.5 w-3.5" />
        </HeaderButtonWithToolTip>
      </div>
