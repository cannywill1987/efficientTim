...
function getVsCodeBridge(): vscode | undefined {
  if (typeof vscode !== "undefined") {
    return vscode;
  }

  const globalBridge = (globalThis as any).vscode ?? (window as any).vscode;
  if (globalBridge) {
    return globalBridge as vscode;
  }

  if (typeof window.acquireVsCodeApi === "function") {
    try {
      return window.acquireVsCodeApi() as vscode;
    } catch (error) {
      console.warn("Failed to acquire VS Code bridge", error);
    }
  }
