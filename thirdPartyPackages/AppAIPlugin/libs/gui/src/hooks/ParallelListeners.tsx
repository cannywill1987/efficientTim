  ...
  const handleConfigUpdate = useCallback(
    async (isInitial: boolean, result: FromCoreProtocol["configUpdate"][0]) => {
      ...
      const normalizedConfig =
        !configResult.config.selectedModelByRole.chat &&
        configResult.config.modelsByRole.chat.length > 0
          ? {
              ...configResult.config,
              selectedModelByRole: {
                ...configResult.config.selectedModelByRole,
                chat: configResult.config.modelsByRole.chat[0],
              },
            }
          : configResult.config;

      const normalizedConfigResult = {
        ...configResult,
      ...
      dispatch(setConfigResult(normalizedConfigResult));
      ...
      if (normalizedConfig?.ui?.fontSize) {
        setLocalStorage("fontSize", normalizedConfig.ui.fontSize);
        document.body.style.fontSize = `${normalizedConfig.ui.fontSize}px`;
      }

      const chatModel = normalizedConfig?.selectedModelByRole.chat;
