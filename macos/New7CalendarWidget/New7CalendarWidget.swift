...
struct TimerManager {
    ...
    static let extraLargeCalendar = Calendar.current
    static var extraLargeCurrentDate = Date()
    static var extraLargeSelectedDate: Date? = Date()
    
    static var extraLargeCurrentPage:Int = 0
    static var extraLargeTotalPages: Int = 0
}
...
enum CalendarAction: String, AppEnum {
    ...
    case extraLargeNextPage = "extraLargeNextPage"
    case extraLargePreviousPage = "extraLargePreviousPage"
    
    case extraLargePreviousMonth = "extraLargePreviousMonth"
    case extraLargeNextMonth = "extraLargeNextMonth"
    case extraLargeResetToToday = "extraLargeResetToToday"
    case extraLargeSelectDate = "extraLargeSelectDate"
    ...
    //    case ExtraLargeCurrentPage = "ExtraLargeCurrentPage"
    //    case ExtraLargeTotalPages = "ExtraLargeTotalPages"
    ...
    static var caseDisplayRepresentations: [Self: DisplayRepresentation] {
        [
            ...
            .extraLargeNextPage: "extraLargeNextPage",
            .extraLargePreviousPage: "extraLargePreviousPage",
            .extraLargePreviousMonth: "extraLargePreviousMonth",
            .extraLargeNextMonth: "extraLargeNextMonth",
            .extraLargeResetToToday: "extraLargeResetToToday",
            .extraLargeSelectDate: "extraLargeSelectDate"
        ]
    }
}
...
struct CalendarActionIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult {
        ...
        case .mediumNextPage:
            ...
            break;
        case .extraLargePreviousMonth:
            TimerManager.extraLargeCurrentDate = calendar.date(byAdding: .month, value: -1, to: TimerManager.extraLargeCurrentDate) ?? TimerManager.extraLargeCurrentDate
            break;
        case .extraLargeNextMonth:
            TimerManager.extraLargeCurrentDate = calendar.date(byAdding: .month, value: 1, to: TimerManager.extraLargeCurrentDate) ?? TimerManager.extraLargeCurrentDate
            
            break;
        case .extraLargeResetToToday:
            TimerManager.extraLargeCurrentDate = Date()
            
            break;
        case .extraLargeSelectDate:
            TimerManager.extraLargeSelectedDate = dayDate
            break;
            
        case .extraLargeNextPage:
            if TimerManager.extraLargeCurrentPage < TimerManager.extraLargeTotalPages - 1 {
                withAnimation {
                    TimerManager.extraLargeCurrentPage = TimerManager.extraLargeCurrentPage + 1
                }
            }
            break;
        case .extraLargePreviousPage:
            if TimerManager.extraLargeCurrentPage > 0 {
                TimerManager.extraLargeCurrentPage = TimerManager.extraLargeCurrentPage - 1
            }
            break;
            //        case .ExtraLargeCurrentPage:
            //            break;
            //        case .ExtraLargeTotalPages:
            //            break;
...
struct CalendarWidgetView: View {
    ...
    var body: some View {
        ...
        if family == .systemMedium {
            ...
            ExtraCalendarComponent2(missionModelList: entry.missionModelList)
                .padding(.top, 50)
                .padding(.bottom, 50)
        }
    }
...
struct MediumCalendarComponent2: View {
    ...
    func areDatesEqual(date1Timestamp: Int, date2Timestamp: Int) -> Bool {
        ...
        // 检查年份、月份和日期是否完全相等
        let res = components1.year == components2.year &&
...
struct CalendarWidgetProvider: TimelineProvider {
    ...
    func placeholder(in context: Context) -> CalendarWidgetEntry {
        CalendarWidgetEntry(date: Date(), missionModelList: [], currentPage: 0)
        
    }
    
    func getSnapshot(in context: Context, completion: @escaping (CalendarWidgetEntry) -> Void) {
        let entry = CalendarWidgetEntry(date: Date(), missionModelList: [MissionModelList(time: Int(Date().addingTimeInterval(7 * 24 * 60 * 60).timeIntervalSince1970 * 1000), lunar: "", listMissionModel: [])], currentPage: 0)
        completion(entry)
    }
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<CalendarWidgetEntry>) -> Void) {
        ...
        do {
            ...
            print("Could not decode primaryData")
        }
        
        // 计算当天午夜的时间
        let calendar = Calendar.current
        var midnight: Date? = calendar.startOfDay(for: currentDate) ?? Date()
        var nextMidnight = calendar.date(byAdding: .day, value: 1, to: midnight!)
        entries.append(CalendarWidgetEntry(
            date: nextMidnight ?? currentDate,
            missionModelList: myCalendarMissionStoreData?.listMissionModelList ?? [],
            currentPage: 0
        ))
        myCalendarMissionStoreData?.listMissionModelList
        for index1 in 0..<(myCalendarMissionStoreData?.listMissionModelList.count ?? 0) {
            let missionModelList:MissionModelList? = myCalendarMissionStoreData?.listMissionModelList[index1] ?? nil
            let time = myCalendarMissionStoreData?.listMissionModelList[index1].time ?? 0
            
            for index2 in 0..<(missionModelList?.listMissionModel.count ?? 0) {
                let date1 = Date(timeIntervalSince1970: TimeInterval(time) / 1000)
                print(date1)
                let model: MissionModel? = missionModelList?.listMissionModel[index2] ?? nil
            }
        }
    ...
    var body: some View {
        VStack(spacing: 1) {
            ...
            VStack() {
                // 顶部星期标题行
                HStack(spacing: 1) {
                    ForEach(["日", "一", "二", "三", "四", "五", "六"], id: \ .self) { dayOfWeek in
                        Text(dayOfWeek)
...
struct ExtraCalendarComponent2: View {
    let calendar = Calendar.current
    //    @State var currentDate = Date()
    //    @State var selectedDate: Date? = nil
    //    @State var currentPage:Int = 0
    var daysInMonth: [Int] {
        let range = calendar.range(of: .day, in: .month, for: TimerManager.extraLargeCurrentDate)!
        return range.map { $0 }
    }
    var missionModelList: [MissionModelList] = []
    ...
    func isSelectedDate(timestampMilli: Int) -> Bool {
        if TimerManager.extraLargeSelectedDate == nil {
            return false;
        }
        let date = Date(timeIntervalSince1970: TimeInterval(timestampMilli) / 1000)
        let calendar = Calendar.current
        //        return calendar.isDateInToday(date)
        let components1 = calendar.dateComponents([.year, .month, .day], from: TimerManager.extraLargeSelectedDate!)
        let components2 = calendar.dateComponents([.year, .month, .day], from: date)
    ...
    func getTotalPages(maxMissionsPerPage: Int) -> Int {
        let dayTimestamp = Int((TimerManager.extraLargeSelectedDate ?? Date()).timeIntervalSince1970 * 1000)
        let missions = getMissionModels(dayTimestamp: dayTimestamp) ?? []
        //        let maxMissionsPerPage = 5
        let totalPages = (missions.count + maxMissionsPerPage - 1) / maxMissionsPerPage
        TimerManager.extraLargeTotalPages = totalPages
        return totalPages
    }
    ...
    var body: some View {
        HStack(spacing: 0) {
            VStack(spacing: 1) {
                // 顶部年月标题和左右切换按钮
                HStack {
                    Button(intent:  CalendarActionIntent(action: .extraLargePreviousMonth, dayDate: Date(),objectId: "")) {
                        Image(systemName: "chevron.left")
                            .foregroundColor(.white)
                            .font(.system(size: 12, weight: .bold))
                        ...
                        .background(Color.clear)
                    Text(TimerManager.extraLargeCurrentDate, formatter: monthFormatter)
                        .foregroundColor(.white)
                        .font(.system(size: 12, weight: .bold))
                    Button(intent:  CalendarActionIntent(action: .extraLargeNextMonth, dayDate: Date(),objectId: "")) {
                        Image(systemName: "chevron.right")
                            .foregroundColor(.white)
                            .font(.system(size: 12, weight: .bold))
                    }.buttonStyle(PlainButtonStyle())
                        .background(Color.clear)
                    Spacer()
                    Button(intent:  CalendarActionIntent(action: .extraLargeResetToToday, dayDate: Date(),objectId: "")) {
                        Image(systemName: "scope")
                            .foregroundColor(.white)
                            .font(.system(size: 12, weight: .bold))
                    }.buttonStyle(PlainButtonStyle())
                ...
                let firstDayOfMonth = calendar.date(from: calendar.dateComponents([.year, .month], from: TimerManager.extraLargeCurrentDate))!
                let firstWeekday = calendar.component(.weekday, from: firstDayOfMonth) - 1
                let previousMonthDate = calendar.date(byAdding: .month, value: -1, to: TimerManager.extraLargeCurrentDate)!
                let previousMonthRange = calendar.range(of: .day, in: .month, for: previousMonthDate)!
                let previousMonthDays = Array(previousMonthRange)
                let nextMonthDate = calendar.date(byAdding: .month, value: 1, to: TimerManager.extraLargeCurrentDate)!
                let nextMonthRange = calendar.range(of: .day, in: .month, for: nextMonthDate)!
                let nextMonthDays = Array(nextMonthRange)
                let totalSlots = daysInMonth.count + firstWeekday
                let rows = totalSlots / 7 + (totalSlots % 7 == 0 ? 0 : 1)
                    ...
                    HStack(spacing: 1) {
                        ForEach(0..<7, id: \ .self) { columnIndex in
                            ...
                            if rowIndex == 0 && columnIndex < firstWeekday {
                                ...
                                let dayTimestamp:Int = Int(calendar.startOfDay(for: dayDate).timeIntervalSince1970 * 1000)
                                let list = getMissionModels(dayTimestamp: dayTimestamp) ?? []
                                ExtraLargeDayView(timestampMilli: dayTimestamp, day: "\(day)", missions: list, onTap: {
                                    TimerManager.extraLargeSelectedDate = dayDate
                                }, isSelected: isSelectedDate(timestampMilli: dayTimestamp), dayInt: day, monthDate: previousMonthDate, row: Int(rows))
                                .opacity(0.5) // 显示为淡色表示不是
                            } else if dayIndex >= 0 && dayIndex < daysInMonth.count {
                                // 当前月的日期
                                let day = daysInMonth[dayIndex]
                                let monthDate = calendar.date(from: calendar.dateComponents([.year, .month], from: TimerManager.extraLargeCurrentDate)) ?? Date()
                                let dayDate2 = calendar.date(bySetting: .day, value: day, of: monthDate) ?? Date()
                                let dayTimestamp = Int(dayDate2.timeIntervalSince1970 * 1000)
                                let list = getMissionModels(dayTimestamp: dayTimestamp) ?? []
                                let calendar = Calendar.current
                                let startOfMonth2 = calendar.date(from: calendar.dateComponents([.year, .month], from: TimerManager.extraLargeCurrentDate))
                                ExtraLargeDayView(timestampMilli: dayTimestamp, day: "\(day)", missions: list, onTap: {
                                    //                                    TimerManager.mediumSelectedDate = dayDate
                                }, isSelected: isSelectedDate(timestampMilli: dayTimestamp), dayInt: day, monthDate: monthDate, row: Int(rows))
                            } else if rowIndex == rows - 1 && columnIndex > lastWeekday {
                                // 填充下个月的日期
                                let day = nextMonthDays[columnIndex - lastWeekday - 1]
                                let dayDate = calendar.date(bySetting: .day, value: day, of: nextMonthDate) ?? Date()
                                ...
                                ExtraLargeDayView(timestampMilli: dayTimestamp, day: "\(day)", missions: list, onTap: {
                                    TimerManager.extraLargeSelectedDate = dayDate
                                }, isSelected: isSelectedDate(timestampMilli: dayTimestamp), dayInt: day, monthDate: nextMonthDate, row: Int(rows))
                                .opacity(0.5) // 显示为淡色表示不是
                            } else {
                                ExtraLargeDayView(timestampMilli: 0, day: "", missions: [], onTap: {}, isSelected: false, dayInt: 1, monthDate: Date(), row: Int(rows)) // 用空的DayView保持对称
                            }
                        }
                    }.frame(width: .infinity)
                    //                        .background(Color.yellow) // HStack 背景颜色
                        .padding(.trailing, 12)
                        .padding(.leading, 12)
                }
            }
            ...
            // 右侧任务列表
            VStack(alignment: .leading, spacing: 1) {
                if let selectedDate = TimerManager.extraLargeSelectedDate {
                    let dayTimestamp = Int(selectedDate.timeIntervalSince1970 * 1000)
                    let missions = getMissionModels(dayTimestamp: dayTimestamp) ?? []
                    let maxMissionsPerPage = 15
                    //                    let totalPages = (missions.count + maxMissionsPerPage - 1) / maxMissionsPerPage
                    let totalPages = getTotalPages(maxMissionsPerPage: maxMissionsPerPage)
                    ...
                    if !missions.isEmpty {
                        ...
                        HStack {
                            Spacer()
                            Button(intent: CalendarActionIntent(action: .extraLargePreviousPage, dayDate: Date(),objectId: "")) {
                                Image(systemName: "chevron.left")
                                    .foregroundColor(TimerManager.extraLargeCurrentPage > 0 ? .white : .gray)
                                    .font(.system(size: 12, weight: .bold))
                            }.frame(width: 12, height: 12).buttonStyle(PlainButtonStyle())
                                .background(Color.clear)
                            
                            Text("\(TimerManager.extraLargeCurrentPage + 1) / \(TimerManager.extraLargeTotalPages)")
                                .foregroundColor(.white)
                                .font(.system(size: 12))
                            Button(intent: CalendarActionIntent(action: .extraLargeNextPage, dayDate: Date(),objectId: "")) {
                                Image(systemName: "chevron.right")
                                    .foregroundColor(TimerManager.extraLargeCurrentPage < TimerManager.extraLargeTotalPages - 1 ? .white : .gray)
                                    .font(.system(size: 12, weight: .bold))
                            }.frame(width: 12, height: 12).buttonStyle(PlainButtonStyle())
                                .background(Color.clear)
                        }
                        ...
                        .padding(.top, 5)
                        VStack(alignment: .trailing, spacing: 1) {
                            if TimerManager.extraLargeCurrentPage * maxMissionsPerPage < missions.count {
                                ForEach(missions[(TimerManager.extraLargeCurrentPage * maxMissionsPerPage)..<min((TimerManager.extraLargeCurrentPage + 1) * maxMissionsPerPage, missions.count)], id: \.self) { mission in
                                    let model: MissionModel? = mission as MissionModel // 必须加上 否则会有subtype解析错误
                                    let objectId: String = model?.objectId ?? ""
                                    Button(intent: CalendarActionIntent(action: .clickFinishMission, dayDate: Date(),objectId: objectId ?? "")) {
...
struct ExtraLargeDayView: View {
    ...
    var body: some View {
        let count = row == 5 ? 3 : 2
        Button(intent: CalendarActionIntent(action: .extraLargeSelectDate, dayDate: calendar.date(bySetting: .day, value: dayInt, of: monthDate) ?? Date(),objectId: "")) {
        VStack(alignment: .leading, spacing: 1) {
        ...
        //            .background(Color.red) // HStack 背景颜色
            .background(RoundedRectangle(cornerRadius: 3)
                        //                        .frame(minWidth: 0, maxWidth: .infinity)
                        //                        .frame(height: .infinity)
