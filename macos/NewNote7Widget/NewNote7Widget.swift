...
struct Provider: TimelineProvider {
    @AppStorage("WQBMissionStoreDataNote7", store: UserDefaults(suiteName: "S4CLCWPCGH.com.timespeed.timehello")) var primaryData : Data = Data()
...
struct NewNote7WidgetEntryView : View {
    var entry: Provider.Entry

    var body: some View {
//        Text("123")
    NoteViewWidget(missionData: entry.missionData, subTitle: "note7".localizable())
       }
}


struct NewNote7Widget: Widget {
    let kind: String = "NewNoteWidget"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: Provider()) { entry in
            if #available(macOS 14.0, *) {
                NewNote7WidgetEntryView(entry: entry)
        ...
        .configurationDisplayName("note7".localizable())
