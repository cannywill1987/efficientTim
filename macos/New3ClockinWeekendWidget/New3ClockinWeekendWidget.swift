...
//        }
    ...
    var body: some View {
        VStack {
            // 顶部显示星期几
            HStack {
                Text("\(entry.date)-\(TimerManager.isLoading ? "111": "")").foregroundColor(.white)
            ...
            ForEach(entry.tasks.prefix(6)) { task in
                HStack {
                    ...
                    ForEach(0..<7) { dayIndex in
                        Button(intent: TimerIntent(missionID: task.objectId ?? "", timestamp: task.timestamp + dayIndex * 24 * 60 * 60 *  1000)) {
                            Rectangle()
