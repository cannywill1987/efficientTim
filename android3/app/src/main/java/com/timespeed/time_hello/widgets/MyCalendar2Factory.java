...
class MyCalendar2Factory implements RemoteViewsService.RemoteViewsFactory {
    private JSONArray mData;
    private Context mContext;
//    int priority = 1;

    public MyCalendar2Factory(Context context, JSONArray mData) {
        this.mContext = context;
        this.mData = mData;
//        this.priority = priority;
    }

    public MyCalendar2Factory(Context context) {
        mContext = context;
    }
...
//            } else {
            rv = new RemoteViews(mContext.getPackageName(), R.layout.item_my_calendar);
//            }
...
//            rv.setViewLayoutWidth(R.id.image_bg, newWidth, TypedValue.COMPLEX_UNIT_PX);
            if((json).getLong("color") != null) {
                Utility.setImageViewResources(rv, (json).getLong("color") != null ? Long.toHexString((json).getLong("color")) : null, (json).getBoolean("isFinished"));
...
//                }
                ...
                if ((json).getBoolean("isFinished") == true) {
                    rv.setImageViewResource(R.id.image_select, R.drawable.bg_ff7800_corner);
                }
                rv.setImageViewResource(R.id.image_bg, R.drawable.bg_ff7800_corner_light);
            }
