...
class DragAndDropListsState extends State<DragAndDropLists> {
  ...
  _internalOnListReorder(
    ...
    int receiverListIndex = -1;
    for (int i = 0; i < widget.children.length; i++) {
      DragAndDropListInterface item = widget.children[i];
      if ((item == reordered) && reorderedListIndex == -1) {
        reorderedListIndex = i;
      }
      if ((item == receiver) && receiverListIndex == -1) {
        receiverListIndex = i;
      }
      // if ((item == reordered || item.key == reordered.key) && reorderedListIndex == -1) {
      //   reorderedListIndex = i;
      // }
      // if ((item == receiver || item.key == receiver.key) && receiverListIndex == -1) {
      //   receiverListIndex = i;
      // }
    }
