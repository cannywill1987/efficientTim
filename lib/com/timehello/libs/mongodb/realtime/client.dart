...
import 'package:time_hello/com/timehello/libs/mongodb/realtime/Change.dart';
import 'package:time_hello/com/timehello/libs/mongodb/response/MongoDbError.dart';
...
class Client {
  ...
  Future<dynamic> getServerConfiguration() async {
    return MongoDbDio.getInstance().getByUrl(DEFAULT_REAL_TIME_DATA_HOST_HTTP);
  }
  ...
  Future listen(
    ...
    getServerConfiguration().then((data){
      ...
      if (!transports.contains(PROTOCOL_WEBSOCKET)) {
        errorCallback(new MongoDbError(9015, "websocket not supported"));
      } else {
        ///获取配置信息成功后开始进行连接
  ...
  void connect(String session, int heartbeat,
    ...
    webSocket?.listen((event) {
      ...
      switch (code) {
        ...
        case 5:
          {
            ...
            ///服务端发送消息给客户端，数据变化
            if (message.name?.isNotEmpty == true && message.name == "server_pub") {
        ...
        case 7:
          // error
          onError(new MongoDbError(int.parse(parts[2]), parts[3]));
          break;
        case 8:
          // noop
          break;
      ...
      onError(new MongoDbError(9015, error.toString()));
    }, cancelOnError: true);
  }
