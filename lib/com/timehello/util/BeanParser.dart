...
class BeanParser {
  ...
  static List<MissionModel> parseMissionModelListFromGpt<T>(List<Map<String, dynamic>> data) {
    List<Map<String, dynamic>> datas = List<Map<String, dynamic>>.from(data?[0]?['datas']) ?? [];
  List<MissionModel> list = datas.map((d) {
    try {
      ...
      DateTime dateTimeStart = d['startTime'] == null ? DateTime.now() : Utility
          .getUtcDateTimeToLocalFromString(d['startTime']);
      DateTime dateTimeEnd = d['endTime'] == null ? dateTimeStart.add(
          Duration(hours: 1)) : Utility.getUtcDateTimeToLocalFromString(d['endTime']);
      // timeZoneOffset
      // missionModel.priorityStatus = d['priorityStatus'];
  ...
  static List<Map> parseSubmissionModelListToJsonMap<T>(List<SubmissionModel> data) {
    List<Map> listTmp = [];
  ...
  static List<SubmissionModel> parseSubmissionModelList<T>(List<dynamic> data) {
    List<SubmissionModel> list = data.map((d) {
      ...
      submissionModel.key = ValueKey(submissionModel.id ?? Utility.getRandom(from: 0, max: 10000000));
  ...
  static List<EventCollectionModel> parseEventCollectionModel<T>(List<dynamic> data) {
