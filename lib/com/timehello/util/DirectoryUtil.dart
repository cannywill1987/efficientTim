...
class DirectoryUtil {
  ...
  static String? getStoragePath(
      {required String category,  String? fileName,  String? format}) {
    return getPath(
      _storageDir!,
      category: category,
      fileName: fileName,
  ...
  static Directory? createTempDirSync({required String category}) {
    String? path = getTempPath(category: category);
    return createDirSync(path!);
  }

  static Directory? createAppDocDirSync({required String category}) {
    String? path = getAppDocPath(category: category);
    return createDirSync(path!);
  }

  static Directory? createAppSupportDirSync({required String category}) {
    String? path = getAppSupportPath(category: category);
    return createDirSync(path!);
  }

  static Directory? createStorageDirSync({required String category}) {
    String? path = getStoragePath(category: category);
    return createDirSync(path!);
  }

  static Future<Directory?> createTempDir({required String category}) async {
    await initTempDir();
    String? path = getTempPath(category: category);
    return createDir(path!);
  }

  static Future<Directory?> createAppDocDir({required String category}) async {
    await initAppDocDir();
    String? path = getAppDocPath(category: category);
    return createDir(path!);
  }

  static Future<Directory?> createAppSupportDir({required String category}) async {
    await initAppSupportDir();
    String? path = getAppSupportPath(category: category);
    return createDir(path!);
  }

  static Future<Directory?> createStorageDir({required String category}) async {
    await initStorageDir();
    String? path = getStoragePath(category: category);
    return createDir(path!);
  }
