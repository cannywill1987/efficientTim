...
import 'package:time_hello/com/timehello/config/DatabaseTablesSQL.dart';
...
class TablesInit{
  ...
  Future init() async{
    ...
    String path = join(Params.databaseName);
    ...
    List<Map<String, Object?>>? tableMaps = await db?.rawQuery('SELECT name FROM sqlite_master WHERE type = "table"');
  ...
  Future<List<String>> getNoCreateTables(Map<String,String> tableSqls) async {
    ...
    tableMaps?.forEach((item){
      existingTables.add(item['name'] as String);
    });
