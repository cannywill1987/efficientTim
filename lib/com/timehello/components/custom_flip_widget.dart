...
/// There two types of Flip Animations for [CustomFlipWidget].
///
/// - middleFlip is used for CustomFlipWidgets that flip in the middle, like Mechanical Flip Boards do.
/// - spinFlip is used for CustomFlipWidgets that flip like cards do (roll flip).
enum FlipType {
  middleFlip,
  spinFlip,
}
...
class CustomFlipWidget<T> extends StatefulWidget {
  /// CustomFlipWidget constructor.
  ///
  ...
  @override
  State<CustomFlipWidget<T>> createState() => flipType == FlipType.middleFlip
      ? _MiddleCustomFlipWidgetState<T>()
      : _SpinCustomFlipWidgetState<T>();
}

abstract class _CustomFlipWidgetState<T> extends State<CustomFlipWidget<T>>
  ...
  @override
  void didUpdateWidget(covariant CustomFlipWidget<T> oldWidget) {
...
/// CustomFlipWidget state class.
///
/// Performs flip animations as widget.itemStream delivers items.
/// Parameters are documented in [CustomFlipWidget] class constructor.
class _MiddleCustomFlipWidgetState<T> extends _CustomFlipWidgetState<T> {
  @override
  Widget buildFirstFlipPanel() {
  ...
  @override
  Transform? transform2FirstPanel(AxisDirection direction) {
    final isPastMiddle = _flipAnimation.value > _CustomFlipWidgetState._piBy2;
    final isUpOrLeft =
        direction == AxisDirection.up || direction == AxisDirection.left;
...
class _SpinCustomFlipWidgetState<T> extends _CustomFlipWidgetState<T> {
  @override
  Widget buildFirstFlipPanel() {
    final movingPanel = transform2FirstPanel(widget.flipDirection);
