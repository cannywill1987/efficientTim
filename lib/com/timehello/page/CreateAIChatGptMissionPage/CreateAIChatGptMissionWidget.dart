...
class CreateAIChatGptMissionWidget extends BaseWidget {
  /// Creates default getting started calendar
  Function? onRefresh;
  List<MissionModel> listMissionModel = [];

  CreateAIChatGptMissionWidget(
      {Key? key, required this.listMissionModel, Function? onRefresh})
      : super(key: key);
  ...
  BaseWidgetState<BaseWidget<ChangeNotifier>> getState() {
    ...
    listMissionModel.forEach((element) {
      if (element.time_mode == null) {
        ...
        if (element.start_time == null) {
          element.start_time = DateTime.now().millisecondsSinceEpoch +
    ...
    return CreateAIChatGptMissionWidgetState(
        listMissionModel: listMissionModel);
  }
}

class CreateAIChatGptMissionWidgetState
    extends BaseWidgetState<CreateAIChatGptMissionWidget> {
  List<MissionModel> listMissionModel = [];
  ...
  CreateAIChatGptMissionWidgetState({required this.listMissionModel}) {
    calendarModel =
        Utility.initCalendarModelWithMissionList(this.listMissionModel);
  }
  ...
  void initState() {
    ...
    isAppBarVisible = Utility.isHandsetBySize() == true ? false : true;
  ...
  Widget baseBuild(BuildContext context) {
    _events = _DataSource(getAppointmentDetails(calendarModel.dayModelList));
                    ...
                    Row(
                      children: [
                        CommonCalendarHeaderWidget(
                            ...
                            onChange: (data) {
                              if (data != null) {
                                PickerDateRange? dateTimePickerDateRange = data;
                                this.curDateTime =
                        ...
                        // ButtonListWidget(
                        //   list: CONSTANTS.getGptRoleResourceDeliveryInfoBeanList(),
                        //   onTapListener: (obj) {
  ...
  MissionModel? getMissionModelByObjectId(MissionModel objectId) {
    for (int i = 0; i < this.listMissionModel.length; i++) {
      if (this.listMissionModel[i] == objectId) {
        return this.listMissionModel[i];
      }
    }
    return null;
  }
        ...
        Utility.openPagePCAndMobile(context,
            child: CreateMissionPage(
                ...
                popOkCallback: (res) {
                  if (res != null) {
                    Utility.popupPagePCAndMobile(context);
                    this.listMissionModel.add(res);
                    calendarModel = Utility.initCalendarModelWithMissionList(
                        this.listMissionModel);
                    updateUI();
                  }
                },
                onRefresh: () {}));
      ...
      onTap: (CalendarTapDetails details) async {
        ...
        if (appointmentDynamic != null) {
          ...
          this.onClick('onClickSettingItem', {
            'missionModel':
                getMissionModelByObjectId(appointment.id as MissionModel)
          });
        }
        // }
      },
        ...
        if (missionModel?.time_mode == 0 || missionModel?.time_mode == null) {
          if (missionModel?.repetiveType == 0) {
            missionModel?.end_time =
                (appointment.endTime?.millisecondsSinceEpoch ?? 0);
          }
...
// MissionModel getMissionModelByAppointment(Appointment appointment) {
//   return this.listMissionModel[appointment.id as ];
// }
}
