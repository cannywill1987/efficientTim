...
import '../../RichEditor/components/CustomInputWidget.dart';
...
class WQBRichEditorPage extends BaseWidget {
  ...
  BaseWidgetState<BaseWidget<ChangeNotifier>> getState() {
    ...
    return WQBRichEditorPageState();
  }
}

class WQBRichEditorPageState extends BaseWidgetState<WQBRichEditorPage> {
  ...
  String getFileName() {
    ...
    // http://oss.timerbell.com/home/0aa14757-7695-4e52-9b23-45f839a16715/2023-10-17 07:29:06.233717Z
      return this.widget.url!.split("/").last;
    }
  }
  ...
  save() async {
    ...
    BaseBean response = await HttpManager.getInstance().doPostRequest(Apis.uploadOssJSON,
        params: {
          'schema': res,
          "title": getFileName() //文件标题
        },
        context: Utility.getGlobalContext(),
        // callback: (BaseBean response, String scene, bool isFromCache) async {
      // EasyLoadingManager.getInstance().dismissLoading();
      // onOkListener(
      //     {'schema': res, 'plainText': plainText, 'url': response.data});
    // }
    );
    return {'schema': res, 'plainText': plainText, 'url': response.data};
  }
  ...
  requestData() async {
    BaseBean res = await HttpManager.getInstance()
        .doGetFileContentRequest(this.widget.url ?? "");
    loadData(res.data);
  }
