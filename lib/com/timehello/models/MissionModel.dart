...
class MissionModel extends MongoDbObject{
  ...
  List? _subMissions = []; // [{"id":id, isFinished: false, "title": "title", "notificationTime": timestamp, "numToamatoesFocused": 0, "numToamatoTotal": 0, "create_time": 0, "update_time": 0}]
  ...
  List? do_it_now = []; //现在开始做的时间 map {"end_time": 毫秒时间戳, "buffer_end_time": 缓冲时间戳, money: 0, "real_end_time": 0}
  ...
  List<Map<String, dynamic>>? repeativeDate = []; // 重复模式的日期 [{date:"2020-12-12", isFinished: true}]等
  ...
  set subMissions(List? value) {
    ...
    if(this._subMissionModels?.length == 0) {
      ...
      this._subMissionModels?.add(SubmissionModel(isFinished: false,
          id: id,
          shouldFocus: false,
          title: "",
  ...
  set subMissionModels(List<SubmissionModel> value) {
    this._subMissions = BeanParser.parseSubmissionModelListToJsonMap(value ?? []);
